...
def load_mock_employees() -> List[Employee]:
    """Legacy fallback — loads from seed file if DB is empty."""
    try:
        ...
        logger.warning(f"Seed file fallback failed: {e}")
        return []


@router.get("/employees")
async def get_employees(venue_id: Optional[str] = Query(None)):
    """Get employees from MongoDB. Falls back to seed file if collection is empty."""
    db = get_database()
    query = {}
    if venue_id:
        query["venue_id"] = venue_id

    employees = await db.employees.find(query).to_list(length=1000)
    ...
    # Fallback to seed file for backwards compatibility
    logger.info("No employees in DB, falling back to seed file")
...
async def run_payroll(
    ...
    db = get_database()
    db_employees = await db.employees.find({"venue_id": venue_id}).to_list(length=1000)

    if db_employees:
        for emp in db_employees:
            emp["_id"] = str(emp["_id"])
        venue_employees = [Employee(**e) for e in db_employees]
    else:
        # Fallback
        all_employees = load_mock_employees()
        venue_employees = [e for e in all_employees if e.venue_id == venue_id]
