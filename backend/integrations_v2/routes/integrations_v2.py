...
# Default capability matrix — used when no DB records exist
...
def create_integrations_v2_router():
    ...
    async def list_delivery_orders(
        ...
        orders = (
            await db.delivery_orders.find({"venue_id": venue_id}, {"_id": 0})
            .sort("created_at", -1)
            .to_list(100)
        )
    ...
    async def list_menu_mappings(
        ...
        mappings = await db.external_menu_maps.find(query, {"_id": 0}).to_list(500)
    ...
    async def list_connector_capabilities(
        ...
        Reads from DB (connector_capabilities collection) with fallback to defaults.
        """
        db_capabilities = await db.connector_capabilities.find(
            {}, {"_id": 0}
        ).to_list(50)

        if db_capabilities:
            return {"ok": True, "data": db_capabilities}

        # Fallback to built-in defaults
        return {"ok": True, "data": DEFAULT_CAPABILITIES}
