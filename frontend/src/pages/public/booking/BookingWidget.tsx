...
export default function BookingWidget() {
    ...
    const renderStep2 = () => (
        <div className="space-y-4">
            <h3 className="text-sm font-medium text-muted-foreground">Available Slots for {date}</h3>
            <div className="grid grid-cols-3 gap-2 max-h-60 overflow-y-auto">
                {slots.filter(s => s.available).map((slot, idx) => (
                    <Button
                        key={idx}
                        variant={selectedSlot === slot.time ? "default" : "outline"}
                        className={`text-xs ${selectedSlot === slot.time ? 'bg-blue-600 border-blue-600' : 'border-border text-secondary-foreground'}`}
    ...
    const renderStep4 = () => (
        <div className="text-center py-8 space-y-4">
            ...
            <div>
                ...
                <p className="text-muted-foreground mt-1">Ref: {confirmedRes?.id?.split('-')[0]}</p>
            ...
            <Card className="w-full max-w-md bg-card border-border text-foreground shadow-2xl">
                <CardHeader className="text-center pb-2">
                    ...
                    <CardDescription>
                        ...
                        {!source && <span className="text-muted-foreground">Live Availability</span>}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {step === 1 && renderStep1()}
