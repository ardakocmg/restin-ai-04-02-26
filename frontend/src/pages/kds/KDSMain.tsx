...
export default function KDSMain() {
  ...
  return (
    <div className="min-h-screen bg-background">
      {/* Header */}
      <header className="fixed top-0 left-0 right-0 h-16 bg-background/80 backdrop-blur-md border-b border-border z-50 flex items-center justify-between px-4">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-card border border-border">
            ...
            <span className="text-xs font-mono text-muted-foreground">{isOnline ? 'ONLINE' : 'OFFLINE'}</span>
...
function ItemCard({ item, settings, onStart, onReady, onHold, onPassApprove, onDeliver }) {
  ...
  return (
    <div
      className="bg-card/50 backdrop-blur-sm rounded-xl overflow-hidden border border-border hover:border-border transition-all hover:shadow-2xl hover:scale-[1.01]"
      style={{ borderLeft: `4px solid ${statusColor}` }} /* keep-inline */ /* keep-inline */
    >
      {/* Card Header */}
      ...
      {/* Timer Section */}
      <div className="p-4 bg-black/40 border-t border-b border-border">
      ...
      <div
        ...
        style={{ backgroundColor: statusColor }} /* keep-inline */ /* keep-inline */
      >
        {item.status}
      </div>
    </div>
