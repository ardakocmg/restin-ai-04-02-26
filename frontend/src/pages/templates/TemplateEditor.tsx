        ...
        <div className="ts-root">
            ...
            <div className="ts-header">
                <div className="ts-row" style={{ gap: 12 }}> /* keep-inline */
                    ...
                    >
                        <ChevronLeft style={{ width: 20, height: 20 }} /> /* keep-inline */
                    </button>
                    <div>
                        <div className="ts-row" style={{ gap: 8 }}> /* keep-inline */
                            <LayoutTemplate style={{ width: 18, height: 18, color: 'hsl(267 100% 72%)' }} /> /* keep-inline */
                            <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--foreground)' }}> /* keep-inline */
                                {t('Template Wizard')}
                            </span>
                        </div>
                    </div>
                ...
                <div className="ts-row" style={{ gap: 8 }}> /* keep-inline */
                    {templateStatus === 'active' && (
                        <span className="ts-type-chip active-chip" style={{ gap: 4 }}> /* keep-inline */
                            <CheckCircle style={{ width: 12, height: 12 }} /> /* keep-inline */
                            {t('Published')}
                        </span>
                    )}
                    {!isNew && (
                        <button onClick={handlePreview} className="ts-btn ts-btn-ghost" title={t('Preview')}>
                            <Eye style={{ width: 16, height: 16 }} /> /* keep-inline */
                            {t('Preview')}
                        </button>
                    )}
                    <button onClick={handleSave} disabled={saving} className="ts-btn ts-btn-save" title={t('Save Draft')}>
                        <Save style={{ width: 16, height: 16 }} /> /* keep-inline */
                    ...
                    {canPublish && !isNew && (
                        <button onClick={handlePublish} disabled={publishing} className="ts-btn ts-btn-primary" title={t('Publish')}>
                            <Sparkles style={{ width: 16, height: 16 }} /> /* keep-inline */
                            {publishing ? t('Publishing...') : t('Publish')}
                        </button>
                    )}
                    {!canPublish && !isNew && (
                        <div className="ts-type-chip" style={{ background: 'hsl(45 93% 47% / 0.1)', color: 'hsl(45 93% 47%)', border: '1px solid hsl(45 93% 47% / 0.2)', fontSize: 11 }}> /* keep-inline */
                            <AlertTriangle style={{ width: 12, height: 12 }} /> /* keep-inline */
            ...
            <div className="ts-toolbar">
                ...
                <div style={{ width: 1, height: 20, background: 'var(--border)', margin: '0 8px' }} /> /* keep-inline */
                ...
                <input
                    ...
                    style={{ width: 200, background: 'transparent', border: 'none', fontWeight: 600, fontSize: 13 }} /* keep-inline */
                />
                ...
                <select
                    ...
                    style={{ marginLeft: 6 }} /* keep-inline */
                >
                ...
                <div className="ts-zoom">
                    <button className="ts-zoom-btn" onClick={() => setZoom(z => Math.max(z - 10, 40))} title={t('Zoom out')}>
                        <ZoomOut style={{ width: 14, height: 14 }} /> /* keep-inline */
                    </button>
                    <span style={{ minWidth: 32, textAlign: 'center' }}>{zoom}%</span> /* keep-inline */
                    <button className="ts-zoom-btn" onClick={() => setZoom(z => Math.min(z + 10, 200))} title={t('Zoom in')}>
                        <ZoomIn style={{ width: 14, height: 14 }} /> /* keep-inline */
                    </button>
                </div>
            </div>
            ...
            <div className="ts-body">
                ...
                <div className="ts-sidebar">
                    ...
                    {sidebarTab === 'templates' && (
                        <div className="ts-blocks-panel">
                            {BLOCK_CATEGORIES.map(cat => (
                                <div key={cat.title} className="ts-block-category">
                                    ...
                                    {cat.blocks.map(bt => (
                                        ...
                                        >
                                            <GripVertical className="icon" style={{ width: 12, height: 12, opacity: 0.3 }} /> /* keep-inline */
                                            {bt.icon}
                            ...
                            <div style={{ fontSize: 11, color: 'var(--muted-foreground)', textAlign: 'center', marginTop: 12, opacity: 0.6 }}> /* keep-inline */
                    ...
                    {sidebarTab === 'printer' && (
                        <div className="ts-blocks-panel">
                                ...
                                <div key={pr.name} className="ts-printer-card">
                                    <div className="ts-printer-card-header">
                                        ...
                                        <span className={`ts-printer-status ${pr.online ? 'online' : 'offline'}`}>
                                            {pr.online ? <><Wifi style={{ width: 10, height: 10 }} /> Online</> : <><WifiOff style={{ width: 10, height: 10 }} /> Offline</>} /* keep-inline */
                                        </span>
                                    </div>
                            ...
                            <button className="ts-block-item" style={{ justifyContent: 'center', marginTop: 8 }}> /* keep-inline */
                                <Plus className="icon" />
                            ...
                            <div className="ts-asset-grid">
                                ...
                                {assets.map(a => (
                                    <div key={a.id} className="ts-asset-card" onClick={() => { navigator.clipboard.writeText(a.url); toast.success(t('URL copied')); }}>
                                        <Image className="icon" style={{ width: 24, height: 24, color: 'var(--muted-foreground)' }} /> /* keep-inline */
                                        <div className="ts-asset-overlay"><span>{a.name}</span></div>
                                    </div>
                                ))}
                            </div>
                            <input ref={assetInputRef} type="file" accept="image/*" onChange={handleAssetUpload} style={{ display: 'none' }} /> /* keep-inline */
                        </div>
                    )}
                    ...
                    {sidebarTab === 'imports' && (
                        <div className="ts-blocks-panel">
                            ...
                            <input ref={fileInputRef} type="file" accept=".json" onChange={importTemplate} style={{ display: 'none' }} /> /* keep-inline */
                        </div>
                    )}
                    ...
                    {sidebarTab === 'audit' && (
                        <div className="ts-blocks-panel">
                            ...
                            {auditLog.length === 0 ? (
                                <div style={{ fontSize: 12, color: 'var(--muted-foreground)', textAlign: 'center', padding: '24px 0', opacity: 0.6 }}> /* keep-inline */
                ...
                {viewMode === 'canvas' ? (
                    ...
                    >
                        <div className="ts-paper" style={{ width: paperWidthPx }}> /* keep-inline */
                            {/* Top tear */}
                            ...
                            <div className="ts-paper-content" style={{ /* keep-inline */
                                ...
                                {blocks.length === 0 ? (
                                    ...
                                    <React.Fragment key={block.id as string}>
                                        ...
                                        >
                                            <div className="ts-row" style={{ gap: 4 }}> /* keep-inline */
                                                <GripVertical style={{ width: 12, height: 12, color: '#bbb', cursor: 'grab', flexShrink: 0 }} /> /* keep-inline */
                                                <div style={{ flex: 1 }}><CanvasBlockRenderer block={block} /></div> /* keep-inline */
                                            </div>
                                            ...
                                            <div className="ts-canvas-block-actions">
                                                ...
                                                <button className="ts-canvas-block-action delete" title={t('Remove block')} onClick={e => { e.stopPropagation(); removeBlock(block.id as string); }}>
                                                    <Trash2 style={{ width: 12, height: 12 }} /> /* keep-inline */
                                                </button>
                                            </div>
                                        </div>
                    ...
                    <div className="ts-canvas-area">
                        <div className="ts-text-view">
                            ...
                            <div className="ts-text-section">
                                ...
                                <div className="ts-grid-2">
                                    ...
                                    <div className="ts-field">
                                        ...
                                        <div className={`ts-type-chip ${templateStatus === 'active' ? 'active-chip' : 'normal'}`} style={{ marginTop: 4 }}> /* keep-inline */
                                            {templateStatus === 'active' ? <><CheckCircle style={{ width: 12, height: 12 }} /> {t('Published')}</> : t(templateStatus)} /* keep-inline */
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="ts-text-section">
                                ...
                                {versions.length === 0 ? (
                                    <div style={{ fontSize: 12, color: 'var(--muted-foreground)', textAlign: 'center', padding: '16px 0' }}> /* keep-inline */
                            ...
                            {canPublish && !isNew && (
                                <div className="ts-text-section">
                                    ...
                                    <button onClick={handlePublish} disabled={publishing} className="ts-btn ts-btn-primary" style={{ width: '100%', justifyContent: 'center' }}> /* keep-inline */
                                        <Sparkles style={{ width: 16, height: 16 }} /> /* keep-inline */
            ...
            {showPreview && (
                <div style={{ /* keep-inline */
                    ...
                    <div style={{ /* keep-inline */
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid var(--border)' }}> /* keep-inline */
                            <div className="ts-row" style={{ gap: 8 }}> /* keep-inline */
                                <Eye style={{ width: 18, height: 18, color: 'hsl(267 100% 72%)' }} /> /* keep-inline */
                                <span style={{ fontSize: 15, fontWeight: 600, color: 'var(--foreground)' }}>{t('Preview')}</span> /* keep-inline */
                            </div>
                            <div className="ts-row" style={{ gap: 6 }}> /* keep-inline */
                                <button
                                    className={`ts-toolbar-tab ${showThermal ? 'active' : ''}`}
                                    onClick={() => setShowThermal(!showThermal)}
                                    style={{ padding: '4px 12px', borderRadius: 6 }} /* keep-inline */
                                >
                                    {t('Thermal')}
                                </button>
                        ...
                        <div style={{ flex: 1, overflow: 'auto', padding: 24, display: 'flex', justifyContent: 'center', background: 'hsl(240 5% 12%)' }}> /* keep-inline */
                            <div
