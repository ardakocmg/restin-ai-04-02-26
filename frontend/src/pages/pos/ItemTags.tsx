    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 12 }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                {filtered.map(tag => (
                    <div key={tag.id} className="pos-card" style={{ cursor: 'pointer', padding: 14 }} onClick={() => setEditing({ ...tag })}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        <div className="pos-flex pos-flex--center pos-gap-10 pos-mb-8">
                            <span style={{ fontSize: 24 }}>{tag.icon}</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                            <div style={{ flex: 1, minWidth: 0 }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                                <h3 className="pos-modal-title" style={{ fontSize: 15, margin: 0 }}>{tag.name}</h3> /* keep-inline */ /* keep-inline */ /* keep-inline */
                                {tag.description && <div className="pos-cell-secondary" style={{ marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{tag.description}</div>} /* keep-inline */ /* keep-inline */ /* keep-inline */
                                <span className="pos-cell-secondary">{tag.itemCount} items</span>
                            </div>
                            <div style={{ width: 12, height: 12, borderRadius: 3, background: tag.color }} /> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        </div>
                        <div className="pos-flex pos-gap-4 pos-flex--wrap">
                            {tag.showOnPOS && <span className="pos-badge pos-badge--blue" style={{ fontSize: 9 }}>POS</span>} /* keep-inline */ /* keep-inline */ /* keep-inline */
                            {tag.showOnReceipt && <span className="pos-badge pos-badge--green" style={{ fontSize: 9 }}>Receipt</span>} /* keep-inline */ /* keep-inline */ /* keep-inline */
                            {tag.showOnKDS && <span className="pos-badge pos-badge--amber" style={{ fontSize: 9 }}>KDS</span>} /* keep-inline */ /* keep-inline */ /* keep-inline */
                            {tag.showOnWeb && <span className="pos-badge pos-badge--purple" style={{ fontSize: 9 }}>Web</span>} /* keep-inline */ /* keep-inline */ /* keep-inline */
                        </div>
                    </div>
                ))}
            </div>
            ...
            {editing && <div className="pos-modal-overlay" onClick={() => setEditing(null)}>
                <div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pos-form-label pos-text-bold pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Visibility</div> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    <div className="pos-form-grid pos-mb-16">
                        {([['showOnPOS', 'Show on POS'], ['showOnReceipt', 'Show on Receipt'], ['showOnKDS', 'Show on KDS'], ['showOnWeb', 'Show on Website']] as const).map(([key, label]) =>
                            <label key={key} className="pos-toggle-label" style={{ padding: '6px 10px', background: 'var(--bg-secondary)', borderRadius: 6 }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    ...
                    <div className="pos-modal-footer">
                        ...
                        <button title="Delete tag" className="pos-btn-outline" style={{ color: '#EF4444' }} onClick={() => { setTags(p => p.filter(t => t.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button> /* keep-inline */ /* keep-inline */ /* keep-inline */
