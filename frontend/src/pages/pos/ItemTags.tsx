    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 12  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                {filtered.map(tag => (
                    <div key={tag.id} className="pos-card" style={{ cursor: 'pointer', padding: 14  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => setEditing({ ...tag })}>
                        <div className="pos-flex pos-flex--center pos-gap-10 pos-mb-8">
                            <span style={{ fontSize: 24  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{tag.icon}</span>
                            <div style={{ flex: 1, minWidth: 0  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                                <h3 className="pos-modal-title" style={{ fontSize: 15, margin: 0  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{tag.name}</h3>
                                {tag.description && <div className="pos-cell-secondary" style={{ marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{tag.description}</div>}
                        ...
                        <div className="pos-flex pos-gap-4 pos-flex--wrap">
                            {tag.showOnPOS && <span className="pos-badge pos-badge--blue" style={{ fontSize: 9  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>POS</span>}
                            {tag.showOnReceipt && <span className="pos-badge pos-badge--green" style={{ fontSize: 9  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Receipt</span>}
                            {tag.showOnKDS && <span className="pos-badge pos-badge--amber" style={{ fontSize: 9  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>KDS</span>}
                            {tag.showOnWeb && <span className="pos-badge pos-badge--purple" style={{ fontSize: 9  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Web</span>}
                        </div>
                    </div>
                ))}
            </div>
            ...
            {editing && <div className="pos-modal-overlay" onClick={() => setEditing(null)}>
                <div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pos-form-label pos-text-bold pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Visibility</div>
                    ...
                    <div className="pos-modal-footer">
                        ...
                        <button title="Delete tag" className="pos-btn-outline" style={{ color: '#EF4444'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => { setTags(p => p.filter(t => t.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button>
