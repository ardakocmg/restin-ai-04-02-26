        ...
        <div className="pos-page"><div className="pos-container">
            ...
            <div className="pos-toggle-group pos-mb-20" style={{ width: 'fit-content' }}> /* keep-inline */
                {(['users', 'groups'] as TabKey[]).map(t => (
                    <button key={t} onClick={() => setTab(t)} className={`pos-toggle-btn ${tab === t ? 'pos-toggle-btn--active' : ''}`} style={{ padding: '10px 24px', fontSize: 13, fontWeight: tab === t ? 600 : 400 }}>{t === 'users' ? '👤 Users' : '🛡️ Groups'}</button> /* keep-inline */
                ))}
            </div>
            ...
            {loading && <div className="pos-card pos-flex pos-flex--center" style={{ justifyContent: 'center', gap: 8, padding: 40 }}><Loader2 size={18} className="animate-spin" style={{ color: '#3B82F6' }} /><span className="pos-text-secondary">{"Loading "}staff from API...</span></div>} /* keep-inline */
            {error && <div className="pos-card pos-flex pos-flex--between pos-flex--center pos-mb-16" style={{ borderColor: '#EF4444', padding: 16 }}><span style={{ color: '#EF4444', fontSize: 13 }}>⚠ {error} — showing cached data</span><button className="pos-btn-outline" style={{ padding: '6px 14px', fontSize: 12 }} onClick={() => refetch()}>Retry</button></div>} /* keep-inline */

            {/* Users Tab */}
            {tab === 'users' && !loading && <div className="pos-card">
                <div className="pos-table-header" style={{ gridTemplateColumns: userTableCols }}><div></div><div>Name</div><div>Group</div><div>PIN</div><div>Last Login</div><div></div></div> /* keep-inline */
                {filteredUsers.length === 0 && <div className="pos-empty-state">{"No "}staff found. {!venueId && 'Please select a venue first.'}</div>}
                {filteredUsers.map(user => (
                    <div key={user.id} className="pos-table-row" style={{ gridTemplateColumns: userTableCols, opacity: user.isActive ? 1 : 0.5 }} onClick={() => setEditUser({ ...user })}> /* keep-inline */
                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: 'rgba(59,130,246,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 600, color: '#3B82F6' }}>{user.avatar}</div> /* keep-inline */
                        <div><div className="pos-cell-value">{user.name}</div><div className="pos-cell-secondary">{user.email}</div></div>
                        <span className="pos-badge" style={{ background: `${getGroupColor(user.group)}15`, color: getGroupColor(user.group) }}>{user.group}</span> /* keep-inline */
                        <span className="pos-cell-secondary" style={{ fontFamily: 'monospace' }}>{'••••'}</span> /* keep-inline */
            ...
            {/* Groups Tab */}
            {tab === 'groups' && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 16 }}> /* keep-inline */
                {filteredGroups.map(group => (
                    <div key={group.id} className="pos-card" style={{ cursor: 'pointer', borderLeft: `4px solid ${group.color}` }} onClick={() => setEditGroup({ ...group })}> /* keep-inline */
                        <div className="pos-flex pos-flex--between pos-flex--center pos-mb-12">
                            <div className="pos-flex pos-flex--center pos-gap-8"><Shield size={16} style={{ color: group.color }} /><h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>{group.name}</h3></div> /* keep-inline */
                ...
                <div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pos-form-grid">
                        <div><label className="pos-form-label">PIN Code</label>
                            <div style={{ position: 'relative' }}><input type={showPin ? 'text' : 'password'} className="pos-input" value={editUser.pin} onChange={e => setEditUser(p => p ? { ...p, pin: e.target.value } : null)} maxLength={4} /> /* keep-inline */
                    ...
                    <div className="pos-modal-footer">
                        <button className="pos-btn-primary" style={{ flex: 1, justifyContent: 'center' }} onClick={saveUser}><Save size={14} /> Save</button> /* keep-inline */
                        <button title="Delete user" className="pos-btn-outline" style={{ color: '#EF4444', borderColor: 'rgba(239,68,68,0.3)' }} onClick={() => { setLocalUsers(p => p.filter(u => u.id !== editUser.id)); setEditUser(null); toast.success('Deleted'); }}><Trash2 size={14} /></button> /* keep-inline */
                ...
                <div className="pos-card pos-modal" style={{ maxHeight: '85vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}> /* keep-inline */
                    ...
                    <div className="pos-form-group"><label className="pos-form-label pos-mb-4">Permissions</label>
                        <div className="pos-flex pos-flex--col pos-gap-6">
                            {ALL_PERMS.map(perm => <label key={perm} className="pos-flex pos-flex--center pos-gap-10" style={{ padding: '8px 12px', background: 'var(--bg-secondary,#09090b)', borderRadius: 8, cursor: 'pointer', border: editGroup.permissions.includes(perm) ? '1px solid #3B82F6' : '1px solid rgba(255,255,255,0.04)' }}> /* keep-inline */
                    ...
                    <div className="pos-modal-footer">
                        <button className="pos-btn-primary" style={{ flex: 1, justifyContent: 'center' }} onClick={saveGroup}><Save size={14} /> Save</button> /* keep-inline */
