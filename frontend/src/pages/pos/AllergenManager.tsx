        ...
        <div className="pos-page"><div className="pos-container">
            <div className="pos-header">
                <div>
                    ...
                    <p className="pos-subtitle">EU 14 mandatory allergens — enable/disable per your menu{apiWired && <span className="pos-badge--green" style={{ marginLeft: 8, fontSize: 11 }}>● Live</span>}</p> /* keep-inline */
                </div>
                <button className="pos-btn-primary" onClick={() => toast.success('Allergen settings saved')}><Save size={16} /> Save Changes</button>
            </div>

            {/* Loading / Error */}
            {apiLoading && <div className="pos-card pos-flex pos-flex--center" style={{ justifyContent: 'center', gap: 8, padding: 30 }}><Loader2 size={18} className="animate-spin" style={{ color: '#3B82F6' }} /><span className="pos-text-secondary">{"Loading "}allergens...</span></div>} /* keep-inline */
            ...
            {/* Stats */}
            <div className="pos-stats-grid" style={{ gridTemplateColumns: 'repeat(3,1fr)' }}> /* keep-inline */
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 12 }}> /* keep-inline */
                {filtered.map(allergen => (
                    <div key={allergen.id} className="pos-card pos-flex pos-flex--center pos-gap-12" style={{ cursor: 'pointer', padding: 14, borderColor: allergen.isActive ? 'rgba(59,130,246,0.3)' : undefined, opacity: allergen.isActive ? 1 : 0.5 }} onClick={() => toggle(allergen.id)}> /* keep-inline */
                        <span style={{ fontSize: 28 }}>{allergen.icon}</span> /* keep-inline */
                        <div style={{ flex: 1 }}> /* keep-inline */
                            <div className="pos-cell-value" style={{ fontSize: 14, fontWeight: 600 }}>{allergen.name}</div> /* keep-inline */
            ...
            <div className="pos-card" style={{ background: 'rgba(245,158,11,0.04)', borderColor: 'rgba(245,158,11,0.2)', marginTop: 16 }}> /* keep-inline */
                <div className="pos-text-sm pos-text-bold pos-mb-8" style={{ color: '#F59E0B' }}>⚠️ Legal Requirement</div> /* keep-inline */
                <p className="pos-cell-secondary pos-mb-0" style={{ lineHeight: 1.6 }}> /* keep-inline */
