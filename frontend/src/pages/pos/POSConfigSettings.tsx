...
const Toggle: React.FC<{ value: boolean; onChange: () => void }> = ({ value, onChange }) => (
    <div onClick={onChange} style={{ width: 44, height: 24, borderRadius: 12, background: value ? '#3B82F6' : '#3f3f46', cursor: 'pointer', position: 'relative', flexShrink: 0 }}> /* keep-inline */ /* keep-inline */
        ...
        <div className="pos-page">
            <div className="pos-container pos-container--sidebar">
                ...
                <div className="pos-sidebar">
                    <button onClick={() => navigate('/pos/runtime')} className="pos-btn pos-btn--outline" style={{ width: '100%', marginBottom: 16 }}> /* keep-inline */ /* keep-inline */
                        <ArrowLeft size={16} /> Back to POS
                    </button>

                    <div className="pos-card" style={{ padding: 8 }}> /* keep-inline */ /* keep-inline */
                        {TABS.map(tab => (
                            <button
                                key={tab.key}
                    ...
                    {/* Quick Actions */}
                    <div className="pos-card" style={{ padding: 12, marginTop: 12 }}> /* keep-inline */ /* keep-inline */
                        <div className="pos-section-title">Actions</div>
                        <button className="pos-btn pos-btn--outline" style={{ width: '100%', fontSize: 12, padding: '8px 12px', marginBottom: 6 }}><Link size={12} /> View Linked Devices</button> /* keep-inline */ /* keep-inline */
                        <button className="pos-btn pos-btn--outline" style={{ width: '100%', fontSize: 12, padding: '8px 12px', marginBottom: 6 }}><Copy size={12} /> Duplicate Config</button> /* keep-inline */ /* keep-inline */
                    </div>
                </div>
                ...
                <div className="pos-main">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}> /* keep-inline */ /* keep-inline */
                        <h1 style={{ fontSize: 22, fontWeight: 700 }}>{TABS.find(t => t.key === activeTab)?.label}</h1> /* keep-inline */ /* keep-inline */
                        <div style={{ display: 'flex', gap: 12 }}> /* keep-inline */ /* keep-inline */
                        ...
                        <>
                            <div className="pos-card">
                                ...
                                <SettingRow label="Configuration Name" desc="Name displayed in the Back Office">
                                    <input className="pos-input" style={{ width: 250 }} value={config.name} onChange={e = aria-label="Input field"> set('name', e.target.value)} /> /* keep-inline */ /* keep-inline */
                                </SettingRow>
                            </div>
                            <div className="pos-card">
                                <h3 className="pos-section-title">Menu Settings</h3>
                                <SettingRow label="Menu" desc="Select the menu to use for this configuration">
                                    <select className="pos-input pos-select" style={{ width: 250 }} value={config.menu} onChange={e = aria-label="Input field"> set('menu', e.target.value)} aria-label="Menu"> /* keep-inline */ /* keep-inline */
                    ...
                    {activeTab === 'orders' && (
                        <>
                            <div className="pos-card">
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 16 }}> /* keep-inline */ /* keep-inline */
                                ...
                                {config.orderMode === 'table-service' && (
                                    <>
                                        <SettingRow label="Floor Plan" desc="Select from available floor plans">
                                            <select className="pos-input pos-select" style={{ width: 200 }} value={config.selectedFloorPlan} onChange={e = aria-label="Input field"> set('selectedFloorPlan', e.target.value)} aria-label="Floor plan"> /* keep-inline */ /* keep-inline */
                                ...
                                <SettingRow label="Default order profile for direct sales">
                                    <select className="pos-input pos-select" style={{ width: 180 }} value={config.defaultOrderProfile} onChange={e = aria-label="Input field"> set('defaultOrderProfile', e.target.value)} aria-label="Default order profile"> /* keep-inline */ /* keep-inline */
                                ...
                                <SettingRow label="Order number reset at" desc="Set the last order number before count resets">
                                    <input type="number" className="pos-input" style={{ width: 100 }} value={config.orderNumberReset} onChange={e = aria-label="Input field"> set('orderNumberReset', parseInt(e.target.value) || 0)} /> /* keep-inline */ /* keep-inline */
                                </SettingRow>
                                    ...
                                    <>
                                        <SettingRow label="Max time between orders">
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}> /* keep-inline */ /* keep-inline */
                                                <input type="number" className="pos-input" style={{ width: 80 }} value={config.groupMaxTimeBetween} onChange={e = aria-label="Input field"> set('groupMaxTimeBetween', parseInt(e.target.value) || 0)} /> /* keep-inline */ /* keep-inline */
                                                <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>min</span> /* keep-inline */ /* keep-inline */
                                            </div>
                                        </SettingRow>
                                        <SettingRow label="Max time since first order">
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}> /* keep-inline */ /* keep-inline */
                                                <input type="number" className="pos-input" style={{ width: 80 }} value={config.groupMaxTimeSinceFirst} onChange={e = aria-label="Input field"> set('groupMaxTimeSinceFirst', parseInt(e.target.value) || 0)} /> /* keep-inline */ /* keep-inline */
                                                <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>min</span> /* keep-inline */ /* keep-inline */
                                            </div>
                                        </SettingRow>
                                    </>
                                            ...
                                            <>
                                                ...
                                                <SettingRow label="Suggested values">
                                                    <div style={{ display: 'flex', gap: 8 }}> /* keep-inline */ /* keep-inline */
                                                        {config.payTipValues.map((v, idx) => (
                                                            <input aria-label="Input" key={idx} type="number" className="pos-input" style={{ width: 70 }} /* keep-inline */ /* keep-inline */
                                                                value={v}
                                                        ...
                                                        <span style={{ fontSize: 12, color: 'var(--text-secondary)', alignSelf: 'center' }}> /* keep-inline */ /* keep-inline */
                                                ...
                                                {config.payTipType === 'dynamic' && (
                                                    <SettingRow label="Dynamic threshold" desc="Show percentages when total is above this amount">
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}> /* keep-inline */ /* keep-inline */
                                                            <span style={{ fontSize: 14, color: 'var(--text-secondary)' }}>€</span> /* keep-inline */ /* keep-inline */
                                                            <input type="number" className="pos-input" style={{ width: 80 }} value={config.payDynamicThreshold} onChange={e = aria-label="Input field"> set('payDynamicThreshold', parseInt(e.target.value) || 0)} /> /* keep-inline */ /* keep-inline */
                                                        </div>
                                                    </SettingRow>
                                                )}
                                            </>
                        ...
                        <>
                            <div className="pos-card">
                                ...
                                {config.timedEvents.map((event, idx) => (
                                    <div key={idx} className="pos-card" style={{ background: 'var(--bg-secondary, #09090b)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}> /* keep-inline */ /* keep-inline */
                                        <div>
                                            <div style={{ fontWeight: 600 }}>{event.title}</div> /* keep-inline */ /* keep-inline */
                                            <div style={{ fontSize: 12, color: 'var(--text-secondary, #a1a1aa)' }}> /* keep-inline */ /* keep-inline */
                                                {event.menu} • {event.allDay ? 'All day' : `${event.startTime} - ${event.endTime}`} • {event.days.join(', ')}
                                            </div>
                                        </div>
                                        <button className="pos-btn pos-btn--outline" style={{ padding: '6px 10px' }} onClick={() => setConfig(p => ({ ...p, timedEvents: p.timedEvents.filter((_, i) => i !== idx) }))}> /* keep-inline */ /* keep-inline */
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                ))}
                                <button className="pos-btn pos-btn--outline" style={{ fontSize: 13 }} onClick={() => { /* keep-inline */ /* keep-inline */
                                    setConfig(p => ({
                                        ...p,
                            ...
                            <div className="pos-card">
                                ...
                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}> /* keep-inline */ /* keep-inline */
                                    {config.reportTags.map((tag, idx) => (
                                        <span key={idx} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', background: 'rgba(59,130,246,0.1)', border: '1px solid rgba(59,130,246,0.2)', borderRadius: 6, fontSize: 13, color: '#3B82F6' }}> /* keep-inline */ /* keep-inline */
                                            {tag}
                                            <button style={{ background: 'none', border: 'none', color: '#3B82F6', cursor: 'pointer', padding: 0 }} onClick={() => setConfig(p => ({ ...p, reportTags: p.reportTags.filter((_, i) => i !== idx) }))}> /* keep-inline */ /* keep-inline */
                                                <X size={12} />
                                            </button>
                                        </span>
                                    ))}
                            ...
                            <div className="pos-card">
                                ...
                                {config.stockTransfers.map((transfer, idx) => (
                                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}> /* keep-inline */ /* keep-inline */
                                        <select className="pos-input pos-select" style={{ flex: 1 }} value={transfer.from} onChange={e = aria-label="Input field"> setConfig(p => ({ ...p, stockTransfers: p.stockTransfers.map((t, i) => i === idx ? { ...t, from: e.target.value } : t) }))} aria-label="From stock"> /* keep-inline */ /* keep-inline */
                                            <option>Main Stock</option><option>Bar Stock</option><option>Kitchen Stock</option>
                                        </select>
                                        <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>→</span> /* keep-inline */ /* keep-inline */
                                        <select className="pos-input pos-select" style={{ flex: 1 }} value={transfer.to} onChange={e = aria-label="Input field"> setConfig(p => ({ ...p, stockTransfers: p.stockTransfers.map((t, i) => i === idx ? { ...t, to: e.target.value } : t) }))} aria-label="To stock"> /* keep-inline */ /* keep-inline */
                                            <option>Main Stock</option><option>Bar Stock</option><option>Kitchen Stock</option>
                                        </select>
                                        <button className="pos-btn pos-btn--outline" style={{ padding: '6px 10px' }} onClick={() => setConfig(p => ({ ...p, stockTransfers: p.stockTransfers.filter((_, i) => i !== idx) }))}> /* keep-inline */ /* keep-inline */
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                ))}
                                <button className="pos-btn pos-btn--outline" style={{ fontSize: 13 }} onClick={() => setConfig(p => ({ ...p, stockTransfers: [...p.stockTransfers, { from: 'Main Stock', to: 'Bar Stock' }] }))}> /* keep-inline */ /* keep-inline */
