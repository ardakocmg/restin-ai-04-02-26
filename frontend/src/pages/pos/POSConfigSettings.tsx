                        ...
                        <>
                            <div className="pos-card">
                                ...
                                <SettingRow label="Configuration Name" desc="Name displayed in the Back Office">
                                    <input className="pos-input" style={{ width: 250 }} value={config.name} onChange={e = aria-label="Input field"> set('name', e.target.value)} />
                                </SettingRow>
                            </div>
                            <div className="pos-card">
                                <h3 className="pos-section-title">Menu Settings</h3>
                                <SettingRow label="Menu" desc="Select the menu to use for this configuration">
                                    <select className="pos-input pos-select" style={{ width: 250 }} value={config.menu} onChange={e = aria-label="Input field"> set('menu', e.target.value)} aria-label="Menu">
                        ...
                        <>
                            <div className="pos-card">
                                ...
                                {config.orderMode === 'table-service' && (
                                    <>
                                        <SettingRow label="Floor Plan" desc="Select from available floor plans">
                                            <select className="pos-input pos-select" style={{ width: 200 }} value={config.selectedFloorPlan} onChange={e = aria-label="Input field"> set('selectedFloorPlan', e.target.value)} aria-label="Floor plan">
                                ...
                                <SettingRow label="Default order profile for direct sales">
                                    <select className="pos-input pos-select" style={{ width: 180 }} value={config.defaultOrderProfile} onChange={e = aria-label="Input field"> set('defaultOrderProfile', e.target.value)} aria-label="Default order profile">
                                ...
                                <SettingRow label="Order number reset at" desc="Set the last order number before count resets">
                                    <input type="number" className="pos-input" style={{ width: 100 }} value={config.orderNumberReset} onChange={e = aria-label="Input field"> set('orderNumberReset', parseInt(e.target.value) || 0)} />
                                </SettingRow>
                                    ...
                                    <>
                                        <SettingRow label="Max time between orders">
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                <input type="number" className="pos-input" style={{ width: 80 }} value={config.groupMaxTimeBetween} onChange={e = aria-label="Input field"> set('groupMaxTimeBetween', parseInt(e.target.value) || 0)} />
                                                <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>min</span>
                                            </div>
                                        </SettingRow>
                                        <SettingRow label="Max time since first order">
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                <input type="number" className="pos-input" style={{ width: 80 }} value={config.groupMaxTimeSinceFirst} onChange={e = aria-label="Input field"> set('groupMaxTimeSinceFirst', parseInt(e.target.value) || 0)} />
                                                <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>min</span>
                                            </div>
                                        </SettingRow>
                                    </>
                                                ...
                                                <SettingRow label="Suggested values">
                                                    <div style={{ display: 'flex', gap: 8 }}>
                                                        {config.payTipValues.map((v, idx) => (
                                                            <input aria-label="Input" key={idx} type="number" className="pos-input" style={{ width: 70 }}
                                                                value={v}
                                                ...
                                                {config.payTipType === 'dynamic' && (
                                                    <SettingRow label="Dynamic threshold" desc="Show percentages when total is above this amount">
                                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                            ...
                                                            <input type="number" className="pos-input" style={{ width: 80 }} value={config.payDynamicThreshold} onChange={e = aria-label="Input field"> set('payDynamicThreshold', parseInt(e.target.value) || 0)} />
                                                        </div>
                                                    </SettingRow>
                                                )}
                                            </>
                            ...
                            <div className="pos-card">
                                ...
                                {config.stockTransfers.map((transfer, idx) => (
                                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                                        <select className="pos-input pos-select" style={{ flex: 1 }} value={transfer.from} onChange={e = aria-label="Input field"> setConfig(p => ({ ...p, stockTransfers: p.stockTransfers.map((t, i) => i === idx ? { ...t, from: e.target.value } : t) }))} aria-label="From stock">
                                            <option>Main Stock</option><option>Bar Stock</option><option>Kitchen Stock</option>
                                        </select>
                                        <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>→</span>
                                        <select className="pos-input pos-select" style={{ flex: 1 }} value={transfer.to} onChange={e = aria-label="Input field"> setConfig(p => ({ ...p, stockTransfers: p.stockTransfers.map((t, i) => i === idx ? { ...t, to: e.target.value } : t) }))} aria-label="To stock">
                                            <option>Main Stock</option><option>Bar Stock</option><option>Kitchen Stock</option>
                                        </select>
