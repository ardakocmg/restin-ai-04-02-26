            ...
            <div style={s.modal} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ ...s.paper, maxWidth: format === 'thermal' ? 320 : 500 }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    ...
                    <div style={s.receiptInfo}>
                        <div style={s.infoRow}>
                            <span>Receipt #:</span>
                            <span style={{ fontWeight: 700 }}>{receiptNumber}</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        </div>
                        <div style={s.infoRow}>
                            <span>Date:</span>
                    ...
                    <div style={s.itemsSection}>
                        <div style={s.itemHeader}>
                            <span style={{ flex: 1 }}>Item</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                            <span style={{ width: 40, textAlign: 'center' }}>Qty</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                            <span style={{ width: 70, textAlign: 'right' }}>Amount</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        </div>
                        {(items || []).map((item, idx) => {
                            const price = (item.unit_price || item.price || 0) * (item.qty || 1);
                            return (
                                <div key={item.id || idx} style={s.itemRow}>
                                    <span style={{ flex: 1, fontSize: 12 }}>{item.menu_item_name || item.name}</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                                    <span style={{ width: 40, textAlign: 'center', fontSize: 12 }}>{item.qty || 1}</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                                    <span style={{ width: 70, textAlign: 'right', fontSize: 12, fontWeight: 600 }}>€{price.toFixed(2)}</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                                </div>
                            );
                        })}
                    </div>
