            ...
            <div style={s.modal} onClick={e => e.stopPropagation()}>
                ...
                <div style={s.actionBar}>
                    <div style={{ display: 'flex', gap: 4 }}> /* keep-inline */ /* keep-inline */
                        <button
                            style={{ ...s.formatBtn, backgroundColor: format === 'thermal' ? '#2A9D8F' : '#333' }} /* keep-inline */ /* keep-inline */
                            onClick={() => setFormat('thermal')}
                        >
                            Thermal
                        </button>
                        <button
                            style={{ ...s.formatBtn, backgroundColor: format === 'a4' ? '#2A9D8F' : '#333' }} /* keep-inline */ /* keep-inline */
                ...
                {/* Receipt Paper */}
                <div style={{ ...s.paper, maxWidth: format === 'thermal' ? 320 : 500 }}> /* keep-inline */ /* keep-inline */
                    ...
                    <div style={s.receiptInfo}>
                        <div style={s.infoRow}>
                            <span>Receipt #:</span>
                            <span style={{ fontWeight: 700 }}>{receiptNumber}</span> /* keep-inline */ /* keep-inline */
                        </div>
                        <div style={s.infoRow}>
                            <span>Date:</span>
                    ...
                    <div style={s.itemsSection}>
                        <div style={s.itemHeader}>
                            <span style={{ flex: 1 }}>Item</span> /* keep-inline */ /* keep-inline */
                            <span style={{ width: 40, textAlign: 'center' }}>Qty</span> /* keep-inline */ /* keep-inline */
                            <span style={{ width: 70, textAlign: 'right' }}>Amount</span> /* keep-inline */ /* keep-inline */
                        </div>
                        {(items || []).map((item, idx) => {
                            const price = (item.unit_price || item.price || 0) * (item.qty || 1);
                            return (
                                <div key={item.id || idx} style={s.itemRow}>
                                    <span style={{ flex: 1, fontSize: 12 }}>{item.menu_item_name || item.name}</span> /* keep-inline */ /* keep-inline */
                                    <span style={{ width: 40, textAlign: 'center', fontSize: 12 }}>{item.qty || 1}</span> /* keep-inline */ /* keep-inline */
                                    <span style={{ width: 70, textAlign: 'right', fontSize: 12, fontWeight: 600 }}>€{price.toFixed(2)}</span> /* keep-inline */ /* keep-inline */
                                </div>
                            );
                        })}
                    </div>
                    ...
                    {order?.tender_type && (
                        <>
                            ...
                            {order.tender_type === 'ROOM_CHARGE' && order.room_number && (
                                <div style={{ ...s.paymentMethod, marginTop: 2 }}> /* keep-inline */ /* keep-inline */
                    ...
                    <div style={s.divider} />
                    <div style={s.footer}>
                        <div style={{ fontSize: 11, textAlign: 'center', color: '#666', marginBottom: 8 }}> /* keep-inline */ /* keep-inline */
                            Thank you for dining with us!
                        </div>
                        <div style={{ fontSize: 10, textAlign: 'center', color: '#888' }}> /* keep-inline */ /* keep-inline */
                            Powered by Restin.AI
                        </div>
                        {/* Barcode placeholder */}
                        <div style={s.barcode}>
                            {'|'.repeat(40).split('').map((_, i) => (
                                <div key={i} style={{ width: Math.random() > 0.5 ? 2 : 1, height: 30, backgroundColor: '#333' }} /> /* keep-inline */ /* keep-inline */
                            ))}
                        </div>
                    </div>
                </div>
