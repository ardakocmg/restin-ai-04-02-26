        ...
        <div style={pg}>
            <div style={ct}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}> /* keep-inline */
                    <div>
                        <button onClick={() => navigate(-1)} style={{ ...bo, marginBottom: 8, padding: '6px 14px', fontSize: 12 }}><ArrowLeft size={14} /> Back</button> /* keep-inline */
                        <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>Item Library</h1> /* keep-inline */
                        <p style={{ fontSize: 13, color: 'var(--text-secondary, #a1a1aa)', margin: '4px 0 0' }}>Manage all POS items, combos, and item groups{apiWired && <span style={{ marginLeft: 8, fontSize: 11, color: '#10B981' }}>● Live</span>}</p> /* keep-inline */
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}> /* keep-inline */
                        <button style={{ ...bo, fontSize: 12, padding: '8px 14px' }}><Upload size={14} /> Import</button> /* keep-inline */
                        <button style={{ ...bo, fontSize: 12, padding: '8px 14px' }}><Download size={14} /> Export</button> /* keep-inline */
                ...
                {/* Loading / Error */}
                {apiLoading && <div style={{ ...cd, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: 30 }}><Loader2 size={18} className="animate-spin" style={{ color: '#3B82F6' }} /><span style={{ color: 'var(--text-secondary)' }}>{"Loading "}items from API...</span></div>} /* keep-inline */
                {apiError && <div style={{ ...cd, borderColor: '#EF4444', padding: 14, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ color: '#EF4444', fontSize: 13 }}>⚠ {apiError}</span><button style={{ ...bo, padding: '6px 14px', fontSize: 12 }} onClick={() => refetch()}>Retry</button></div>} /* keep-inline */
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12, marginBottom: 20 }}> /* keep-inline */
                    {[
                        ...
                        <div key={i} style={{ ...cd, padding: 16, display: 'flex', alignItems: 'center', gap: 12 }}> /* keep-inline */
                            <div style={{ width: 36, height: 36, borderRadius: 8, background: `${s.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: s.color }}>{s.icon}</div> /* keep-inline */
                            <div>
                                <div style={{ fontSize: 20, fontWeight: 700 }}>{s.value}</div> /* keep-inline */
                                <div style={{ fontSize: 12, color: 'var(--text-secondary, #a1a1aa)' }}>{s.label}</div> /* keep-inline */
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {/* Toolbar */}
                <div style={{ ...cd, padding: 12, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}> /* keep-inline */
                    <div style={{ position: 'relative', flex: 1, minWidth: 200 }}> /* keep-inline */
                        <Search size={14} style={{ position: 'absolute', left: 12, top: 10, color: 'var(--text-secondary)' }} />
                        <input style={{ ...ip, paddingLeft: 34, fontSize: 13, padding: '8px 12px 8px 34px' }} placeholder="Search items by name or SKU..." value={search} onChange={e => setSearch(e.target.value)} aria-label="Search items" /> /* keep-inline */
                    </div>

                    <button style={{ ...bo, padding: '8px 12px', fontSize: 12, ...(showFilters ? { background: 'rgba(59,130,246,0.1)', borderColor: '#3B82F6', color: '#3B82F6' } : {}) }} onClick={() => setShowFilters(!showFilters)}> /* keep-inline */
                        <Filter size={14} /> Filters {showFilters ? '▴' : '▾'}
                    </button>

                    <div style={{ display: 'flex', border: '1px solid var(--border-primary, #27272a)', borderRadius: 8, overflow: 'hidden' }}> /* keep-inline */
                        <button title="List view" onClick={() => setViewMode('list')} style={{ padding: '7px 10px', background: viewMode === 'list' ? 'rgba(59,130,246,0.1)' : 'transparent', border: 'none', color: viewMode === 'list' ? '#3B82F6' : 'var(--text-secondary)', cursor: 'pointer' }}><List size={14} /></button> /* keep-inline */
                        <button title="Grid view" onClick={() => setViewMode('grid')} style={{ padding: '7px 10px', background: viewMode === 'grid' ? 'rgba(59,130,246,0.1)' : 'transparent', border: 'none', color: viewMode === 'grid' ? '#3B82F6' : 'var(--text-secondary)', cursor: 'pointer' }}><Grid3X3 size={14} /></button> /* keep-inline */
                    </div>

                    <div style={{ display: 'flex', border: '1px solid var(--border-primary, #27272a)', borderRadius: 8, overflow: 'hidden' }}> /* keep-inline */
                        <button onClick={() => setShowArchived(false)} style={{ padding: '7px 14px', fontSize: 12, background: !showArchived ? 'rgba(59,130,246,0.1)' : 'transparent', border: 'none', color: !showArchived ? '#3B82F6' : 'var(--text-secondary)', cursor: 'pointer' }}>Active</button> /* keep-inline */
                        <button onClick={() => setShowArchived(true)} style={{ padding: '7px 14px', fontSize: 12, background: showArchived ? 'rgba(59,130,246,0.1)' : 'transparent', border: 'none', color: showArchived ? '#3B82F6' : 'var(--text-secondary)', cursor: 'pointer' }}>Archived</button> /* keep-inline */
                    </div>
                </div>

                {/* Filters Row */}
                {showFilters && (
                    <div style={{ ...cd, padding: 14, display: 'flex', gap: 12, alignItems: 'center' }}> /* keep-inline */
                        <select style={{ ...sl, width: 200, fontSize: 12, padding: '8px 12px' }} value={filterGroup} onChange={e => setFilterGroup(e.target.value)} aria-label="Filter by group"> /* keep-inline */
                            <option value="all">All Accounting Groups</option>
                            {GROUPS.map(g => <option key={g} value={g}>{g}</option>)}
                        </select>
                        <select style={{ ...sl, width: 160, fontSize: 12, padding: '8px 12px' }} value={filterType} onChange={e => setFilterType(e.target.value as typeof filterType)} aria-label="Filter by type"> /* keep-inline */
                            <option value="all">All Types</option>
                            <option value="single">Single Items</option>
                            <option value="combo">Combos</option>
                            <option value="item-group">Item Groups</option>
                        </select>
                        <select style={{ ...sl, width: 160, fontSize: 12, padding: '8px 12px' }} value={filterCenter} onChange={e => setFilterCenter(e.target.value)} aria-label="Filter by production center"> /* keep-inline */
                            <option value="all">All Centers</option>
                            {CENTERS.map(c => <option key={c} value={c}>{c}</option>)}
                        </select>
                        <button style={{ ...bo, padding: '8px 12px', fontSize: 12, marginLeft: 'auto' }} onClick={() => { setFilterGroup('all'); setFilterType('all'); setFilterCenter('all'); }}><X size={12} /> Clear</button> /* keep-inline */
                    </div>
                )}

                {/* Bulk Actions */}
                {selectedIds.size > 0 && (
                    <div style={{ ...cd, padding: 12, display: 'flex', alignItems: 'center', gap: 12, background: 'rgba(59,130,246,0.05)', borderColor: 'rgba(59,130,246,0.2)' }}> /* keep-inline */
                        <Check size={16} color="#3B82F6" />
                        <span style={{ fontSize: 13, fontWeight: 600 }}>{selectedIds.size} selected</span> /* keep-inline */
                        <div style={{ display: 'flex', gap: 8, marginLeft: 'auto' }}> /* keep-inline */
                            <button style={{ ...bo, padding: '6px 12px', fontSize: 12 }} onClick={bulkArchive}><Archive size={12} /> {showArchived ? 'Unarchive' : 'Archive'}</button> /* keep-inline */
                            <div style={{ position: 'relative' }}> /* keep-inline */
                                <button style={{ ...bo, padding: '6px 12px', fontSize: 12 }} onClick={() => setShowBulkMenu(!showBulkMenu)}> /* keep-inline */
                                ...
                                {showBulkMenu && (
                                    <div style={{ position: 'absolute', top: '100%', left: 0, marginTop: 4, background: 'var(--bg-card, #18181b)', border: '1px solid var(--border-primary, #27272a)', borderRadius: 8, padding: 4, zIndex: 100, minWidth: 200, boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
                                        {GROUPS.map(g => (
                                            <button key={g} onClick={() => bulkAssignGroup(g)} style={{ width: '100%', padding: '8px 12px', background: 'none', border: 'none', color: 'var(--text-primary)', fontSize: 12, textAlign: 'left', cursor: 'pointer', borderRadius: 4 }}> /* keep-inline */
                                                {g}
                                            </button>
                                        ))}
                                    </div>
                            ...
                            <button style={{ ...bo, padding: '6px 12px', fontSize: 12, color: '#EF4444', borderColor: 'rgba(239,68,68,0.2)' }} onClick={bulkDelete}><Trash2 size={12} /> Delete</button> /* keep-inline */
                            <button style={{ ...bo, padding: '6px 12px', fontSize: 12 }} onClick={() => setSelectedIds(new Set())}><X size={12} /> Clear</button> /* keep-inline */
                        </div>
                    </div>
                )}
                ...
                {viewMode === 'list' ? (
                    <div style={cd}>
                        {/* Header Row */}
                        <div style={{ display: 'grid', gridTemplateColumns: '32px 1fr 130px 100px 130px 100px 80px', gap: 12, padding: '8px 12px', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', color: 'var(--text-secondary, #a1a1aa)', letterSpacing: 0.5, borderBottom: '1px solid rgba(255,255,255,0.06)' }}> /* keep-inline */
                            <div style={{ cursor: 'pointer' }} onClick={selectAll}> /* keep-inline */
                                <div style={{ width: 16, height: 16, borderRadius: 4, border: selectedIds.size === filtered.length && filtered.length > 0 ? '2px solid #3B82F6' : '2px solid var(--border-primary, #27272a)', background: selectedIds.size === filtered.length && filtered.length > 0 ? '#3B82F6' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}> /* keep-inline */
                        ...
                        {/* Items */}
                        {filtered.map(item => (
                            <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '32px 1fr 130px 100px 130px 100px 80px', gap: 12, padding: '12px 12px', alignItems: 'center', borderBottom: '1px solid rgba(255,255,255,0.03)', cursor: 'pointer' }} /* keep-inline */
                                onClick={() => setEditingItem({ ...item })}>
                                <div onClick={e => { e.stopPropagation(); toggleSelect(item.id); }} style={{ cursor: 'pointer' }}> /* keep-inline */
                                    <div style={{ width: 16, height: 16, borderRadius: 4, border: selectedIds.has(item.id) ? '2px solid #3B82F6' : '2px solid var(--border-primary, #27272a)', background: selectedIds.has(item.id) ? '#3B82F6' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center' }}> /* keep-inline */
                                        {selectedIds.has(item.id) && <Check size={10} color="#fff" />}
                                    </div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}> /* keep-inline */
                                    <div style={{ width: 32, height: 32, borderRadius: 6, background: `${item.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: item.color, flexShrink: 0 }}> /* keep-inline */
                                        {item.hasImage ? <Image size={14} /> : <Package size={14} />}
                                    </div>
                                    <div>
                                        <div style={{ fontSize: 13, fontWeight: 500 }}>{item.name}</div> /* keep-inline */
                                        <div style={{ fontSize: 11, color: 'var(--text-secondary)', display: 'flex', gap: 6 }}> /* keep-inline */
                                            {item.sku && <span>{item.sku}</span>}
                                            {item.allergens.length > 0 && <span>⚠ {item.allergens.length} allergens</span>}
                                        </div>
                                    </div>
                                </div>
                                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{item.accountingGroup.split(' - ')[1] || item.accountingGroup}</div> /* keep-inline */
                                <div>
                                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 4, background: item.type === 'combo' ? 'rgba(139,92,246,0.1)' : 'rgba(59,130,246,0.1)', color: item.type === 'combo' ? '#8B5CF6' : '#3B82F6' }}> /* keep-inline */
                                        {item.type === 'single' ? 'Single' : item.type === 'combo' ? 'Combo' : 'Group'}
                                    </span>
                                </div>
                                <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{item.productionCenter}</div> /* keep-inline */
                                <div style={{ fontSize: 13, fontWeight: 600 }}>€{item.price.toFixed(2)}</div> /* keep-inline */
                                <div style={{ display: 'flex', gap: 4 }}> /* keep-inline */
                                    <button title="Edit item" style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', padding: 4 }} onClick={e => { e.stopPropagation(); setEditingItem({ ...item }); }}><Edit3 size={13} /></button> /* keep-inline */
                                    <button title="Duplicate item" style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', padding: 4 }} onClick={e => { e.stopPropagation(); setItems(prev => [...prev, { ...item, id: crypto.randomUUID(), name: `${item.name} (Copy)` }]); toast.success('Item duplicated'); }}><Copy size={13} /></button> /* keep-inline */
                                </div>
                            </div>
                        ))}

                        {filtered.length === 0 && (
                            <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-secondary)' }}> /* keep-inline */
                                <Package size={36} style={{ opacity: 0.3, marginBottom: 8 }} /> /* keep-inline */
                                <p style={{ fontSize: 14, fontWeight: 500 }}>{"No "}items found</p> /* keep-inline */
                                <p style={{ fontSize: 12 }}>{search ? 'Try a different search term' : 'Create your first item to get started'}</p> /* keep-inline */
                            </div>
                        )}
                    </div>
                ) : (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 12 }}> /* keep-inline */
                        {filtered.map(item => (
                            <div key={item.id} style={{ ...cd, padding: 14, cursor: 'pointer' }} onClick={() => setEditingItem({ ...item })}> /* keep-inline */
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}> /* keep-inline */
                                    <div style={{ width: 40, height: 40, borderRadius: 8, background: `${item.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: item.color }}> /* keep-inline */
                                        <Package size={18} />
                                    </div>
                                    <span style={{ fontSize: 11, padding: '2px 6px', borderRadius: 4, background: item.type === 'combo' ? 'rgba(139,92,246,0.1)' : 'rgba(59,130,246,0.1)', color: item.type === 'combo' ? '#8B5CF6' : '#3B82F6' }}> /* keep-inline */
                                        {item.type}
                                    </span>
                                </div>
                                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 4 }}>{item.name}</div> /* keep-inline */
                                <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 8 }}>{item.accountingGroup}</div> /* keep-inline */
                                <div style={{ fontSize: 18, fontWeight: 700, color: '#10B981' }}>€{item.price.toFixed(2)}</div> /* keep-inline */
                            </div>
                        ))}
                    </div>
                )}

                <div style={{ padding: '12px 0', fontSize: 12, color: 'var(--text-secondary)', textAlign: 'center' }}> /* keep-inline */
            ...
            {editingItem && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setEditingItem(null)}> /* keep-inline */
                    <div style={{ ...cd, width: 560, maxHeight: '85vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}> /* keep-inline */
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}> /* keep-inline */
                            <h3 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>{items.find(i => i.id === editingItem.id) ? 'Edit Item' : 'Create New Item'}</h3> /* keep-inline */
                            <button title="Close" style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setEditingItem(null)}><X size={20} /></button> /* keep-inline */
                        </div>

                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}> /* keep-inline */
                            <div style={{ gridColumn: '1 / -1' }}> /* keep-inline */
                            ...
                            <div>
                                <label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Course</label> /* keep-inline */
                            ...
                            <div>
                                <label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Cost Price (€)</label> /* keep-inline */
                            ...
                            <div>
                                <label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Accounting Group</label> /* keep-inline */
                            ...
                            <div>
                                <label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Production Center</label> /* keep-inline */
                            ...
                            <div>
                                <label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Tax Profile</label> /* keep-inline */
                            ...
                            <div>
                                <label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>SKU</label> /* keep-inline */
                                <input style={ip} value={editingItem.sku} onChange={e => setEditingItem(p => p ? { ...p, sku: e.target.value } : null)} placeholder="e.g. MN-001" />
                            </div>
                            <div>
                                <label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Barcode</label> /* keep-inline */
                        ...
                        <div style={{ marginTop: 16 }}> /* keep-inline */
                            <label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 6, display: 'block' }}>Allergens</label> /* keep-inline */
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}> /* keep-inline */
                                {ALLERGENS.map(a => (
                        ...
                        {editingItem.price > 0 && editingItem.costPrice > 0 && (
                            <div style={{ marginTop: 16, padding: 12, background: 'var(--bg-secondary, #09090b)', borderRadius: 8, fontSize: 12 }}> /* keep-inline */
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}> /* keep-inline */
                                    <span style={{ color: 'var(--text-secondary)' }}>Margin:</span> /* keep-inline */
                                    <span style={{ fontWeight: 600, color: '#10B981' }}>{((1 - editingItem.costPrice / editingItem.price) * 100).toFixed(1)}%</span> /* keep-inline */
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between' }}> /* keep-inline */
                                    <span style={{ color: 'var(--text-secondary)' }}>Profit:</span> /* keep-inline */
                                    <span style={{ fontWeight: 600 }}>€{(editingItem.price - editingItem.costPrice).toFixed(2)}</span> /* keep-inline */
                                </div>
                            </div>
                        )}

                        <div style={{ display: 'flex', gap: 8, marginTop: 20 }}> /* keep-inline */
                            <button style={{ ...bp, flex: 1, justifyContent: 'center' }} onClick={saveItem}><Save size={14} /> Save Item</button> /* keep-inline */
