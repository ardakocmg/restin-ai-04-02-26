...
const ProductionCenters: React.FC = () => {
    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            {apiLoading && <div className="pos-card pos-flex pos-flex--center" style={{ justifyContent: 'center', gap: 8, padding: 30 }}><Loader2 size={18} className="animate-spin" style={{ color: '#3B82F6' }} /><span className="pos-text-secondary">{"Loading "}production centers...</span></div>} /* keep-inline */ /* keep-inline */
            {apiError && <div className="pos-card pos-flex pos-flex--between pos-flex--center" style={{ borderColor: '#EF4444', padding: 14 }}><span style={{ color: '#EF4444', fontSize: 13 }}>⚠ {apiError}</span><button className="pos-btn-outline" style={{ padding: '6px 14px', fontSize: 12 }} onClick={() => refetch()}>Retry</button></div>} /* keep-inline */ /* keep-inline */
            ...
            <div className="pos-stats-grid pos-mb-20">
                {[
                    ...
                    <div key={i} className="pos-stat-card">
                        <div className="pos-stat-icon" style={{ background: `${s.color}15`, color: s.color }}>{s.icon}</div> /* keep-inline */ /* keep-inline */
            ...
            {/* Active Centers */}
            <div className="pos-text-sm pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Active Centers ({active.length})</div> /* keep-inline */ /* keep-inline */
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 16, marginBottom: 24 }}> /* keep-inline */ /* keep-inline */
                {active.map(center => (
                    <div key={center.id} className="pos-card" style={{ cursor: 'pointer', borderLeft: `4px solid ${center.color}` }} onClick={() => setEditing({ ...center })}> /* keep-inline */ /* keep-inline */
                        <div className="pos-flex pos-flex--between" style={{ alignItems: 'flex-start', marginBottom: 10 }}> /* keep-inline */ /* keep-inline */
                            <div className="pos-flex pos-flex--center pos-gap-10">
                                <div className="pos-stat-icon" style={{ background: `${center.color}15`, color: center.color }}><Factory size={16} /></div> /* keep-inline */ /* keep-inline */
                                <div><h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>{center.name}</h3> /* keep-inline */ /* keep-inline */
                                    <span className="pos-cell-secondary" style={{ textTransform: 'capitalize' }}>{center.type}</span></div> /* keep-inline */ /* keep-inline */
                            </div>
                            <button title="Edit center" className="pos-btn-icon" onClick={e => { e.stopPropagation(); setEditing({ ...center }); }}><Edit3 size={13} /></button>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 10 }}> /* keep-inline */ /* keep-inline */
                            <div style={{ padding: '8px 12px', background: 'var(--bg-secondary,#09090b)', borderRadius: 6 }}> /* keep-inline */ /* keep-inline */
                                <div style={{ fontSize: 18, fontWeight: 700 }}>{center.itemCount}</div><div className="pos-cell-secondary">Items</div></div> /* keep-inline */ /* keep-inline */
                            <div style={{ padding: '8px 12px', background: 'var(--bg-secondary,#09090b)', borderRadius: 6 }}> /* keep-inline */ /* keep-inline */
                                <div style={{ fontSize: 18, fontWeight: 700 }}>{center.avgPrepTime}m</div><div className="pos-cell-secondary">Avg Prep</div></div> /* keep-inline */ /* keep-inline */
                        </div>
                        <div className="pos-cell-secondary pos-flex pos-flex--col pos-gap-4">
                            ...
                            {center.backupPrinter !== 'None' && <div className="pos-flex pos-flex--center pos-gap-6" style={{ opacity: 0.6 }}><Printer size={11} /> Backup: {center.backupPrinter}</div>} /* keep-inline */ /* keep-inline */
            ...
            {/* Inactive */}
            {inactive.length > 0 && <>
                <div className="pos-text-sm pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Inactive ({inactive.length})</div> /* keep-inline */ /* keep-inline */
                <div className="pos-card" style={{ opacity: 0.5 }}> /* keep-inline */ /* keep-inline */
                    {inactive.map(c => <div key={c.id} className="pos-flex pos-flex--center pos-gap-12" style={{ padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.03)', cursor: 'pointer' }} onClick={() => setEditing({ ...c })}> /* keep-inline */ /* keep-inline */
                        <div style={{ width: 12, height: 12, borderRadius: 3, background: c.color }} /> <span style={{ fontSize: 14, flex: 1 }}>{c.name}</span> /* keep-inline */ /* keep-inline */
                        <button className="pos-btn-outline" style={{ padding: '4px 10px', fontSize: 11 }} onClick={e => { e.stopPropagation(); setCenters(prev => prev.map(x => x.id === c.id ? { ...x, isActive: true } : x)); toast.success('Activated'); }}>Activate</button> /* keep-inline */ /* keep-inline */
                    </div>)}
                </div>
            </>}
        </div>
            ...
            {editing && <div className="pos-modal-overlay" onClick={() => setEditing(null)}>
                <div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pos-form-group"><label className="pos-form-label pos-mb-4">Color</label>
                        <div className="pos-flex pos-flex--wrap pos-gap-6">{COLORS.map(c => <div key={c} onClick={() => setEditing(p => p ? { ...p, color: c } : null)} style={{ width: 28, height: 28, borderRadius: 6, background: c, cursor: 'pointer', border: editing.color === c ? '3px solid #fff' : '3px solid transparent' }} />)}</div></div> /* keep-inline */ /* keep-inline */
                    ...
                    <div className="pos-flex pos-flex--center pos-flex--between pos-mb-16" style={{ padding: '12px 0' }}><span className="pos-cell-value">Active</span> /* keep-inline */ /* keep-inline */
                    ...
                    <div className="pos-modal-footer">
                        ...
                        <button title="Delete center" className="pos-btn-outline" style={{ color: '#EF4444', borderColor: 'rgba(239,68,68,0.3)' }} onClick={() => { setCenters(prev => prev.filter(c => c.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button> /* keep-inline */ /* keep-inline */
