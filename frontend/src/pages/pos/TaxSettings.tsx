 ...
 */
import { ArrowLeft,Edit3,FileText,Percent,Plus,Save,Search,Trash2,Wifi,X } from 'lucide-react';
    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div className="pos-toggle-group pos-mb-20" style={{ width: 'fit-content'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                {(['rates', 'profiles'] as TabKey[]).map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={`pos-toggle-btn ${activeTab === tab ? 'pos-toggle-btn--active' : ''}`} style={{ padding: '10px 24px', fontSize: 13, fontWeight: activeTab === tab ? 600 : 400  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{tab === 'rates' ? '📊 Tax Rates' : '📋 Tax Profiles'}</button>))}
            </div>
            ...
            {activeTab === 'rates' && <div className="pos-card">
                <div className="pos-table-header" style={{ gridTemplateColumns: rateTableCols  /* keep-inline */ }} /* keep-inline */ /* keep-inline */><div>Name</div><div>Rate</div><div>Default</div><div>Accounting Ref</div><div>Status</div><div>Actions</div></div>
                {filteredRates.map(rate => (<div key={rate.id} className="pos-table-row" style={{ gridTemplateColumns: rateTableCols  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => setEditingRate({ ...rate })}>
                    ...
                    <span style={{ fontSize: 15, fontWeight: 700, color: '#F59E0B'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{rate.rate}%</span>
                    <div>{rate.isDefault && <span className="pos-badge pos-badge--blue" style={{ fontSize: 10, fontWeight: 600  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>DEFAULT</span>}</div>
                    <span className="pos-cell-secondary" style={{ fontFamily: 'monospace'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{rate.accountingRef}</span>
            ...
            {activeTab === 'profiles' && <div className="pos-card">
                <div className="pos-table-header" style={{ gridTemplateColumns: profileTableCols  /* keep-inline */ }} /* keep-inline */ /* keep-inline */><div>Profile Name</div><div>Default Tax Rate</div><div>Rules</div><div>Items</div><div>Actions</div></div>
                {filteredProfiles.map(profile => (<div key={profile.id} className="pos-table-row" style={{ gridTemplateColumns: profileTableCols  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => setEditingProfile({ ...profile })}>
                    <div className="pos-flex pos-flex--center pos-gap-8"><FileText size={14} color="#3B82F6" /><div><span className="pos-cell-value">{profile.name}</span>{!profile.isActive && <span className="pos-cell-red" style={{ fontSize: 10, marginLeft: 6  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>(Inactive)</span>}</div></div>
            ...
            {editingRate && <div className="pos-modal-overlay" onClick={() => setEditingRate(null)}><div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                ...
                <div className="pos-modal-footer"><button className="pos-btn-primary" style={{ flex: 1, justifyContent: 'center'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={saveRate}><Save size={14} /> Save</button><button className="pos-btn-outline" onClick={() => setEditingRate(null)}>Cancel</button></div>
            </div></div>}
            ...
            {editingProfile && <div className="pos-modal-overlay" onClick={() => setEditingProfile(null)}><div className="pos-card pos-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="pos-form-group">
                    <div className="pos-text-xs pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Conditional Rules</div>
                    ...
                    {editingProfile.rules.map((rule, idx) => (<div key={idx} className="pos-flex pos-gap-8 pos-mb-8 pos-flex--center">
                        <select className="pos-select" style={{ flex: 1, padding: '8px 12px', fontSize: 12  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ value={rule.orderType} onChange={e => { const nr = [...editingProfile.rules]; nr[idx] = { ...rule, orderType: e.target.value }; setEditingProfile(p => p ? { ...p, rules: nr } : null); }} aria-label="Order type">{ORDER_TYPES.map(t => <option key={t}>{t}</option>)}</select>
                        <span className="pos-text-secondary">→</span>
                        <select className="pos-select" style={{ flex: 1, padding: '8px 12px', fontSize: 12  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ value={rule.taxRate} onChange={e => { const nr = [...editingProfile.rules]; nr[idx] = { ...rule, taxRate: e.target.value }; setEditingProfile(p => p ? { ...p, rules: nr } : null); }} aria-label="Tax rate">{rates.filter(r => r.isActive).map(r => <option key={r.id} value={r.name}>{r.name} ({r.rate}%)</option>)}</select>
                    ...
                    <button className="pos-btn-outline" style={{ padding: '6px 12px', fontSize: 12  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => setEditingProfile(p => p ? { ...p, rules: [...p.rules, { orderType: ORDER_TYPES[0], taxRate: rates[0]?.name || '' }] } : null)}><Plus size={12} /> Add Rule</button>
                </div>
                ...
                <div className="pos-modal-footer"><button className="pos-btn-primary" style={{ flex: 1, justifyContent: 'center'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={saveProfile}><Save size={14} /> Save</button><button className="pos-btn-outline" onClick={() => setEditingProfile(null)}>Cancel</button></div>
            </div></div>}
        </div>
    );
};
