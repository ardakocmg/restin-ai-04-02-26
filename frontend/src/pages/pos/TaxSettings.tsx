...
const TaxSettings: React.FC = () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    useEffect(() => { if (apiRates?.length > 0) { setRates(apiRates.map((r: Record<string, unknown>) => ({ id: r.id || r._id || crypto.randomUUID(), name: r.name || '', rate: r.rate ?? 0, isDefault: r.isDefault ?? r.is_default ?? false, isActive: r.isActive ?? r.is_active ?? true, accountingRef: r.accountingRef ?? r.accounting_ref ?? '' }))); setIsLive(true); } }, [apiRates]);
    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div className="pos-toggle-group pos-mb-20" style={{ width: 'fit-content' }}> /* keep-inline */ /* keep-inline */
                {(['rates', 'profiles'] as TabKey[]).map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={`pos-toggle-btn ${activeTab === tab ? 'pos-toggle-btn--active' : ''}`} style={{ padding: '10px 24px', fontSize: 13, fontWeight: activeTab === tab ? 600 : 400 }}>{tab === 'rates' ? '📊 Tax Rates' : '📋 Tax Profiles'}</button>))} /* keep-inline */ /* keep-inline */
            </div>
            ...
            {activeTab === 'rates' && <div className="pos-card">
                <div className="pos-table-header" style={{ gridTemplateColumns: rateTableCols }}><div>Name</div><div>Rate</div><div>Default</div><div>Accounting Ref</div><div>Status</div><div>Actions</div></div> /* keep-inline */ /* keep-inline */
                {filteredRates.map(rate => (<div key={rate.id} className="pos-table-row" style={{ gridTemplateColumns: rateTableCols }} onClick={() => setEditingRate({ ...rate })}> /* keep-inline */ /* keep-inline */
                    ...
                    <span style={{ fontSize: 15, fontWeight: 700, color: '#F59E0B' }}>{rate.rate}%</span> /* keep-inline */ /* keep-inline */
                    <div>{rate.isDefault && <span className="pos-badge pos-badge--blue" style={{ fontSize: 10, fontWeight: 600 }}>DEFAULT</span>}</div> /* keep-inline */ /* keep-inline */
                    <span className="pos-cell-secondary" style={{ fontFamily: 'monospace' }}>{rate.accountingRef}</span> /* keep-inline */ /* keep-inline */
            ...
            {activeTab === 'profiles' && <div className="pos-card">
                <div className="pos-table-header" style={{ gridTemplateColumns: profileTableCols }}><div>Profile Name</div><div>Default Tax Rate</div><div>Rules</div><div>Items</div><div>Actions</div></div> /* keep-inline */ /* keep-inline */
                {filteredProfiles.map(profile => (<div key={profile.id} className="pos-table-row" style={{ gridTemplateColumns: profileTableCols }} onClick={() => setEditingProfile({ ...profile })}> /* keep-inline */ /* keep-inline */
                    <div className="pos-flex pos-flex--center pos-gap-8"><FileText size={14} color="#3B82F6" /><div><span className="pos-cell-value">{profile.name}</span>{!profile.isActive && <span className="pos-cell-red" style={{ fontSize: 10, marginLeft: 6 }}>(Inactive)</span>}</div></div> /* keep-inline */ /* keep-inline */
            ...
            {editingRate && <div className="pos-modal-overlay" onClick={() => setEditingRate(null)}><div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                ...
                <div className="pos-form-group"><label className="pos-form-label">Name *</label><input className="pos-input" value={editingRate.name} onChange={e = aria-label="Input field"> setEditingRate(p => p ? { ...p, name: e.target.value } : null)} placeholder="e.g. Standard Rate" /></div>
                <div className="pos-form-grid"><div><label className="pos-form-label">Rate (%)</label><input type="number" step="0.1" className="pos-input" value={editingRate.rate} onChange={e = aria-label="Input field"> setEditingRate(p => p ? { ...p, rate: parseFloat(e.target.value) || 0 } : null)} aria-label="Tax rate percentage" /></div><div><label className="pos-form-label">Accounting Ref</label><input className="pos-input" value={editingRate.accountingRef} onChange={e = aria-label="Input field"> setEditingRate(p => p ? { ...p, accountingRef: e.target.value } : null)} placeholder="e.g. VAT-18" /></div></div>
                <div className="pos-flex pos-gap-16 pos-mb-16"><label className="pos-toggle-label"><input type="checkbox" checked={editingRate.isDefault} onChange={() = aria-label="Input field"> setEditingRate(p => p ? { ...p, isDefault: !p.isDefault } : null)} /> Default rate</label><label className="pos-toggle-label"><input type="checkbox" checked={editingRate.isActive} onChange={() = aria-label="Input field"> setEditingRate(p => p ? { ...p, isActive: !p.isActive } : null)} /> Active</label></div>
                <div className="pos-modal-footer"><button className="pos-btn-primary" style={{ flex: 1, justifyContent: 'center' }} onClick={saveRate}><Save size={14} /> Save</button><button className="pos-btn-outline" onClick={() => setEditingRate(null)}>Cancel</button></div> /* keep-inline */ /* keep-inline */
            </div></div>}
            ...
            {editingProfile && <div className="pos-modal-overlay" onClick={() => setEditingProfile(null)}><div className="pos-card pos-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="pos-form-group"><label className="pos-form-label">Profile Name *</label><input className="pos-input" value={editingProfile.name} onChange={e = aria-label="Input field"> setEditingProfile(p => p ? { ...p, name: e.target.value } : null)} placeholder="e.g. Standard 18%" /></div>
                <div className="pos-form-group"><label className="pos-form-label">Default Tax Rate</label><select className="pos-select" value={editingProfile.defaultTaxRate} onChange={e = aria-label="Input field"> setEditingProfile(p => p ? { ...p, defaultTaxRate: e.target.value } : null)} aria-label="Default tax rate">{rates.filter(r => r.isActive).map(r => <option key={r.id} value={r.name}>{r.name} ({r.rate}%)</option>)}</select></div>
                <div className="pos-form-group">
                    <div className="pos-text-xs pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Conditional Rules</div> /* keep-inline */ /* keep-inline */
                    ...
                    {editingProfile.rules.map((rule, idx) => (<div key={idx} className="pos-flex pos-gap-8 pos-mb-8 pos-flex--center">
                        <select className="pos-select" style={{ flex: 1, padding: '8px 12px', fontSize: 12 }} value={rule.orderType} onChange={e = aria-label="Input field"> { const nr = [...editingProfile.rules]; nr[idx] = { ...rule, orderType: e.target.value }; setEditingProfile(p => p ? { ...p, rules: nr } : null); }} aria-label="Order type">{ORDER_TYPES.map(t => <option key={t}>{t}</option>)}</select> /* keep-inline */ /* keep-inline */
                        <span className="pos-text-secondary">→</span>
                        <select className="pos-select" style={{ flex: 1, padding: '8px 12px', fontSize: 12 }} value={rule.taxRate} onChange={e = aria-label="Input field"> { const nr = [...editingProfile.rules]; nr[idx] = { ...rule, taxRate: e.target.value }; setEditingProfile(p => p ? { ...p, rules: nr } : null); }} aria-label="Tax rate">{rates.filter(r => r.isActive).map(r => <option key={r.id} value={r.name}>{r.name} ({r.rate}%)</option>)}</select> /* keep-inline */ /* keep-inline */
                    ...
                    <button className="pos-btn-outline" style={{ padding: '6px 12px', fontSize: 12 }} onClick={() => setEditingProfile(p => p ? { ...p, rules: [...p.rules, { orderType: ORDER_TYPES[0], taxRate: rates[0]?.name || '' }] } : null)}><Plus size={12} /> Add Rule</button> /* keep-inline */ /* keep-inline */
                </div>
                <div className="pos-mb-16"><label className="pos-toggle-label"><input type="checkbox" checked={editingProfile.isActive} onChange={() = aria-label="Input field"> setEditingProfile(p => p ? { ...p, isActive: !p.isActive } : null)} /> Active</label></div>
                <div className="pos-modal-footer"><button className="pos-btn-primary" style={{ flex: 1, justifyContent: 'center' }} onClick={saveProfile}><Save size={14} /> Save</button><button className="pos-btn-outline" onClick={() => setEditingProfile(null)}>Cancel</button></div> /* keep-inline */ /* keep-inline */
            </div></div>}
        </div>
    );
};
