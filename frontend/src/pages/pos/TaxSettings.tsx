    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            {activeTab === 'rates' && <div className="pos-card">
                ...
                {filteredRates.map(rate => (<div key={rate.id} className="pos-table-row" style={{ gridTemplateColumns: rateTableCols }} onClick={() => setEditingRate({ ...rate })}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    ...
                    <span style={{ fontSize: 15, fontWeight: 700, color: '#F59E0B' }}>{rate.rate}%</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    <div>{rate.isDefault && <span className="pos-badge pos-badge--blue" style={{ fontSize: 10, fontWeight: 600 }}>DEFAULT</span>}</div> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    <span className="pos-cell-secondary" style={{ fontFamily: 'monospace' }}>{rate.accountingRef}</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
            ...
            {activeTab === 'profiles' && <div className="pos-card">
                <div className="pos-table-header" style={{ gridTemplateColumns: profileTableCols }}><div>Profile Name</div><div>Default Tax Rate</div><div>Rules</div><div>Items</div><div>Actions</div></div> /* keep-inline */ /* keep-inline */ /* keep-inline */
                {filteredProfiles.map(profile => (<div key={profile.id} className="pos-table-row" style={{ gridTemplateColumns: profileTableCols }} onClick={() => setEditingProfile({ ...profile })}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    <div className="pos-flex pos-flex--center pos-gap-8"><FileText size={14} color="#3B82F6" /><div><span className="pos-cell-value">{profile.name}</span>{!profile.isActive && <span className="pos-cell-red" style={{ fontSize: 10, marginLeft: 6 }}>(Inactive)</span>}</div></div> /* keep-inline */ /* keep-inline */ /* keep-inline */
            ...
            {editingRate && <div className="pos-modal-overlay" onClick={() => setEditingRate(null)}><div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                ...
                <div className="pos-modal-footer"><button className="pos-btn-primary" style={{ flex: 1, justifyContent: 'center' }} onClick={saveRate}><Save size={14} /> Save</button><button className="pos-btn-outline" onClick={() => setEditingRate(null)}>Cancel</button></div> /* keep-inline */ /* keep-inline */ /* keep-inline */
            </div></div>}
            ...
            {editingProfile && <div className="pos-modal-overlay" onClick={() => setEditingProfile(null)}><div className="pos-card pos-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="pos-form-group">
                    <div className="pos-text-xs pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Conditional Rules</div> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    ...
                    <button className="pos-btn-outline" style={{ padding: '6px 12px', fontSize: 12 }} onClick={() => setEditingProfile(p => p ? { ...p, rules: [...p.rules, { orderType: ORDER_TYPES[0], taxRate: rates[0]?.name || '' }] } : null)}><Plus size={12} /> Add Rule</button> /* keep-inline */ /* keep-inline */ /* keep-inline */
                </div>
                ...
                <div className="pos-modal-footer"><button className="pos-btn-primary" style={{ flex: 1, justifyContent: 'center' }} onClick={saveProfile}><Save size={14} /> Save</button><button className="pos-btn-outline" onClick={() => setEditingProfile(null)}>Cancel</button></div> /* keep-inline */ /* keep-inline */ /* keep-inline */
            </div></div>}
        </div>
    );
};
