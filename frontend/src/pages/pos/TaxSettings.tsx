            ...
            {editingProfile && <div className="pos-modal-overlay" onClick={() => setEditingProfile(null)}><div className="pos-card pos-modal" onClick={e => e.stopPropagation()}>
                ...
                <div className="pos-form-group">
                    ...
                    {editingProfile.rules.map((rule, idx) => (<div key={idx} className="pos-flex pos-gap-8 pos-mb-8 pos-flex--center">
                        <select className="pos-select" style={{ flex: 1, padding: '8px 12px', fontSize: 12 }} value={rule.orderType} onChange={e = aria-label="Input field"> { const nr = [...editingProfile.rules]; nr[idx] = { ...rule, orderType: e.target.value }; setEditingProfile(p => p ? { ...p, rules: nr } : null); }} aria-label="Order type">{ORDER_TYPES.map(t => <option key={t}>{t}</option>)}</select>
