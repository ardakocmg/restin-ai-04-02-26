...
const KitchenAnalytics: React.FC = () => {
    ...
    return (
        <div className="pos-page"><div className="pos-container" style={{ maxWidth: 1200 }}> /* keep-inline */ /* keep-inline */
            <div className="pos-header">
                ...
                <div className="pos-toggle-group">
                    {(['today', 'week', 'month'] as const).map(p => (
                        <button key={p} onClick={() => setPeriod(p)} className={`pos-toggle-btn ${period === p ? 'pos-toggle-btn--active' : ''}`} style={{ textTransform: 'capitalize' }}>{p}</button> /* keep-inline */ /* keep-inline */
                    ))}
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}> /* keep-inline */ /* keep-inline */
                ...
                <div className="pos-card">
                    ...
                    {STATIONS.map(station => (
                        <div key={station.name} className="pos-flex pos-flex--center pos-gap-12" style={{ padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}> /* keep-inline */ /* keep-inline */
                            ...
                            <div style={{ textAlign: 'right' }}> /* keep-inline */ /* keep-inline */
                                <div style={{ fontSize: 16, fontWeight: 700 }}>{station.avgTime}m</div> /* keep-inline */ /* keep-inline */
                                <div className="pos-flex pos-flex--center pos-gap-4" style={{ fontSize: 11, justifyContent: 'flex-end', color: station.trend < 0 ? '#10B981' : '#EF4444' }}> /* keep-inline */ /* keep-inline */
                ...
                <div className="pos-card">
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 180, paddingBottom: 20, position: 'relative' }}> /* keep-inline */ /* keep-inline */
                        {HOURLY.map(h => {
                            const pct = (h.avgTime / maxTime) * 100; return (
                                <div key={h.hour} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}> /* keep-inline */ /* keep-inline */
                                    <span style={{ fontSize: 9, fontWeight: 600, color: h.avgTime > 15 ? '#EF4444' : '#3B82F6' }}>{h.avgTime}m</span> /* keep-inline */ /* keep-inline */
                ...
                {/* Slowest Items */}
                <div className="pos-card">
                    <h3 className="pos-card-title pos-flex pos-flex--center pos-gap-6"><AlertTriangle size={14} style={{ color: '#F59E0B' }} /> Slowest Items</h3> /* keep-inline */ /* keep-inline */
                    {TOP_SLOW.map((item, i) => (
                        <div key={item.name} className="pos-flex pos-flex--center pos-gap-10" style={{ padding: '8px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}> /* keep-inline */ /* keep-inline */
                            <span style={{ width: 20, height: 20, borderRadius: 4, background: 'rgba(239,68,68,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: '#EF4444' }}>{i + 1}</span> /* keep-inline */ /* keep-inline */
                            <div style={{ flex: 1 }}> /* keep-inline */ /* keep-inline */
                                <div className="pos-cell-value">{item.name}</div>
                                <div className="pos-cell-secondary">{item.orders} orders today</div>
                            </div>
                            <span style={{ fontSize: 15, fontWeight: 700, color: '#EF4444' }}>{item.avg}m</span> /* keep-inline */ /* keep-inline */
                        </div>
                    ))}
                </div>

                {/* Speed Tips */}
                <div className="pos-card" style={{ background: 'rgba(16,185,129,0.04)', border: '1px solid rgba(16,185,129,0.2)' }}> /* keep-inline */ /* keep-inline */
                    <h3 className="pos-card-title" style={{ color: '#10B981' }}>💡 Speed Insights</h3> /* keep-inline */ /* keep-inline */
                    {[
                        ...
                        <div key={i} className="pos-cell-secondary" style={{ padding: '6px 0', borderBottom: i < 3 ? '1px solid rgba(255,255,255,0.04)' : 'none', lineHeight: 1.5 }}>{tip}</div> /* keep-inline */ /* keep-inline */
                    ))}
                </div>
            </div>
        </div></div>
