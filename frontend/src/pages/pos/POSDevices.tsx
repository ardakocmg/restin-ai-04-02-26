...
const TYPE_ICONS: Record<DeviceType, React.ReactNode> = { terminal: <Monitor size={18} />, printer: <Printer size={18} />, scanner: <span style={{ fontSize: 16  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>📷</span>, drawer: <span style={{ fontSize: 16  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>💰</span>, display: <Smartphone size={18} />, payment: <span style={{ fontSize: 16  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>💳</span> };
        ...
        <div className="pos-page"><div className="pos-container pos-container--1200">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                <div>
                    ...
                    <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>POS Devices</h1>
                    <p style={{ fontSize: 13, color: 'var(--text-secondary,#a1a1aa)', margin: '4px 0 0'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{online} online · {offline} offline · {devices.length} total</p>
                </div>
            ...
            {/* Filter Bar */}
            <div className="pos-tab-group" style={{ marginBottom: 16, flexWrap: 'wrap'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                <button onClick={() => setFilterType('all')} className={`pos-filter-btn${filterType === 'all' ? ' pos-filter-btn--active' : ''}`}>All ({devices.length})</button>
                {(['terminal', 'printer', 'scanner', 'drawer', 'display', 'payment'] as DeviceType[]).map(t => { const count = devices.filter(d => d.type === t).length; return count > 0 ? <button key={t} onClick={() => setFilterType(t)} style={{ padding: '6px 14px', borderRadius: 6, border: 'none', fontSize: 12, cursor: 'pointer', textTransform: 'capitalize', background: filterType === t ? `${TYPE_COLORS[t]}15` : 'transparent', color: filterType === t ? TYPE_COLORS[t] : 'var(--text-secondary)'  /* keep-inline */ }}>{t} ({count})</button> : null; })}
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 12  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                {filtered.map(device => (
                    <div key={device.id} className="pos-card" style={{ cursor: 'pointer', padding: 16, opacity: device.isActive ? 1 : 0.5  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => setEditing({ ...device })}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                                ...
                                <div><div style={{ fontSize: 14, fontWeight: 600  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{device.name}</div>
                                    <div style={{ fontSize: 11, color: 'var(--text-secondary)'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{device.model}</div></div>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                                {device.status === 'online' ? <Wifi size={12} style={{ color: '#10B981'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ /> : <WifiOff size={12} style={{ color: STATUS_COLORS[device.status]  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ />}
                                <span style={{ fontSize: 10, color: STATUS_COLORS[device.status], textTransform: 'capitalize'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{device.status}</span>
                            </div>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, fontSize: 11, color: 'var(--text-secondary)'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
            ...
            {editing && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => setEditing(null)}>
                <div className="pos-card" style={{ width: 480, maxHeight: '85vh', overflow: 'auto'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={e => e.stopPropagation()}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                        <h3 style={{ fontSize: 18, fontWeight: 700, margin: 0  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{devices.find(d => d.id === editing.id) ? 'Edit' : 'New'} Device</h3>
                        <button style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => setEditing(null)}><X size={20} /></button>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Type</label>
                            <select className="pos-input pos-select" value={editing.type} onChange={e => setEditing(p => p ? { ...p, type: e.target.value as DeviceType } : null)} aria-label="Device type">
                                {(['terminal', 'printer', 'scanner', 'drawer', 'display', 'payment'] as DeviceType[]).map(t => <option key={t} value={t} style={{ textTransform: 'capitalize'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{t}</option>)}
                            </select></div>
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Model</label>
                            <input className="pos-input" value={editing.model} onChange={e => setEditing(p => p ? { ...p, model: e.target.value } : null)} /></div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>IP / Connection</label>
                        ...
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Location</label>
                            <input className="pos-input" value={editing.location} onChange={e => setEditing(p => p ? { ...p, location: e.target.value } : null)} /></div>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Serial Number</label>
                            <input className="pos-input" value={editing.serialNumber} onChange={e => setEditing(p => p ? { ...p, serialNumber: e.target.value } : null)} /></div>
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Firmware</label>
                            <input className="pos-input" value={editing.firmware} onChange={e => setEditing(p => p ? { ...p, firmware: e.target.value } : null)} /></div>
                    </div>
                    <div style={{ marginBottom: 16  /* keep-inline */ }} /* keep-inline */ /* keep-inline */><label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                        <input type="checkbox" checked={editing.isActive} onChange={() => setEditing(p => p ? { ...p, isActive: !p.isActive } : null)} /> Active</label></div>
                    <div style={{ display: 'flex', gap: 8  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                        <button className="pos-btn pos-btn--primary" style={{ flex: 1, justifyContent: 'center'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={save}><Save size={14} /> Save</button>
                        <button className="pos-btn pos-btn--outline" style={{ color: '#EF4444'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => { setDevices(p => p.filter(d => d.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button>
