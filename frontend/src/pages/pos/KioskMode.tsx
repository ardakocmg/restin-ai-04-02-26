            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                ...
                <div>
                    <div className="pos-card">
                        <h3 className="pos-card-title pos-flex pos-flex--center pos-gap-6"><Settings size={16} style={{ color: '#3B82F6' }} /> General</h3> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        ...
                        <div className="pos-setting-row">
                            ...
                            <div className="pos-flex pos-gap-4">
                                {(['dine-in', 'takeaway', 'both'] as const).map(t => (
                                    <button key={t} onClick={() => upd('orderType', t)} className={`pos-btn-outline ${config.orderType === t ? 'pos-toggle-btn--active' : ''}`} style={{ padding: '4px 12px', fontSize: 11, textTransform: 'capitalize' }}>{t}</button> /* keep-inline */ /* keep-inline */ /* keep-inline */
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="pos-card">
                        <h3 className="pos-card-title pos-flex pos-flex--center pos-gap-6"><CreditCard size={16} style={{ color: '#10B981' }} /> Payment</h3> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        ...
                        <div className="pos-form-group" style={{ marginTop: 8 }}><label className="pos-form-label">Accepted Methods</label> /* keep-inline */ /* keep-inline */ /* keep-inline */
                            <div className="pos-flex pos-gap-6 pos-flex--wrap">{PAYMENTS.map(p => { const on = config.acceptedPayments.includes(p); return <button key={p} onClick={() => upd('acceptedPayments', on ? config.acceptedPayments.filter(x => x !== p) : [...config.acceptedPayments, p])} className={`pos-btn-outline ${on ? 'pos-badge--green' : ''}`} style={{ padding: '4px 12px', fontSize: 11, background: on ? 'rgba(16,185,129,0.1)' : undefined, color: on ? '#10B981' : undefined, borderColor: on ? 'rgba(16,185,129,0.3)' : undefined }}>{p}</button>; })}</div></div> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        <div className="pos-form-group" style={{ marginTop: 12 }}><label className="pos-form-label">Max Order Value (€)</label> /* keep-inline */ /* keep-inline */ /* keep-inline */
                ...
                <div>
                    <div className="pos-card">
                        <h3 className="pos-card-title pos-flex pos-flex--center pos-gap-6"><Monitor size={16} style={{ color: '#F59E0B' }} /> Display</h3> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    ...
                    <div className="pos-card">
                        ...
                        <div className="pos-form-group" style={{ marginBottom: 14 }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                            ...
                            <div className="pos-flex pos-gap-6">{['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899'].map(c => (<div key={c} onClick={() => upd('accentColor', c)} style={{ width: 28, height: 28, borderRadius: 6, background: c, cursor: 'pointer', border: config.accentColor === c ? '3px solid #fff' : '3px solid transparent' }} />))}</div> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        </div>
                        <div className="pos-form-group">
                            <label className="pos-form-label">Language</label>
                            <div className="pos-flex pos-gap-6">
                                {([['en', 'English'], ['mt', 'Maltese'], ['it', 'Italian'], ['multi', 'Multi']] as const).map(([v, l]) => (
                                    <button key={v} onClick={() => upd('language', v)} className={`pos-btn-outline ${config.language === v ? 'pos-toggle-btn--active' : ''}`} style={{ padding: '4px 12px', fontSize: 11 }}>{l}</button> /* keep-inline */ /* keep-inline */ /* keep-inline */
                                ))}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="pos-card">
                        <h3 className="pos-card-title pos-flex pos-flex--center pos-gap-6"><ShieldCheck size={16} style={{ color: '#EF4444' }} /> Security</h3> /* keep-inline */ /* keep-inline */ /* keep-inline */
