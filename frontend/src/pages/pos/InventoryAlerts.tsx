...
const InventoryAlerts: React.FC = () => {
    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            {/* Loading / Error */}
            {apiLoading && <div className="pos-card pos-flex pos-flex--center" style={{ justifyContent: 'center', gap: 8, padding: 30 }}><Loader2 size={18} className="animate-spin" style={{ color: '#3B82F6' }} /><span className="pos-text-secondary">{"Loading "}stock data...</span></div>} /* keep-inline */
            {apiError && <div className="pos-card pos-flex pos-flex--between pos-flex--center pos-mb-16" style={{ borderColor: '#EF4444', padding: 14 }}><span style={{ color: '#EF4444', fontSize: 13 }}>⚠ {apiError}</span><button className="pos-btn-outline" style={{ padding: '6px 14px', fontSize: 12 }} onClick={() => refetch()}>Retry</button></div>} /* keep-inline */
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 16 }}> /* keep-inline */
                <div>
                    {/* Filters */}
                    <div className="pos-flex pos-gap-8 pos-mb-12">
                        <div className="pos-search-wrapper" style={{ flex: 1 }}><Search size={14} className="pos-search-icon" /><input className="pos-input pos-search-input" placeholder="Search items..." value={search} onChange={e => setSearch(e.target.value)} /></div> /* keep-inline */
                        <div className="pos-toggle-group">
                            {['all', 'out', 'critical', 'low', 'ok'].map(s => <button key={s} onClick={() => setFilterStatus(s)} className={`pos-toggle-btn ${filterStatus === s ? 'pos-toggle-btn--active' : ''}`} style={{ textTransform: 'capitalize', color: filterStatus === s ? (STATUS_COLORS[s] || '#3B82F6') : undefined }}>{s}</button>)} /* keep-inline */
                        </div>
                    </div>
                    ...
                    <div className="pos-card">
                        {filtered.map(item => (
                            <div key={item.id} className="pos-flex pos-flex--center pos-gap-12" style={{ padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}> /* keep-inline */
                                <div style={{ width: 8, height: 8, borderRadius: '50%', background: STATUS_COLORS[item.status], flexShrink: 0 }} /> /* keep-inline */
                                <div style={{ flex: 1 }}> /* keep-inline */
                                    <div className="pos-cell-value">{item.name}</div>
                                    <div className="pos-cell-secondary">{item.category}</div>
                                </div>
                                <div style={{ textAlign: 'center', minWidth: 80 }}> /* keep-inline */
                                    <div style={{ fontSize: 15, fontWeight: 700, color: STATUS_COLORS[item.status] }}>{item.currentStock} {item.unit}</div> /* keep-inline */
                                    <div className="pos-cell-secondary" style={{ fontSize: 10 }}>of {item.parLevel} par</div> /* keep-inline */
                                </div>
                                <div style={{ width: 100, height: 6, background: 'rgba(255,255,255,0.06)', borderRadius: 3, overflow: 'hidden' }}> /* keep-inline */
                ...
                <div>
                    <div className="pos-card">
                        <h3 className="pos-card-title pos-flex pos-flex--center pos-gap-6"><Bell size={14} style={{ color: '#3B82F6' }} /> Notifications</h3> /* keep-inline */
                    ...
                    {(outCount + critCount) > 0 && <div className="pos-card" style={{ background: 'rgba(239,68,68,0.04)', border: '1px solid rgba(239,68,68,0.2)' }}> /* keep-inline */
                        <div style={{ fontSize: 13, fontWeight: 600, color: '#EF4444', marginBottom: 6 }}>⚠️ Action Required</div> /* keep-inline */
                        <p className="pos-cell-secondary" style={{ margin: 0, lineHeight: 1.5 }}> /* keep-inline */
