...
const OrderProfiles: React.FC = () => {
    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 16 }}> /* keep-inline */ /* keep-inline */
                {filtered.map(profile => (
                    <div key={profile.id} className="pos-card" style={{ cursor: 'pointer', borderLeft: `4px solid ${profile.color}`, opacity: profile.isActive ? 1 : 0.5 }} onClick={() => setEditing({ ...profile })}> /* keep-inline */ /* keep-inline */
                        <div className="pos-flex pos-flex--between" style={{ alignItems: 'flex-start', marginBottom: 12 }}> /* keep-inline */ /* keep-inline */
                            <div className="pos-flex pos-flex--center pos-gap-10">
                                <div style={{ width: 44, height: 44, borderRadius: 10, background: `${profile.color}15`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: profile.color }}>{ICONS[profile.icon] || <ShoppingBag size={20} />}</div> /* keep-inline */ /* keep-inline */
                                <div><h3 style={{ fontSize: 17, fontWeight: 600, margin: 0 }}>{profile.name} {profile.code && <span className="pos-cell-secondary" style={{ fontSize: 10, fontWeight: 400 }}>({profile.code})</span>}</h3> /* keep-inline */ /* keep-inline */
                                    <span className="pos-cell-secondary">{profile.taxProfile}</span></div>
                            </div>
                            <span className={`pos-badge ${profile.isActive ? 'pos-badge--green' : 'pos-badge--red'}`} style={{ fontSize: 9 }}>{profile.isActive ? 'Active' : 'Off'}</span> /* keep-inline */ /* keep-inline */
                        </div>
                        <div className="pos-flex pos-flex--wrap pos-gap-6">
                            ...
                            {profile.takeawayMode !== 'none' && <span className="pos-badge pos-badge--green" style={{ textTransform: 'capitalize' }}>{profile.takeawayMode}</span>} /* keep-inline */ /* keep-inline */
                            {profile.autoGratuity > 0 && <span className="pos-badge pos-badge--amber">Gratuity {profile.autoGratuity}%</span>}
                        </div>
                        <div className="pos-cell-secondary" style={{ marginTop: 8 }}>Print: {profile.printProfile} {profile.serviceCharge !== '— None —' && ` · ${profile.serviceCharge}`}</div> /* keep-inline */ /* keep-inline */
                    </div>
                ))}
            </div>
        </div>
            ...
            {editing && <div className="pos-modal-overlay" onClick={() => setEditing(null)}>
                <div className="pos-card pos-modal" style={{ maxHeight: '85vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}> /* keep-inline */ /* keep-inline */
                    ...
                    <div className="pos-form-grid" style={{ gridTemplateColumns: '2fr 1fr' }}> /* keep-inline */ /* keep-inline */
                        <div><label className="pos-form-label">Name *</label><input className="pos-input" value={editing.name} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, name: e.target.value } : null)} placeholder="e.g. Dine-In" /></div>
                        <div><label className="pos-form-label">Code</label><input className="pos-input" value={editing.code} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, code: e.target.value.toUpperCase() } : null)} placeholder="e.g. DINE" maxLength={8} /></div>
                    </div>
                    <div className="pos-form-group"><label className="pos-form-label pos-mb-4">Icon</label>
                        <div className="pos-flex pos-flex--wrap pos-gap-6">{Object.keys(ICONS).map(key => (
                            <button key={key} onClick={() => setEditing(p => p ? { ...p, icon: key } : null)} className={`pos-btn-outline ${editing.icon === key ? 'pos-badge--blue' : ''}`} style={{ padding: '6px 14px', borderColor: editing.icon === key ? '#3B82F6' : undefined, background: editing.icon === key ? 'rgba(59,130,246,0.1)' : undefined }} title={key}>{ICONS[key]} {key}</button> /* keep-inline */ /* keep-inline */
                        ))}</div></div>
                    <div className="pos-form-grid" style={{ gridTemplateColumns: '1fr auto' }}> /* keep-inline */ /* keep-inline */
                        <div><label className="pos-form-label pos-mb-4">Color</label><div className="pos-flex pos-gap-6">{['#3B82F6', '#10B981', '#F59E0B', '#EF4444', '#8B5CF6', '#EC4899', '#06B6D4', '#F97316', '#6366F1'].map(c => (
                            <div key={c} onClick={() => setEditing(p => p ? { ...p, color: c } : null)} style={{ width: 28, height: 28, borderRadius: 6, background: c, cursor: 'pointer', border: editing.color === c ? '3px solid #fff' : '3px solid transparent' }} />))}</div></div> /* keep-inline */ /* keep-inline */
                        <div><label className="pos-form-label">Sort Order</label><input type="number" min={0} className="pos-input" style={{ width: 80, textAlign: 'center' }} value={editing.sortOrder} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, sortOrder: parseInt(e.target.value) || 0 } : null)} aria-label="Sort order" /></div> /* keep-inline */ /* keep-inline */
                    </div>
                    <div className="pos-form-grid">
                        <div><label className="pos-form-label">Tax Profile</label><select className="pos-select" value={editing.taxProfile} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, taxProfile: e.target.value } : null)} aria-label="Tax profile">{TAX_PROFILES.map(t => <option key={t}>{t}</option>)}</select></div>
                        <div><label className="pos-form-label">Print Profile</label><select className="pos-select" value={editing.printProfile} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, printProfile: e.target.value } : null)} aria-label="Print profile">{PRINT_PROFILES.map(p => <option key={p}>{p}</option>)}</select></div>
                    </div>
                    {/* Takeaway Section */}
                    <div className="pos-card pos-form-group" style={{ background: 'var(--bg-secondary,#09090b)', padding: 14 }}> /* keep-inline */ /* keep-inline */
                        <div className="pos-text-xs pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Takeaway</div> /* keep-inline */ /* keep-inline */
                        <div className="pos-form-grid">
                            <div><label className="pos-form-label">Mode</label><select className="pos-select" value={editing.takeawayMode} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, takeawayMode: e.target.value as OrderProfile['takeawayMode'] } : null)} aria-label="Takeaway mode"><option value="none">None (Dine-in)</option><option value="pickup">Pickup</option><option value="delivery">Delivery</option></select></div>
                            {editing.takeawayMode === 'delivery' && <div><label className="pos-form-label">Delivery Delay</label><select className="pos-select" value={editing.deliveryDelay} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, deliveryDelay: e.target.value as OrderProfile['deliveryDelay'] } : null)} aria-label="Delivery delay"><option value="none">{"No "}delay</option><option value="custom">Custom delay</option><option value="no-time">{"No "}delivery time</option></select></div>}
                        </div>
                    </div>
                    {/* Service Charges Section */}
                    <div className="pos-card pos-form-group" style={{ background: 'var(--bg-secondary,#09090b)', padding: 14 }}> /* keep-inline */ /* keep-inline */
                        <div className="pos-text-xs pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Service Charges & Gratuity</div> /* keep-inline */ /* keep-inline */
                        <div className="pos-form-grid">
                            <div><label className="pos-form-label">Service Charge</label><select className="pos-select" value={editing.serviceCharge} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, serviceCharge: e.target.value } : null)} aria-label="Service charge">{SERVICE_CHARGES.map(s => <option key={s}>{s}</option>)}</select></div>
                            <div><label className="pos-form-label">Automatic Gratuity %</label><input type="number" min={0} max={100} step={0.5} className="pos-input" value={editing.autoGratuity} onChange={e = aria-label="Input field"> setEditing(p => p ? { ...p, autoGratuity: parseFloat(e.target.value) || 0 } : null)} aria-label="Automatic gratuity" /></div>
                        </div>
                        {editing.autoGratuity > 0 && <div className="pos-flex pos-flex--center pos-flex--between" style={{ padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}><span className="pos-cell-value">Apply to quick payment buttons</span><Toggle value={editing.applyToQuickPay} onChange={() => setEditing(p => p ? { ...p, applyToQuickPay: !p.applyToQuickPay } : null)} /></div>} /* keep-inline */ /* keep-inline */
                    </div>
                    {/* Requirements & Behavior */}
                    <div className="pos-card pos-form-group" style={{ background: 'var(--bg-secondary,#09090b)', padding: 14 }}> /* keep-inline */ /* keep-inline */
                        <div className="pos-text-xs pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Requirements & Behavior</div> /* keep-inline */ /* keep-inline */
                        {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
                        {([['continuous', 'Continuous (stays active for subsequent orders)'], ['orderModifier', 'Order modifier (affects tax on posted items)'], ['requiresTable', 'Requires table selection'], ['requiresCustomer', 'Requires customer info'], ['requiresAddress', 'Requires delivery address'], ['autoOpenDrawer', 'Auto-open cash drawer'], ['autoCloseAfterPayment', 'Auto-close after payment'], ['isActive', 'Active']] as const).map(([key, label]) =>
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            <div key={key} className="pos-flex pos-flex--center pos-flex--between" style={{ padding: '10px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}><span className="pos-cell-value">{label}</span><Toggle value={editing[key] as boolean} onChange={() => setEditing(p => p ? { ...p, [key]: !(p as unknown)[key] } : null)} /></div> /* keep-inline */ /* keep-inline */
                        )}
                    </div>
                    <div className="pos-modal-footer">
                        ...
                        <button title="Delete profile" className="pos-btn-outline" style={{ color: '#EF4444', borderColor: 'rgba(239,68,68,0.3)' }} onClick={() => { setProfiles(prev => prev.filter(p => p.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button> /* keep-inline */ /* keep-inline */
