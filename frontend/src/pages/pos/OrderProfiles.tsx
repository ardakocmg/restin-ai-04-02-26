            ...
            {editing && <div className="pos-modal-overlay" onClick={() => setEditing(null)}>
                <div className="pos-card pos-modal max-h-[85vh] overflow-auto" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pos-form-grid grid-cols-[1fr_auto]">
                        ...
                        <div><label className="pos-form-label">Sort Order</label><input aria-label="Input field" type="number" min={0} className="pos-input w-20 text-center" value={editing.sortOrder} onChange={e => setEditing(p => p ? { ...p, sortOrder: parseInt(e.target.value) || 0 } : null)} aria-label="Sort order" /></div>
                    </div>
                    <div className="pos-form-grid">
                        <div><label className="pos-form-label">Tax Profile</label><select aria-label="Select option" className="pos-select" value={editing.taxProfile} onChange={e => setEditing(p => p ? { ...p, taxProfile: e.target.value } : null)} aria-label="Tax profile">{TAX_PROFILES.map(t => <option key={t}>{t}</option>)}</select></div>
                        <div><label className="pos-form-label">Print Profile</label><select aria-label="Select option" className="pos-select" value={editing.printProfile} onChange={e => setEditing(p => p ? { ...p, printProfile: e.target.value } : null)} aria-label="Print profile">{PRINT_PROFILES.map(p => <option key={p}>{p}</option>)}</select></div>
                    </div>
                    {/* Takeaway Section */}
                    <div className="pos-card pos-form-group bg-[var(--bg-secondary,#09090b)] p-3.5">
                        ...
                        <div className="pos-form-grid">
                            <div><label className="pos-form-label">Mode</label><select aria-label="Select option" className="pos-select" value={editing.takeawayMode} onChange={e => setEditing(p => p ? { ...p, takeawayMode: e.target.value as OrderProfile['takeawayMode'] } : null)} aria-label="Takeaway mode"><option value="none">None (Dine-in)</option><option value="pickup">Pickup</option><option value="delivery">Delivery</option></select></div>
                            {editing.takeawayMode === 'delivery' && <div><label className="pos-form-label">Delivery Delay</label><select aria-label="Select option" className="pos-select" value={editing.deliveryDelay} onChange={e => setEditing(p => p ? { ...p, deliveryDelay: e.target.value as OrderProfile['deliveryDelay'] } : null)} aria-label="Delivery delay"><option value="none">{"No "}delay</option><option value="custom">Custom delay</option><option value="no-time">{"No "}delivery time</option></select></div>}
                        </div>
                    </div>
                    ...
                    <div className="pos-card pos-form-group bg-[var(--bg-secondary,#09090b)] p-3.5">
                        ...
                        <div className="pos-form-grid">
                            <div><label className="pos-form-label">Service Charge</label><select aria-label="Select option" className="pos-select" value={editing.serviceCharge} onChange={e => setEditing(p => p ? { ...p, serviceCharge: e.target.value } : null)} aria-label="Service charge">{SERVICE_CHARGES.map(s => <option key={s}>{s}</option>)}</select></div>
                            <div><label className="pos-form-label">Automatic Gratuity %</label><input aria-label="Input field" type="number" min={0} max={100} step={0.5} className="pos-input" value={editing.autoGratuity} onChange={e => setEditing(p => p ? { ...p, autoGratuity: parseFloat(e.target.value) || 0 } : null)} aria-label="Automatic gratuity" /></div>
                        </div>
                    ...
                    <div className="pos-card pos-form-group bg-[var(--bg-secondary,#09090b)] p-3.5">
                        ...
                        {([['continuous', 'Continuous (stays active for subsequent orders)'], ['orderModifier', 'Order modifier (affects tax on posted items)'], ['requiresTable', 'Requires table selection'], ['requiresCustomer', 'Requires customer info'], ['requiresAddress', 'Requires delivery address'], ['autoOpenDrawer', 'Auto-open cash drawer'], ['autoCloseAfterPayment', 'Auto-close after payment'], ['isActive', 'Active']] as const).map(([key, label]) =>
                            // eslint-disable-next-line @typescript-eslint/no-explicit-any
                            <div key={key} className="pos-flex pos-flex--center pos-flex--between py-2.5 border-b border-white/[0.04]"><span className="pos-cell-value">{label}</span><Toggle value={editing[key] as boolean} onChange={() => setEditing(p => p ? { ...p, [key]: !(p as unknown as Record<string, unknown>)[key] } : null)} /></div>
                        )}
                    </div>
