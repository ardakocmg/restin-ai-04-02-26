 ...
 */
import { AlertTriangle,ArrowLeft,Edit3,GripVertical,Plus,Save,Search,Trash2,Wifi,X } from 'lucide-react';
...
const VoidReasons: React.FC = () => {
    ...
    const categoryBadge = (cat: string) => {
        ...
        return <span className={`pos-badge ${cls}`} style={{ textTransform: 'capitalize'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{cat}</span>;
    };
    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div className="pos-card">
                <div className="pos-table-header" style={{ gridTemplateColumns: voidTableCols  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                    <div></div><div>Reason</div><div>Category</div><div>Controls</div><div>Status</div><div>Uses</div><div></div>
                </div>
                {filtered.map(r => (
                    <div key={r.id} className="pos-table-row" style={{ gridTemplateColumns: voidTableCols, opacity: r.isActive ? 1 : 0.5  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => setEditing({ ...r })}>
                        ...
                        <div className="pos-flex pos-gap-4">
                            {r.requiresManager && <span className="pos-badge pos-badge--red" style={{ fontSize: 9  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Mgr</span>}
                            {r.requiresNote && <span className="pos-badge pos-badge--amber" style={{ fontSize: 9  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Note</span>}
            ...
            {editing && <div className="pos-modal-overlay" onClick={() => setEditing(null)}>
                <div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pos-modal-footer">
                        ...
                        <button title="Delete reason" className="pos-btn-outline" style={{ color: '#EF4444'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => { setReasons(p => p.filter(r => r.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button>
