        ...
        <div className="pos-page"><div className="pos-container pos-container--1200">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                <div>
                    ...
                    <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Floor Plans</h1>
                    <p style={{ fontSize: 13, color: 'var(--text-secondary,#a1a1aa)', margin: '4px 0 0'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{floors.length} floors · {totalTables} tables · {totalSeats} seats</p>
                </div>
                ...
                <div style={{ position: 'relative', width: '100%', height: 400, background: 'var(--bg-secondary,#09090b)', backgroundImage: 'radial-gradient(circle, rgba(255,255,255,0.04) 1px, transparent 1px)', backgroundSize: '20px 20px'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                    {floor.tables.map(table => (
                        <div key={table.id} onClick={() => { setSelectedTable(table.id); setEditingTable({ ...table }); }}
                            ...
                            <span style={{ fontSize: 12, fontWeight: 600, color: '#3B82F6'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{table.name}</span>
                            <span style={{ fontSize: 10, color: 'var(--text-secondary)'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>{table.seats} seats</span>
                        </div>
                    ))}
                    {floor.tables.length === 0 && <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: 'var(--text-secondary)'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                        <LayoutGrid size={36} style={{ opacity: 0.3, marginBottom: 8  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ /><p style={{ fontSize: 14  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Click "Add Table" to start designing</p>
                    </div>}
                </div>
            </div>}
            ...
            {editingTable && <div className="pos-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                    <h3 style={{ fontSize: 16, fontWeight: 600, margin: 0  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Table: {editingTable.name}</h3>
                    <div style={{ display: 'flex', gap: 6  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                        <button className="pos-btn pos-btn--primary" style={{ padding: '6px 14px', fontSize: 12  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={saveTable}><Save size={12} /> Save</button>
                        <button className="pos-btn pos-btn--outline" style={{ padding: '6px 14px', fontSize: 12, color: '#EF4444', borderColor: 'rgba(239,68,68,0.3)'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={() => deleteTable(editingTable.id)}><Trash2 size={12} /> Delete</button>
                    </div>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                    <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Name</label>
                        <input className="pos-input" value={editingTable.name} onChange={e => setEditingTable(p => p ? { ...p, name: e.target.value } : null)} /></div>
                    <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Seats</label>
                    ...
                    <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Shape</label>
                        <div style={{ display: 'flex', gap: 6  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                            <button onClick={() => setEditingTable(p => p ? { ...p, shape: 'rect' } : null)} className={`pos-btn pos-btn--outline${editingTable.shape === 'rect' ? ' pos-radio-option--active' : ''}`} style={{ padding: '8px 14px', fontSize: 12  /* keep-inline */ }} /* keep-inline */ /* keep-inline */><Square size={14} /></button>
                            <button onClick={() => setEditingTable(p => p ? { ...p, shape: 'circle' } : null)} className={`pos-btn pos-btn--outline${editingTable.shape === 'circle' ? ' pos-radio-option--active' : ''}`} style={{ padding: '8px 14px', fontSize: 12  /* keep-inline */ }} /* keep-inline */ /* keep-inline */><Circle size={14} /></button>
                        </div></div>
                    <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>Size (W×H)</label>
                        <div style={{ display: 'flex', gap: 4  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                            <input type="number" min={40} className="pos-input" style={{ width: '50%'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ value={editingTable.width} onChange={e => setEditingTable(p => p ? { ...p, width: parseInt(e.target.value) || 40 } : null)} />
                            <input type="number" min={40} className="pos-input" style={{ width: '50%'  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ value={editingTable.height} onChange={e => setEditingTable(p => p ? { ...p, height: parseInt(e.target.value) || 40 } : null)} />
                        </div></div>
                </div>
            </div>}
        </div></div>
