        ...
        <div className="pos-page"><div className="pos-container pos-container--sidebar">
            ...
            <div className="pos-sidebar">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}> /* keep-inline */ /* keep-inline */
                    <button onClick={() => navigate(-1)} className="pos-btn pos-btn--outline" style={{ padding: '6px 10px', fontSize: 12 }}><ArrowLeft size={14} /></button> /* keep-inline */ /* keep-inline */
                    <button className="pos-btn pos-btn--primary" style={{ padding: '6px 12px', fontSize: 12 }} onClick={addGroup}><Plus size={14} /> Add</button> /* keep-inline */ /* keep-inline */
                </div>
                ...
                {groups.map(group => (
                    <button key={group.id} onClick={() => setSelected(group.id)} className={`pos-sidebar-btn${selected === group.id ? ' pos-sidebar-btn--active' : ''}`}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}> /* keep-inline */ /* keep-inline */
                            ...
                            <div style={{ textAlign: 'left' }}><div style={{ fontSize: 13, fontWeight: 500 }}>{group.name}</div> /* keep-inline */ /* keep-inline */
                                <div style={{ fontSize: 10, opacity: 0.6, marginTop: 1 }}>{group.instructions.length} options · {group.type}</div></div> /* keep-inline */ /* keep-inline */
                        </div>
                        <ChevronRight size={14} style={{ opacity: 0.4 }} /> /* keep-inline */ /* keep-inline */
                    </button>
                ))}
            </div>
            ...
            <div className="pos-main">
                {activeGroup ? <>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}> /* keep-inline */ /* keep-inline */
                        <div>
                            <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>{activeGroup.name}</h1> /* keep-inline */ /* keep-inline */
                            <p style={{ fontSize: 12, color: 'var(--text-secondary,#a1a1aa)', margin: '4px 0 0' }}>{activeGroup.type === 'single' ? 'Single' : 'Multi'} select · {activeGroup.instructions.length} options{activeGroup.required ? ' · Required' : ''}</p> /* keep-inline */ /* keep-inline */
                        </div>
                        <div style={{ display: 'flex', gap: 6 }}> /* keep-inline */ /* keep-inline */
                            <button className="pos-btn pos-btn--outline" style={{ padding: '6px 12px', fontSize: 12 }} onClick={() => setEditingGroup({ ...activeGroup })}><Edit3 size={12} /> Edit</button> /* keep-inline */ /* keep-inline */
                            <button className="pos-btn pos-btn--outline" style={{ padding: '6px 12px', fontSize: 12, color: '#EF4444' }} onClick={() => deleteGroup(activeGroup.id)}><Trash2 size={12} /></button> /* keep-inline */ /* keep-inline */
                        </div>
                    </div>

                    {/* Group Settings Card */}
                    <div className="pos-card" style={{ marginBottom: 16 }}> /* keep-inline */ /* keep-inline */
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}> /* keep-inline */ /* keep-inline */
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}> /* keep-inline */ /* keep-inline */
                                <ToggleRight size={14} style={{ color: activeGroup.required ? '#10B981' : 'var(--text-secondary)' }} /> /* keep-inline */ /* keep-inline */
                                <span>{activeGroup.required ? 'Required' : 'Optional'}</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}> /* keep-inline */ /* keep-inline */
                                <Printer size={14} style={{ color: activeGroup.printOnKDS ? '#3B82F6' : 'var(--text-secondary)' }} /> /* keep-inline */ /* keep-inline */
                                <span>KDS: {activeGroup.printOnKDS ? 'Yes' : 'No'}</span>
                            </div>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}> /* keep-inline */ /* keep-inline */
                                <Printer size={14} style={{ color: activeGroup.printOnTicket ? '#3B82F6' : 'var(--text-secondary)' }} /> /* keep-inline */ /* keep-inline */
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}> /* keep-inline */ /* keep-inline */
                        <h3 className="pos-section-title" style={{ margin: 0 }}>Options</h3> /* keep-inline */ /* keep-inline */
                        <button className="pos-btn pos-btn--primary" style={{ padding: '6px 12px', fontSize: 12 }} onClick={addInstruction}><Plus size={12} /> Add Option</button> /* keep-inline */ /* keep-inline */
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}> /* keep-inline */ /* keep-inline */
                        {activeGroup.instructions.map(inst => (
                            <div key={inst.id} className="pos-card" style={{ cursor: 'pointer', padding: '12px 14px' }} onClick={() => setEditingInstruction({ ...inst })}> /* keep-inline */ /* keep-inline */
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}> /* keep-inline */ /* keep-inline */
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}> /* keep-inline */ /* keep-inline */
                                        <GripVertical size={14} style={{ color: 'var(--text-secondary)', opacity: 0.4 }} /> /* keep-inline */ /* keep-inline */
                                        <div>
                                            <div style={{ fontSize: 13, fontWeight: 500 }}>{inst.name}</div> /* keep-inline */ /* keep-inline */
                                            {inst.isDefault && <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 3, background: 'rgba(59,130,246,0.15)', color: '#3B82F6' }}>Default</span>} /* keep-inline */ /* keep-inline */
                                        </div>
                                    </div>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}> /* keep-inline */ /* keep-inline */
                                        {inst.priceModifier !== 0 && <span style={{ fontSize: 12, fontWeight: 600, color: inst.priceModifier > 0 ? '#10B981' : '#EF4444' }}>{inst.priceModifier > 0 ? '+' : ''}€{inst.priceModifier.toFixed(2)}</span>} /* keep-inline */ /* keep-inline */
                                        <button className="pos-btn pos-btn--outline" style={{ padding: '4px 8px' }} onClick={e => { e.stopPropagation(); deleteInstruction(inst.id); }}><Trash2 size={12} /></button> /* keep-inline */ /* keep-inline */
                                    </div>
                                </div>
                            </div>
                        ))}
                        {activeGroup.instructions.length === 0 && <div style={{ textAlign: 'center', padding: 30, color: 'var(--text-secondary)' }}><ListChecks size={32} style={{ opacity: 0.3, marginBottom: 8 }} /><p>{"No "}options yet</p></div>} /* keep-inline */ /* keep-inline */
                    </div>
                </> : <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', color: 'var(--text-secondary)' }}> /* keep-inline */ /* keep-inline */
                    <LayoutGrid size={48} style={{ opacity: 0.2, marginBottom: 12 }} /><p>Select a group or create one</p> /* keep-inline */ /* keep-inline */
                </div>}
            </div>
        </div>

            {/* Edit Group Modal */}
            {editingGroup && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setEditingGroup(null)}> /* keep-inline */ /* keep-inline */
                <div className="pos-card" style={{ width: 440 }} onClick={e => e.stopPropagation()}> /* keep-inline */ /* keep-inline */
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}> /* keep-inline */ /* keep-inline */
                        <h3 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>Edit Group</h3> /* keep-inline */ /* keep-inline */
                        <button style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setEditingGroup(null)}><X size={20} /></button> /* keep-inline */ /* keep-inline */
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}> /* keep-inline */ /* keep-inline */
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Type</label> /* keep-inline */ /* keep-inline */
                            <div style={{ display: 'flex', gap: 6 }}>{(['single', 'multi'] as const).map(t => ( /* keep-inline */ /* keep-inline */
                                <button key={t} onClick={() => setEditingGroup(p => p ? { ...p, type: t } : null)} className={`pos-radio-option${editingGroup.type === t ? ' pos-radio-option--active' : ''}`} style={{ flex: 1, textAlign: 'center' }}>{t === 'single' ? '○ Single' : '☑ Multi'}</button> /* keep-inline */ /* keep-inline */
                            ))}</div></div>
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Max Selections</label> /* keep-inline */ /* keep-inline */
                    ...
                    <div style={{ display: 'flex', gap: 14, marginBottom: 16 }}> /* keep-inline */ /* keep-inline */
                        {['required', 'printOnKDS', 'printOnTicket'].map(k => (
                            <label key={k} style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}> /* keep-inline */ /* keep-inline */
                    ...
                    <button className="pos-btn pos-btn--primary" style={{ width: '100%', justifyContent: 'center' }} onClick={saveGroup}><Save size={14} /> Save</button> /* keep-inline */ /* keep-inline */
                </div>
            </div>}

            {/* Edit Instruction Modal */}
            {editingInstruction && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1001 }} onClick={() => setEditingInstruction(null)}> /* keep-inline */ /* keep-inline */
                <div className="pos-card" style={{ width: 400 }} onClick={e => e.stopPropagation()}> /* keep-inline */ /* keep-inline */
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}> /* keep-inline */ /* keep-inline */
                        <h3 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>Edit Option</h3> /* keep-inline */ /* keep-inline */
                        <button style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setEditingInstruction(null)}><X size={20} /></button> /* keep-inline */ /* keep-inline */
                    </div>
                    <div style={{ marginBottom: 14 }}><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Name</label> /* keep-inline */ /* keep-inline */
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}> /* keep-inline */ /* keep-inline */
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Price Modifier (€)</label> /* keep-inline */ /* keep-inline */
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-end' }}> /* keep-inline */ /* keep-inline */
                            <label style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}> /* keep-inline */ /* keep-inline */
                    ...
                    <button className="pos-btn pos-btn--primary" style={{ width: '100%', justifyContent: 'center' }} onClick={saveInstruction}><Save size={14} /> Save</button> /* keep-inline */ /* keep-inline */
                </div>
            </div>}
        </div>
    );
