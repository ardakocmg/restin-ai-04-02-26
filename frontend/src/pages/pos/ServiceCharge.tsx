    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))', gap: 16 }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                {rules.map(rule => (
                    <div key={rule.id} className="pos-card" style={{ cursor: 'pointer', opacity: rule.isActive ? 1 : 0.5 }} onClick={() => setEditing({ ...rule })}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        <div className="pos-flex pos-flex--between" style={{ alignItems: 'flex-start', marginBottom: 10 }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
                            <div className="pos-flex pos-flex--center pos-gap-10">
                                ...
                                <div><h3 className="pos-modal-title" style={{ fontSize: 16 }}>{rule.name}</h3> /* keep-inline */ /* keep-inline */ /* keep-inline */
                            ...
                            <span className={`pos-badge ${rule.isActive ? 'pos-badge--green' : 'pos-badge--red'}`} style={{ fontSize: 9 }}>{rule.isActive ? 'Active' : 'Off'}</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        </div>
                        <p className="pos-cell-secondary pos-mb-8" style={{ lineHeight: 1.5 }}>{rule.description}</p> /* keep-inline */ /* keep-inline */ /* keep-inline */
                        <div className="pos-flex pos-gap-6 pos-flex--wrap">
                            {rule.autoApply && <span className="pos-badge pos-badge--blue" style={{ fontSize: 9 }}>Auto-Apply</span>} /* keep-inline */ /* keep-inline */ /* keep-inline */
                            {rule.taxable && <span className="pos-badge pos-badge--amber" style={{ fontSize: 9 }}>Taxable</span>} /* keep-inline */ /* keep-inline */ /* keep-inline */
                            {rule.minGuests > 0 && <span className="pos-badge pos-badge--purple" style={{ fontSize: 9 }}>Min {rule.minGuests} guests</span>} /* keep-inline */ /* keep-inline */ /* keep-inline */
                        </div>
                    </div>
                ))}
            </div>
            ...
            {editing && <div className="pos-modal-overlay" onClick={() => setEditing(null)}>
                <div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pos-modal-footer">
                        ...
                        <button title="Delete rule" className="pos-btn-outline" style={{ color: '#EF4444' }} onClick={() => { setRules(p => p.filter(r => r.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button> /* keep-inline */ /* keep-inline */ /* keep-inline */
