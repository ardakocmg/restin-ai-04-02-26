            ...
            {mode === 'receipt' && (
                <div className="kds1-receipt-grid">
                    {activeOrders.map(order => {
                        ...
                        return (
                            <div
                                ...
                                style={{ borderLeft: `4px solid ${timerColor}` }} /* keep-inline */ /* keep-inline */
                            >
                                <div className="kds1-receipt-tile-header">
                                    ...
                                    <span className="kds1-receipt-timer" style={{ color: timerColor, background: `${timerColor}15` }}> /* keep-inline */ /* keep-inline */
                                        <Clock size={12} style={{ marginRight: 4, display: 'inline' }} /> /* keep-inline */ /* keep-inline */
                                        {getTimerText(mins)}
                                    </span>
                                </div>
                                ...
                                <div className="kds1-receipt-tile-footer">
                                    ...
                                    {order.courses.length > 1 && (
                                        <div className="course-info">
                                            {order.courses.map(c => (
                                                <span key={c} style={{ fontSize: 10, padding: '2px 6px', background: 'rgba(255,255,255,0.06)', borderRadius: 3 }}> /* keep-inline */ /* keep-inline */
                                                    C{c}
                                                </span>
                                            ))}
                                        </div>
                                    )}
                                    {order.customerName && <span style={{ fontSize: 11 }}>👤 {order.customerName}</span>} /* keep-inline */ /* keep-inline */
                                </div>
                            </div>
                        );
                    })}
                    ...
                    {activeOrders.length === 0 && (
                        <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: 60, color: 'var(--kds1-text-secondary)' }}> /* keep-inline */ /* keep-inline */
                            <div style={{ fontSize: 48, opacity: 0.3, marginBottom: 12 }}>🍳</div> /* keep-inline */ /* keep-inline */
                            <div style={{ fontSize: 16, fontWeight: 500 }}>{"No "}active orders</div> /* keep-inline */ /* keep-inline */
                        </div>
                    )}
                </div>
            )}
            ...
            {mode === 'production' && (
                <div className="kds1-production-grid">
                            ...
                            {column.map(item => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="kds1-production-total-qty" style={{ color: timerColor }}> /* keep-inline */ /* keep-inline */
                                            {item.totalQty}
                                        </div>
                    ...
                    {productionItems.length === 0 && (
                        <div style={{ gridColumn: '1 / -1', textAlign: 'center', padding: 60, color: 'var(--kds1-text-secondary)' }}> /* keep-inline */ /* keep-inline */
                            <div style={{ fontSize: 48, opacity: 0.3, marginBottom: 12 }}>🍳</div> /* keep-inline */ /* keep-inline */
                            <div style={{ fontSize: 16, fontWeight: 500 }}>{"No "}items waiting</div> /* keep-inline */ /* keep-inline */
                        </div>
                    )}
                </div>
            )}
            ...
            {selectedProductionItem && (
                <div className="kds1-tap-overlay" onClick={() => setSelectedProductionItem(null)}>
                    <div className="kds1-tap-panel" onClick={e => e.stopPropagation()}>
                        ...
                        <div style={{ textAlign: 'center', fontSize: 36, fontWeight: 800, margin: '16px 0', color: 'var(--kds1-time-green)' }}> /* keep-inline */ /* keep-inline */
                ...
                <div className="kds1-report-modal" onClick={() => setShowReport(false)}>
                    <div className="kds1-report-panel" onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}> /* keep-inline */ /* keep-inline */
                            <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>KDS Statistics Report</h2> /* keep-inline */ /* keep-inline */
