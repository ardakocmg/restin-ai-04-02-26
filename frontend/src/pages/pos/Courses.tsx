    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div className="pos-card">
                <div className="pos-table-header" style={{ gridTemplateColumns: courseTableCols }}>
                ...
                {courses.sort((a, b) => a.sortOrder - b.sortOrder).map(course => (
                    <div key={course.id} className="pos-table-row" style={{ gridTemplateColumns: courseTableCols, opacity: course.isActive ? 1 : 0.5 }} onClick={() => setEditing({ ...course })}>
                        <GripVertical size={14} className="pos-icon-grab" />
                        <div className="pos-stat-icon" style={{ width: 28, height: 28, borderRadius: 6, background: course.color, fontSize: 12, fontWeight: 700, color: '#fff' }}>{course.sortOrder}</div>
            ...
            <div className="pos-card" style={{ background: 'rgba(59,130,246,0.04)', borderColor: 'rgba(59,130,246,0.2)' }}>
                <div className="pos-text-sm pos-text-bold pos-mb-8" style={{ color: '#3B82F6' }}>💡 Course Flow</div>
            ...
            {editing && <div className="pos-modal-overlay" onClick={() => setEditing(null)}>
                <div className="pos-card pos-modal pos-modal--sm" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="pos-modal-footer">
                        ...
                        <button title="Delete course" className="pos-btn-outline" style={{ color: '#EF4444' }} onClick={() => { setCourses(p => p.filter(c => c.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button>
