    ...
    return (
        <div className="pos-page"><div className="pos-container">
            ...
            <div className="pos-card pos-card--compact">
                <div className="pos-text-sm pos-text-bold pos-text-secondary pos-mb-12" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Connected Printers</div> /* keep-inline */
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 10 }}> /* keep-inline */
                    {PRINTERS.map(p => (
                        <div key={p.id} className="pos-flex pos-flex--center pos-gap-10" style={{ padding: '10px 14px', background: 'var(--bg-secondary,#09090b)', borderRadius: 8, border: '1px solid rgba(255,255,255,0.04)' }}> /* keep-inline */
                            <div style={{ width: 8, height: 8, borderRadius: '50%', background: p.isOnline ? '#10B981' : '#EF4444', flexShrink: 0 }} /> /* keep-inline */
                            <div style={{ flex: 1, minWidth: 0 }}> /* keep-inline */
                                <div className="pos-text-xs pos-text-bold" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.name}</div> /* keep-inline */
                                <div className="pos-cell-secondary" style={{ fontFamily: 'monospace' }}>{p.ip}</div> /* keep-inline */
                            </div>
                            <Printer size={14} className="pos-icon-secondary" style={{ flexShrink: 0 }} /> /* keep-inline */
                        </div>
                    ))}
                </div>
            </div>
            ...
            {/* Profile Cards */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(340px,1fr))', gap: 16 }}> /* keep-inline */
                {filtered.map(profile => (
                    <div key={profile.id} className="pos-card" style={{ cursor: 'pointer' }} onClick={() => setEditing({ ...profile })}> /* keep-inline */
                        <div className="pos-flex pos-flex--between" style={{ alignItems: 'flex-start', marginBottom: 12 }}> /* keep-inline */
                            <div><h3 style={{ fontSize: 16, fontWeight: 600, margin: 0 }}>{profile.name}</h3> /* keep-inline */
                                {profile.description && <p className="pos-subtitle">{profile.description}</p>}</div>
                            <span className={`pos-badge ${profile.isActive ? 'pos-badge--green' : 'pos-badge--red'}`} style={{ fontSize: 10, fontWeight: 600 }}>{profile.isActive ? 'Active' : 'Inactive'}</span> /* keep-inline */
                        </div>
                        ...
                        <div className="pos-cell-secondary pos-flex pos-gap-14 pos-flex--wrap">
                            <span><Printer size={11} style={{ marginRight: 4, verticalAlign: 'middle' }} />{profile.printers.length} printer{profile.printers.length !== 1 ? 's' : ''}</span> /* keep-inline */
                        ...
                        {profile.productionCenters.length > 0 && <div className="pos-flex pos-flex--wrap pos-gap-4" style={{ marginTop: 8 }}> /* keep-inline */
                ...
                <div className="pos-card pos-modal" style={{ maxHeight: '85vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}> /* keep-inline */
                    ...
                    <div className="pos-form-group"><label className="pos-form-label pos-mb-4">Assign Printers</label>
                        <div className="pos-flex pos-flex--col pos-gap-6">
                            {PRINTERS.map(p => <label key={p.id} className="pos-flex pos-flex--center pos-gap-10" style={{ padding: '8px 12px', background: 'var(--bg-secondary,#09090b)', borderRadius: 8, cursor: 'pointer', border: editing.printers.includes(p.id) ? '1px solid #3B82F6' : '1px solid rgba(255,255,255,0.04)' }}> /* keep-inline */
                                ...
                                <div style={{ width: 8, height: 8, borderRadius: '50%', background: p.isOnline ? '#10B981' : '#EF4444' }} /> /* keep-inline */
                                <span className="pos-text-sm" style={{ flex: 1 }}>{p.name}</span> /* keep-inline */
                                <span className="pos-cell-secondary" style={{ fontFamily: 'monospace' }}>{p.ip}</span> /* keep-inline */
                            </label>)}
                        </div>
                    </div>
                    <div className="pos-form-group"><label className="pos-form-label pos-mb-4">Production Centers</label>
                        <div className="pos-flex pos-flex--wrap pos-gap-6">
                            {PROD_CENTERS.map(c => <button key={c} onClick={() => setEditing(prev => { if (!prev) return null; const has = prev.productionCenters.includes(c); return { ...prev, productionCenters: has ? prev.productionCenters.filter(x => x !== c) : [...prev.productionCenters, c] }; })}
                                className={`pos-btn-outline ${editing.productionCenters.includes(c) ? 'pos-badge--purple' : ''}`} style={{ padding: '6px 14px', fontSize: 12, borderColor: editing.productionCenters.includes(c) ? '#8B5CF6' : undefined }}>{c}</button>)} /* keep-inline */
                        </div>
                    </div>
                    <div className="pos-form-group"><label className="pos-form-label">Fallback Printer</label>
                        ...
                        <div className="pos-cell-secondary" style={{ marginTop: 4 }}>Used when the primary printer is offline</div> /* keep-inline */
                    </div>
                    <div className="pos-card pos-form-group" style={{ background: 'var(--bg-secondary,#09090b)', padding: 14 }}> /* keep-inline */
                        <div className="pos-text-xs pos-text-bold pos-text-secondary pos-mb-8" style={{ textTransform: 'uppercase', letterSpacing: 0.5 }}>Print Options</div> /* keep-inline */
                        {([['printReceipts', 'Customer Receipts'], ['printOrderTickets', 'Order Tickets'], ['printKitchenTickets', 'Kitchen Tickets'], ['cutAfterPrint', 'Auto-cut'], ['buzzAfterPrint', 'Buzz after print'], ['isActive', 'Active']] as const).map(([key, label]) =>
                            <div key={key} className="pos-flex pos-flex--center pos-flex--between" style={{ padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}><span className="pos-cell-value">{label}</span> /* keep-inline */
                                <Toggle value={editing[key] as boolean} onChange={() => setEditing(p => p ? { ...p, [key]: !(p[key] as boolean) } : null)} /></div>
                        )}
                    </div>
                    <div className="pos-form-grid--3 pos-mb-16" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14 }}> /* keep-inline */
                    ...
                    <div className="pos-modal-footer">
                        ...
                        <button title="Delete profile" className="pos-btn-outline" style={{ color: '#EF4444', borderColor: 'rgba(239,68,68,0.3)' }} onClick={() => { setProfiles(p => p.filter(pp => pp.id !== editing.id)); setEditing(null); toast.success('Deleted'); }}><Trash2 size={14} /></button> /* keep-inline */
