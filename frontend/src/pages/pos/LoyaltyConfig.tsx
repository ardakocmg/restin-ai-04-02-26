...
const LoyaltyConfig: React.FC = () => {
    ...
    const Toggle: React.FC<{ value: boolean; onChange: () => void }> = ({ value, onChange }) => (
        <div onClick={onChange} style={{ width: 42, height: 24, borderRadius: 12, background: value ? '#3B82F6' : 'var(--border-primary,#27272a)', cursor: 'pointer', position: 'relative', transition: 'background 0.2s' }}> /* keep-inline */
        ...
        <div className="pos-page"><div className="pos-container pos-container--900">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}> /* keep-inline */
                <div>
                    ...
                    <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0 }}>Loyalty Program</h1> /* keep-inline */
                    <p style={{ fontSize: 13, color: 'var(--text-secondary,#a1a1aa)', margin: '4px 0 0' }}>{config.enabled ? '✅ Active' : '⏸ Paused'} · {config.tiers.length} tiers · {rewards.filter(r => r.isActive).length} rewards</p> /* keep-inline */
                </div>
                <button className="pos-btn pos-btn--primary" onClick={() => toast.success('Configuration saved')}><Save size={14} /> Save All</button>
            </div>

            {/* Tabs */}
            <div className="pos-tab-group" style={{ marginBottom: 20 }}> /* keep-inline */
            ...
            {/* Settings Tab */}
            {tab === 'settings' && <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}> /* keep-inline */
                <div className="pos-card">
                    ...
                    <SettingRow label="Points Per Euro" desc="Points earned per €1 spent">
                        <input type="number" min={1} className="pos-input" style={{ width: 80, textAlign: 'center' }} value={config.pointsPerEuro} onChange={e => upd('pointsPerEuro', parseInt(e.target.value) || 1)} /></SettingRow> /* keep-inline */
                ...
                <div className="pos-card">
                    ...
                    <SettingRow label="Min Points to Redeem" desc="Minimum points before redemption">
                        <input type="number" min={1} className="pos-input" style={{ width: 80, textAlign: 'center' }} value={config.redeemThreshold} onChange={e => upd('redeemThreshold', parseInt(e.target.value) || 1)} /></SettingRow> /* keep-inline */
                    <SettingRow label="Redemption Value (€)" desc="Euro value per redemption unit">
                        <input type="number" min={1} className="pos-input" style={{ width: 80, textAlign: 'center' }} value={config.redeemValue} onChange={e => upd('redeemValue', parseInt(e.target.value) || 1)} /></SettingRow> /* keep-inline */
                    <SettingRow label="Points Expiry (Days)" desc="Days before points expire">
                        <input type="number" min={0} className="pos-input" style={{ width: 80, textAlign: 'center' }} value={config.expiryDays} onChange={e => upd('expiryDays', parseInt(e.target.value) || 0)} /></SettingRow> /* keep-inline */
                    <SettingRow label="Referral Bonus" desc="Points for successful referral">
                        <input type="number" min={0} className="pos-input" style={{ width: 80, textAlign: 'center' }} value={config.referralBonus} onChange={e => upd('referralBonus', parseInt(e.target.value) || 0)} /></SettingRow> /* keep-inline */
                </div>
                <div className="pos-card">
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}> /* keep-inline */
                        {['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'].map(day => (
                            <button key={day} onClick={() => upd('doublePointsDays', config.doublePointsDays.includes(day) ? config.doublePointsDays.filter(d => d !== day) : [...config.doublePointsDays, day])}
                                className={`pos-radio-option${config.doublePointsDays.includes(day) ? ' pos-radio-option--active' : ''}`} style={{ fontSize: 12, padding: '6px 12px' }}> /* keep-inline */
                                {day.slice(0, 3)}
                            </button>
                        ))}
                    </div>
            ...
            {/* Tiers Tab */}
            {tab === 'tiers' && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(240px,1fr))', gap: 12 }}> /* keep-inline */
                {config.tiers.map(tier => (
                    <div key={tier.id} className="pos-card" style={{ padding: 20, borderTop: `3px solid ${tier.color}` }}> /* keep-inline */
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}> /* keep-inline */
                            <span style={{ fontSize: 24 }}>{tier.icon}</span> /* keep-inline */
                            <div><div style={{ fontSize: 16, fontWeight: 700 }}>{tier.name}</div> /* keep-inline */
                                <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{tier.minPoints.toLocaleString()}+ pts</div></div> /* keep-inline */
                        </div>
                        <div style={{ display: 'inline-block', padding: '3px 8px', borderRadius: 4, background: `${tier.color}20`, color: tier.color, fontSize: 11, fontWeight: 600, marginBottom: 10 }}>{tier.multiplier}x Points</div> /* keep-inline */
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}> /* keep-inline */
                            {tier.perks.map((p, j) => <div key={j} style={{ fontSize: 12, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6 }}><Star size={10} style={{ color: tier.color }} />{p}</div>)} /* keep-inline */
                        </div>
                    </div>
                ))}
            </div>}
            ...
            {/* Rewards Tab */}
            {tab === 'rewards' && <>
                <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}> /* keep-inline */
                    <button className="pos-btn pos-btn--primary" style={{ padding: '6px 12px', fontSize: 12 }} onClick={() => setEditReward({ id: crypto.randomUUID(), name: '', pointsCost: 100, type: 'discount', value: '', isActive: true })}><Plus size={14} /> Add Reward</button> /* keep-inline */
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 12 }}> /* keep-inline */
                    {rewards.map(reward => (
                        <div key={reward.id} className="pos-card" style={{ cursor: 'pointer', opacity: reward.isActive ? 1 : 0.5, padding: 16 }} onClick={() => setEditReward({ ...reward })}> /* keep-inline */
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}> /* keep-inline */
                                <div style={{ width: 36, height: 36, borderRadius: 8, background: reward.type === 'discount' ? 'rgba(59,130,246,0.15)' : reward.type === 'freeItem' ? 'rgba(16,185,129,0.15)' : 'rgba(139,92,246,0.15)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: reward.type === 'discount' ? '#3B82F6' : reward.type === 'freeItem' ? '#10B981' : '#8B5CF6' }}> /* keep-inline */
                                    {reward.type === 'discount' ? <span style={{ fontSize: 14 }}>€</span> : reward.type === 'freeItem' ? <Gift size={16} /> : <Crown size={16} />} /* keep-inline */
                                </div>
                                <div><div style={{ fontSize: 14, fontWeight: 600 }}>{reward.name}</div> /* keep-inline */
                                    <div style={{ fontSize: 11, color: 'var(--text-secondary)' }}>{reward.value}</div></div> /* keep-inline */
                            </div>
                            <div style={{ fontSize: 16, fontWeight: 700, color: '#F59E0B' }}>{reward.pointsCost} <span style={{ fontSize: 11, fontWeight: 400, color: 'var(--text-secondary)' }}>points</span></div> /* keep-inline */
                        </div>
                    ))}
                </div>
            </>}
            ...
            {/* Edit Reward Modal */}
            {editReward && <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setEditReward(null)}> /* keep-inline */
                <div className="pos-card" style={{ width: 480 }} onClick={e => e.stopPropagation()}> /* keep-inline */
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}> /* keep-inline */
                        <h3 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>{rewards.find(r => r.id === editReward.id) ? 'Edit' : 'New'} Reward</h3> /* keep-inline */
                        <button style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }} onClick={() => setEditReward(null)}><X size={20} /></button> /* keep-inline */
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}> /* keep-inline */
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Type</label> /* keep-inline */
                            <div style={{ display: 'flex', gap: 4 }}>{(['discount', 'freeItem', 'upgrade'] as const).map(t => ( /* keep-inline */
                                <button key={t} onClick={() => setEditReward(p => p ? { ...p, type: t } : null)} className={`pos-radio-option${editReward.type === t ? ' pos-radio-option--active' : ''}`} style={{ flex: 1, textAlign: 'center', fontSize: 11, padding: '6px 4px' }}> /* keep-inline */
                                    {t === 'discount' ? '€ Off' : t === 'freeItem' ? 'Free' : 'Upgrade'}
                                </button>
                            ))}</div></div>
                        <div><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Points Cost</label> /* keep-inline */
                    ...
                    <div style={{ marginBottom: 14 }}><label style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 4, display: 'block' }}>Value / Description</label> /* keep-inline */
                        <input className="pos-input" value={editReward.value} onChange={e => setEditReward(p => p ? { ...p, value: e.target.value } : null)} /></div>
                    <div style={{ marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}> /* keep-inline */
                        <Toggle value={editReward.isActive} onChange={() => setEditReward(p => p ? { ...p, isActive: !p.isActive } : null)} />
                        <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>Active</span> /* keep-inline */
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}> /* keep-inline */
                        <button className="pos-btn pos-btn--primary" style={{ flex: 1, justifyContent: 'center' }} onClick={() => { /* keep-inline */
                        ...
                        {rewards.find(r => r.id === editReward.id) && <button className="pos-btn pos-btn--outline" style={{ color: '#EF4444' }} onClick={() => { setRewards(p => p.filter(r => r.id !== editReward.id)); setEditReward(null); toast.success('Reward deleted'); }}><Trash2 size={14} /></button>} /* keep-inline */
