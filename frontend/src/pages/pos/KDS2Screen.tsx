...
const KDS2Screen: React.FC = () => {
    ...
    const getItemActionButton = (item: KDSItem, ticketId: string) => {
        switch (item.status) {
            ...
            default:
                return <Check size={14} style={{ color: 'var(--kds2-status-ready)', opacity: 0.5 }} />; /* keep-inline */
        }
    };
                    ...
                    <div className="kds2-filter-row">
                        ...
                        {settings.statusNew && (
                            <button className={`kds2-filter-btn ${statusFilter === 'new' ? 'active' : ''}`} onClick={() => { setStatusFilter('new'); setShowCompleted(false); }}>
                                <span className="kds2-filter-dot" style={{ background: STATUS_CONFIG.new.color }} /> New <span className="kds2-filter-count">{statusCounts.new}</span> /* keep-inline */
                            </button>
                        )}
                        <button className={`kds2-filter-btn ${statusFilter === 'preparing' ? 'active' : ''}`} onClick={() => { setStatusFilter('preparing'); setShowCompleted(false); }}>
                            <span className="kds2-filter-dot" style={{ background: STATUS_CONFIG.preparing.color }} /> Preparing <span className="kds2-filter-count">{statusCounts.preparing}</span> /* keep-inline */
                        </button>
                        {settings.statusReady && (
                            <button className={`kds2-filter-btn ${statusFilter === 'ready' ? 'active' : ''}`} onClick={() => { setStatusFilter('ready'); setShowCompleted(false); }}>
                                <span className="kds2-filter-dot" style={{ background: STATUS_CONFIG.ready.color }} /> Ready <span className="kds2-filter-count">{statusCounts.ready}</span> /* keep-inline */
                            </button>
                        )}
                        <button className={`kds2-filter-btn ${statusFilter === 'hold' ? 'active' : ''}`} onClick={() => { setStatusFilter('hold'); setShowCompleted(false); }}>
                            <span className="kds2-filter-dot" style={{ background: STATUS_CONFIG.hold.color }} /> On Hold <span className="kds2-filter-count">{statusCounts.hold}</span> /* keep-inline */
                        </button>
                        <button className={`kds2-filter-btn ${showCompleted ? 'active' : ''}`} onClick={() => setShowCompleted(!showCompleted)} style={{ marginLeft: 'auto' }}> /* keep-inline */
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {ticket.courses.length > 1 && (
                                        <div className="kds2-ticket-courses-bar">
                                            {ticket.courses.map((c, idx) => (
                                                <div key={idx} className={`course-segment ${idx === 0 ? 'completed' : ''}`} style={{ background: ['#3B82F6', '#22C55E', '#F97316', '#8B5CF6'][idx % 4] }} /> /* keep-inline */
                                            ))}
                                        </div>
                                    )}
                                    ...
                                    <div className="kds2-ticket-footer" onDoubleClick={(e) => { e.stopPropagation(); bumpTicket(ticket.id); }}>
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}> /* keep-inline */
                                            <span style={{ fontSize: 10, padding: '2px 6px', borderRadius: 4, background: STATUS_CONFIG[ticket.status].color, color: '#fff' }}> /* keep-inline */
                        ...
                        return (
                            <div key={status} className="kds2-items-list-group">
                                <div className="kds2-items-list-group-header">
                                    <span className="kds2-items-list-group-title" style={{ color: STATUS_CONFIG[status as OrderStatus]?.color }}> /* keep-inline */
                                        ...
                                        <div className="kds2-items-list-item-info">
                                            ...
                                            <div>
                                                ...
                                                {item.instructions.map((inst, i) => (
                                                    <div key={i} className={`kds2-instruction ${inst.type}`} style={{ fontSize: 12 }}> /* keep-inline */
                ...
                <div className="kds2-status-menu" style={{ left: statusMenu.x, top: statusMenu.y }} onClick={e => e.stopPropagation()}>
                    {(['new', 'preparing', 'ready', 'hold', 'canceled'] as OrderStatus[]).map(s => (
                        <button key={s} className="kds2-status-menu-item" onClick={() => setTicketStatus(statusMenu.ticketId, s)}>
                            <span className="status-dot" style={{ background: STATUS_CONFIG[s].color }} /> /* keep-inline */
                ...
                <div className="kds2-stats-modal" onClick={() => setShowStats(false)}>
                    <div className="kds2-stats-panel" onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}> /* keep-inline */
                            <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>KDS Statistics</h2> /* keep-inline */
