...
const KDS2Screen: React.FC = () => {
    ...
    return (
        <div className={`kds2-container ${settings.colorTheme === 'dark' ? 'kds2-dark' : ''}`}>
                    ...
                    <div className="kds2-filter-row">
                        ...
                        {settings.statusNew && (
                            <button className={`kds2-filter-btn ${statusFilter === 'new' ? 'active' : ''}`} onClick={() => { setStatusFilter('new'); setShowCompleted(false); }}>
                                <span className="kds2-filter-dot" style={{ background: STATUS_CONFIG.new.color  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ /> New <span className="kds2-filter-count">{statusCounts.new}</span>
                            </button>
                        )}
                        <button className={`kds2-filter-btn ${statusFilter === 'preparing' ? 'active' : ''}`} onClick={() => { setStatusFilter('preparing'); setShowCompleted(false); }}>
                            <span className="kds2-filter-dot" style={{ background: STATUS_CONFIG.preparing.color  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ /> Preparing <span className="kds2-filter-count">{statusCounts.preparing}</span>
                        </button>
                        {settings.statusReady && (
                            <button className={`kds2-filter-btn ${statusFilter === 'ready' ? 'active' : ''}`} onClick={() => { setStatusFilter('ready'); setShowCompleted(false); }}>
                                <span className="kds2-filter-dot" style={{ background: STATUS_CONFIG.ready.color  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ /> Ready <span className="kds2-filter-count">{statusCounts.ready}</span>
                            </button>
                        )}
                        <button className={`kds2-filter-btn ${statusFilter === 'hold' ? 'active' : ''}`} onClick={() => { setStatusFilter('hold'); setShowCompleted(false); }}>
                            <span className="kds2-filter-dot" style={{ background: STATUS_CONFIG.hold.color  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ /> On Hold <span className="kds2-filter-count">{statusCounts.hold}</span>
                        </button>
            ...
            {/* Status Context Menu */}
            {statusMenu && (
                <div className="kds2-status-menu" style={{ left: statusMenu.x, top: statusMenu.y  /* keep-inline */ }} /* keep-inline */ /* keep-inline */ onClick={e => e.stopPropagation()}>
                ...
                <div className="kds2-stats-modal" onClick={() => setShowStats(false)}>
                    <div className="kds2-stats-panel" onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>
                            <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0  /* keep-inline */ }} /* keep-inline */ /* keep-inline */>KDS Statistics</h2>
