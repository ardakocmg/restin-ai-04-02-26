    ...
    return (
        <div className="space-y-6">
            <Card className="bg-zinc-950 border-zinc-800">
                <CardHeader>
                    <CardTitle className="text-zinc-100 flex items-center gap-2"><Plus className="h-5 w-5 text-emerald-400" />Create Keypad PIN</CardTitle>
                    <CardDescription className="text-zinc-400">Create a time-limited PIN for Nuki Keypad 2. The PIN is dispatched directly to the device.</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="grid grid-cols-6 gap-3 items-end">
                        <div className="col-span-2">
                            <label className="text-xs text-zinc-400 mb-1 block">Door</label>
                            <select title="Select door for PIN" className="w-full bg-zinc-900 border border-zinc-700 text-zinc-200 rounded px-3 py-2 text-sm" value={newPin.door_id} onChange={(e) => setNewPin({ ...newPin, door_id: e.target.value })}>
                        ...
                        <div>
                            ...
                            <Input placeholder="Morning Shift" value={newPin.name} onChange={(e) => setNewPin({ ...newPin, name: e.target.value })} className="bg-zinc-900 border-zinc-700 text-zinc-200" />
                        </div>
                        <div>
                            <label className="text-xs text-zinc-400 mb-1 block">PIN (4-6 digits)</label>
                            <Input type="password" placeholder="••••" value={newPin.code} maxLength={6} onChange={(e) => setNewPin({ ...newPin, code: e.target.value.replace(/\D/g, '') })} className="bg-zinc-900 border-zinc-700 text-zinc-200" />
                        </div>
                        <div>
                            <label className="text-xs text-zinc-400 mb-1 block">Valid Until</label>
                            <Input type="datetime-local" value={newPin.valid_until} onChange={(e) => setNewPin({ ...newPin, valid_until: e.target.value })} className="bg-zinc-900 border-zinc-700 text-zinc-200 text-xs" />
                        </div>
                        <Button onClick={createPin} disabled={creating} className="bg-emerald-600 text-white hover:bg-emerald-700">
            ...
            <Card className="bg-zinc-950 border-zinc-800">
                <CardHeader>
                    <CardTitle className="text-zinc-100 flex items-center gap-2 text-sm"><Key className="h-4 w-4 text-emerald-400" />Active PINs ({activePins.length})</CardTitle>
                </CardHeader>
                <CardContent className="p-0">
                    <table className="w-full">
                        ...
                        <tbody>
                            <AnimatePresence>
                                {activePins.map(pin => (
                                    <motion.tr key={pin.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="border-b border-zinc-900 hover:bg-zinc-900/50 transition-colors">
                                        <td className="p-3"><div className="flex items-center gap-2"><Hash className="h-3.5 w-3.5 text-emerald-400" /><span className="text-sm text-zinc-200 font-medium">{pin.name}</span></div></td>
                                        <td className="p-3 text-sm text-zinc-300">{pin.door_display_name}</td>
                                        <td className="p-3"><code className="text-xs text-zinc-400 bg-zinc-900 px-2 py-0.5 rounded">{pin.code_hint}</code></td>
                                        <td className="p-3 text-xs text-zinc-400">
                                            {pin.valid_until ? <div className="flex items-center gap-1"><Timer className="h-3 w-3 text-amber-400" />{new Date(pin.valid_until).toLocaleString('en-GB', { dateStyle: 'short', timeStyle: 'short' })}</div> : <span className="text-zinc-600">No expiry</span>}
                                        </td>
                                        <td className="p-3 text-xs text-zinc-500">{new Date(pin.created_at).toLocaleDateString('en-GB')} by {pin.created_by}</td>
                            ...
                            {activePins.length === 0 && <tr><td colSpan={6} className="p-6 text-center text-zinc-500">No active PINs. Create one above.</td></tr>}
                        </tbody>
                    </table>
                </CardContent>
            </Card>
            ...
            {revokedPins.length > 0 && (
                <Card className="bg-zinc-950 border-zinc-800/50">
                    <CardHeader><CardTitle className="text-zinc-400 flex items-center gap-2 text-sm"><Ban className="h-4 w-4 text-zinc-500" />Revoked PINs ({revokedPins.length})</CardTitle></CardHeader>
                    <CardContent className="p-0">
                        <table className="w-full">
                            <tbody>
                                {revokedPins.map(pin => (
                                    <tr key={pin.id} className="border-b border-zinc-900/30">
                                        ...
                                        <td className="p-3 text-xs text-zinc-600">Revoked {pin.revoked_at ? new Date(pin.revoked_at).toLocaleDateString('en-GB') : ''}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
