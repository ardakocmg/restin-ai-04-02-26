                    ...
                    <div className="grid grid-cols-5 gap-3 items-end">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                <span className="text-xs text-zinc-400">Unlatch</span>
                            </div>
                        </div>
                        <Button onClick={createPermission} className="bg-emerald-600 hover:bg-emerald-700 text-white">
                    ...
                    <table className="w-full">
                        ...
                        <tbody>
                            {permissions.map(perm => (
                                <tr key={perm.id} className="border-b border-zinc-900 hover:bg-zinc-900/30 transition-colors">
                                    <td className="p-3 text-sm text-zinc-200">{getDoorName(perm.door_id)}</td>
                                    <td className="p-3">
                                        <Badge variant="outline" className="border-zinc-700 text-zinc-300">{perm.role_id}</Badge>
                                    </td>
                                    <td className="p-3 text-center">
                                        {perm.can_unlock ? <ShieldCheck className="h-4 w-4 text-emerald-400 mx-auto" /> : <ShieldAlert className="h-4 w-4 text-zinc-600 mx-auto" />}
                                    </td>
                                    <td className="p-3 text-center">
                                        {perm.can_lock ? <ShieldCheck className="h-4 w-4 text-emerald-400 mx-auto" /> : <ShieldAlert className="h-4 w-4 text-zinc-600 mx-auto" />}
                                    </td>
                                    <td className="p-3 text-center">
                                        {perm.can_unlatch ? <ShieldCheck className="h-4 w-4 text-emerald-400 mx-auto" /> : <ShieldAlert className="h-4 w-4 text-zinc-600 mx-auto" />}
                                    </td>
                            ...
                            {permissions.length === 0 && (
                                <tr><td colSpan={6} className="p-6 text-center text-zinc-500">No permissions configured. Add one above.</td></tr>
                            )}
                        </tbody>
                    </table>
                </CardContent>
