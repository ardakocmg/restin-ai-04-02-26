...
const STATUS_COLORS = {
  ...
  ended: "bg-zinc-500/20 text-muted-foreground border-zinc-500/50"
};
              ...
              <div key={contract.id} className="bg-background rounded-lg p-6 border border-border shadow-sm">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <h3 className="text-lg font-bold text-foreground capitalize">{contract.contract_type.replace("_", " ")}</h3>
                  </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-sm mb-4">
                  <div>
                    ...
                    <p className="text-foreground">€{contract.base_rate}/month</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Start Date</p>
                    <p className="text-foreground">{new Date(contract.start_date).toLocaleDateString()}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">Hours/Week</p>
                    <p className="text-foreground">{contract.standard_hours_per_week}</p>
                  </div>
                </div>
