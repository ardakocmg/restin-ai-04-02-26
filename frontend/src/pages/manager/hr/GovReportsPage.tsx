...
export default function GovReportsPage() {
    ...
    const loadData = async () => {
        ...
        try {
            ...
            logger.error("Failed to load report data", { error: String(error) });
    ...
    const handleDownload = async (reportType: string, id: string | null = null) => {
                ...
                <div className="space-y-6">
                    <Tabs defaultValue="monthly" className="w-full">
                        ...
                        <TabsContent value="monthly" className="mt-6 space-y-4">
                            <h3 className="text-lg font-bold text-foreground mb-4">FS5 - Monthly Payer's Receipt</h3>
                                    ...
                                    <Card key={run.id} className="bg-card border-border">
                                        <div className="p-6 flex items-center justify-between">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div>
                                                    <h4 className="font-bold text-foreground text-lg">{run.run_name || 'Payroll Run'}</h4>
                                                    <div className="flex items-center gap-2 mt-1">
                                                        <Badge variant={run.state === 'committed' ? 'secondary' : 'outline'}>
                                                            {run.state?.toUpperCase()}
                                                        </Badge>
                                                        <span className="text-muted-foreground text-sm">Period: {run.period_start} - {run.period_end}</span>
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <Card className="bg-card border-border">
                                    ...
                                    <CardContent className="space-y-4">
                                        {fs7Data ? (
                                            <div className="p-4 bg-secondary/50 rounded-lg space-y-2 text-sm text-muted-foreground">
                                                <div className="flex justify-between">
                                                    <span>Total Gross Emoluments</span>
                                                    <span className="text-foreground font-mono">€{fs7Data.total_gross_emoluments.toFixed(2)}</span>
                                                </div>
                                                <div className="flex justify-between">
                                                    <span>Total FSS Tax</span>
                                                    <span className="text-foreground font-mono">€{fs7Data.total_fss_tax.toFixed(2)}</span>
                                                </div>
                                                <div className="flex justify-between">
                                                    <span>Total SSC</span>
                                                    <span className="text-foreground font-mono">€{fs7Data.total_ssc.toFixed(2)}</span>
                                                </div>
                                                <div className="flex justify-between border-t border-border pt-2 mt-2">
                                                    <span className="font-bold">Total Due</span>
                                                    <span className="text-foreground font-mono font-bold">€{fs7Data.total_due.toFixed(2)}</span>
                                                </div>
                                            </div>
                                        ) : (
