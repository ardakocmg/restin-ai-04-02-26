  ...
  return (
    <PermissionGate requiredRole="MANAGER">
      <div className="min-h-screen bg-[#0A0A0B] text-foreground flex flex-col">
        ...
        <div className="h-16 border-b border-border flex items-center justify-between px-6 bg-card/50 backdrop-blur-xl sticky top-0 z-20">
          <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-black text-foreground uppercase tracking-tighter">Scheduler</h1>
              <Badge variant="outline" className="border-border text-muted-foreground text-[10px] uppercase ml-2 tracking-widest hidden lg:flex">
                v2.1.0
              </Badge>
            </div>
            ...
            {/* View Toggle */}
            <div className="flex items-center bg-black/40 rounded-lg p-1 border border-border shadow-inner">
              <button
                onClick={() => setViewType('week')}
                className={cn(
                  "px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-all",
                  viewType === 'week' ? "bg-blue-500/20 text-blue-400 shadow-[0_0_12px_-2px_rgba(59,130,246,0.3)]" : "text-muted-foreground hover:text-secondary-foreground"
                )}
              >
                Week
              </button>
              <button
                ...
                className={cn(
                  "px-3 py-1 rounded text-[10px] font-bold uppercase tracking-widest transition-all",
                  viewType === 'month' ? "bg-blue-500/20 text-blue-400 shadow-[0_0_12px_-2px_rgba(59,130,246,0.3)]" : "text-muted-foreground hover:text-secondary-foreground"
                )}
              >
                Month
              </button>
            ...
            <div className="flex items-center gap-2 bg-black/40 rounded-lg p-1 border border-border shadow-inner">
              ...
              <div className="flex flex-col items-center px-4 min-w-[140px]">
                <span className="text-xs font-bold text-foreground font-mono tracking-tight">{data?.week_start}</span>
                <span className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">{viewType === 'month' ? 'Month View' : 'Week View'}</span>
              </div>
              <Button
                variant="ghost"
                size="icon"
            ...
            <div className="relative group w-72">
              ...
              <input
                ...
                className="w-full bg-black/40 border border-border rounded-full px-10 py-1.5 text-[11px] text-secondary-foreground placeholder:text-muted-foreground outline-none focus:border-blue-500/50 shadow-inner group-hover:bg-black/60 transition-all border-l-2 border-l-blue-500/30"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="outline" className={cn(
                  "h-8 border-border bg-black/40 text-[10px] uppercase font-bold tracking-widest gap-2 flex hover:text-foreground transition-all",
                  timeRange !== 'all' ? "text-blue-400 border-blue-500/30" : "text-foreground"
                )}>
              ...
              <DropdownMenuContent className="bg-[#18181b] border-border text-secondary-foreground shadow-2xl">
            ...
            <Button className="h-8 bg-blue-600 hover:bg-blue-500 text-foreground text-[10px] font-bold uppercase tracking-widest px-4 shadow-[0_0_15px_-3px_rgba(37,99,235,0.4)] transition-all">
        ...
        <div className="flex-1 overflow-auto p-4 custom-scrollbar">
          <div className="border border-border rounded-xl bg-card/30 overflow-hidden shadow-2xl">
            <table className="w-full border-collapse">
              <thead>
                <tr className="bg-card/80 border-b border-border">
                  <th className="p-4 text-left min-w-[200px] sticky left-0 bg-[#121214] z-10 border-r border-border shadow-[4px_0_12px_-2px_rgba(0,0,0,0.5)]">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center justify-between cursor-pointer group/sort" onClick={() => handleSort('employee_name')}>
                        <div className="flex items-center gap-1">
                          <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Employee</span>
                        ...
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">{filteredAndSortedRows.length} Staff</span>
                      </div>
                  ...
                  <th className="p-3 text-left min-w-[120px] bg-card/50 border-r border-border">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-1 cursor-pointer group/sort" onClick={() => handleSort('occupation')}>
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Occupation</span>
                  ...
                  <th className="p-3 text-left min-w-[100px] bg-card/50 border-r border-border">
                    <div className="flex flex-col gap-2">
                      <div className="flex items-center gap-1 cursor-pointer group/sort" onClick={() => handleSort('cost_centre')}>
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Cost Centre</span>
                  ...
                  {dayKeys.map((dateKey) => {
                    ...
                    return (
                      <th key={dateKey} className="p-3 text-center min-w-[140px] border-r border-border last:border-0 relative group">
                        <div className="absolute inset-0 bg-white/0 group-hover:bg-white/[0.02] pointer-events-none transition-colors" />
                        <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-0.5">{formattedDay}</div>
                        <div className="text-[9px] font-medium text-muted-foreground uppercase tracking-tight">{formattedDate}</div>
                      </th>
                    )
                  })}
                  <th className="p-3 text-center min-w-[100px] border-l border-border bg-card/50 cursor-pointer group/sort" onClick={() => handleSort('basic_hrs_overtime')}>
                    <div className="flex flex-col items-center gap-1">
                      <div className="flex items-center gap-1">
                        <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Hrs</span>
                  ...
                  <th className="p-3 text-center min-w-[100px] bg-card/50 cursor-pointer group/sort" onClick={() => handleSort('cost_eur')}>
                    <div className="flex flex-col items-center gap-1">
                      <div className="flex items-center gap-1">
                  ...
                  <tr key={idx} className="border-b border-border hover:bg-white/[0.02] group transition-colors">
                    ...
                    {dayKeys.map(day => {
                      ...
                      return (
                        <td
                          key={day}
                          className="p-1 border-r border-border last:border-0 relative h-[70px]"
                    ...
                    <td className="p-3 text-center border-l border-border">
                      <span className="text-xs font-mono font-bold text-muted-foreground">{row.basic_hrs_overtime}</span>
                    </td>
        ...
        {/* Context Menu (Custom) */}
        {
          contextMenu.visible && (
            ...
            >
              ...
              <Button variant="ghost" onClick={() => handleAction('Copy')} className="w-full justify-start h-8 text-xs font-medium text-secondary-foreground hover:text-foreground hover:bg-white/10">
