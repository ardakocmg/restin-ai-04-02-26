...
const STATUS_CONFIG: Record<EmployeeStatus, { color: string; label: string }> = {
  ...
  terminated: { color: 'bg-zinc-500/20 text-muted-foreground border-zinc-500/30', label: 'Terminated' },
};
  ...
  return (
    <div className="space-y-1.5">
      ...
      <Input aria-label="Input field"
        ...
        disabled={disabled}
        className="bg-background border-border text-secondary-foreground h-10 disabled:opacity-50"
      />
    </div>
  );
}
...
function InfoField({ label, value, highlight = false }: InfoFieldProps) {
  return (
    <div className="bg-card/40 p-3 rounded-xl border border-border hover:border-border transition-all group">
      ...
      <p className={`text-sm font-bold ${highlight ? 'text-blue-400' : 'text-foreground'}`}>
...
export default function EmployeeDetailPage() {
  ...
  if (!data) return (
    <div className="p-8 text-center text-muted-foreground bg-[#0A0A0B] min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <div className="max-w-6xl mx-auto space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 text-muted-foreground text-xs font-bold uppercase tracking-wider mb-1">
                  ...
                  <span className="text-secondary-foreground">Personnel File</span>
                </div>
            ...
            <div className="flex gap-2">
              <Button
                variant="outline"
                className="border-border bg-card/50 hover:bg-white/5 text-xs font-bold uppercase tracking-widest h-10"
              ...
              <Button
                onClick={handleSave}
                disabled={saving}
                className="bg-blue-600 hover:bg-blue-500 text-foreground font-bold text-xs uppercase tracking-widest h-10 rounded-xl shadow-lg shadow-blue-500/20"
              >
          ...
          <div className="bg-gradient-to-br from-blue-600/10 to-transparent border border-border rounded-3xl p-8 relative overflow-hidden backdrop-blur-3xl">
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
              <div className="h-28 w-28 rounded-3xl bg-secondary border-2 border-border flex items-center justify-center text-4xl font-black text-muted-foreground shadow-2xl overflow-hidden relative group">
                {(data.first_name || data.full_name || data.name || '?').charAt(0)}{(data.last_name || '').charAt(0)}
                <div className="absolute inset-0 bg-blue-500/0 group-hover:bg-blue-500/10 transition-colors" />
              </div>
              <div className="space-y-3 text-center md:text-left flex-1">
                <div>
                  <h2 className="text-3xl font-black text-foreground tracking-tighter uppercase leading-none mb-1">
                    {data.first_name && data.last_name ? `${data.first_name} ${data.last_name}` : data.full_name || data.name || 'Unknown'}
                  </h2>
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-4 text-muted-foreground text-xs">
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <TabsList className="bg-card/50 border border-border p-1 rounded-2xl h-auto backdrop-blur-xl flex flex-wrap gap-1">
              {[
                ...
                <TabsTrigger
                  key={tab.id}
                  value={tab.id}
                  className="rounded-xl px-4 py-2.5 data-[state=active]:bg-white/10 data-[state=active]:text-foreground font-bold uppercase tracking-widest text-[10px] flex items-center gap-1.5"
                >
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  {[
                    ...
                    <button
                      ...
                      className="group flex items-center gap-3 p-3 rounded-xl bg-card/50 border border-border hover:border-blue-500/30 hover:bg-blue-500/5 transition-all text-left"
                    >
                      ...
                      <div className="min-w-0">
                        <div className="text-[11px] font-bold text-foreground group-hover:text-blue-400 transition-colors truncate">{action.label}</div>
                        <div className="text-[9px] text-muted-foreground truncate">{action.desc}</div>
                      </div>
                    </button>
                  ))}
                </div>
            ...
            <TabsContent value="documents" className="space-y-8 animate-in fade-in duration-300">
              <Section
                ...
                icon={FileText}
                actions={
                  <Button size="sm" className="bg-blue-600 hover:bg-blue-500 text-foreground text-[10px] uppercase font-bold tracking-widest h-8">
                    ...
                    {documents.map((doc: EmployeeDocument, i: number) => (
                      <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-card/40 border border-border hover:border-border transition-all group">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-secondary-foreground">{doc.name || doc.filename}</p>
                    ...
                    {payrollRuns.slice(0, 12).map((run: PayrollRun, i: number) => (
                      <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-card/40 border border-border hover:border-border transition-all">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-secondary-foreground">{run.period || run.name}</p>
                            <p className="text-[10px] text-muted-foreground">{run.created_at ? new Date(run.created_at).toLocaleDateString() : ''}</p>
                          </div>
                        </div>
                        <Badge className={
                      ...
                      <CardContent>
                        ...
                        <div className="flex justify-end mt-4">
                          <Button
                            ...
                            className="bg-violet-600 hover:bg-violet-500 text-foreground text-xs font-bold uppercase tracking-widest"
                          >
                    ...
                    {data.user_id ? (
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-sm font-bold text-secondary-foreground">User account linked</p>
                            <p className="text-[10px] text-muted-foreground">ID: {data.user_id}</p>
                          </div>
                        </div>
                        <Button
                          variant="outline"
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <Textarea aria-label="Add an admin note about this employee..."
                      ...
                      className="bg-background border-border text-secondary-foreground min-h-20 flex-1 resize-none"
                    />
                    <Button
                      onClick={handleAddNote}
                      disabled={!newNote.trim() || addingNote}
                      className="bg-blue-600 hover:bg-blue-500 text-foreground px-6 self-end h-10"
                    >
