        ...
        return (
            <PageContainer title="Not Found" description="Payroll run not found">
                <div className="flex flex-col items-center justify-center min-h-[50vh] space-y-4">
                    ...
                    <h3 className="text-xl font-bold text-foreground">Run Not Found</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <Card className="bg-card border-border">
                            <CardContent className="p-6">
                                ...
                                <div className="text-2xl font-bold text-foreground mt-2">€{(run.total_gross + 1200).toLocaleString()}</div> {/* Mock Employer Cost */}
                                <p className="text-[10px] text-muted-foreground mt-1">Includes SSC contribution</p>
                            </CardContent>
                        </Card>
                        <Card className="bg-card border-border">
                            <CardContent className="p-6">
                                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Gross Pay</p>
                                <div className="text-2xl font-bold text-secondary-foreground mt-2">€{run.total_gross.toLocaleString()}</div>
                            </CardContent>
                        </Card>
                        <Card className="bg-card border-border">
                            <CardContent className="p-6">
                                <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Total Tax (FSS)</p>
                    ...
                    <Card className="bg-card border-border">
                        <CardHeader className="border-b border-border pb-4">
                            <div className="flex items-center justify-between">
                                <CardTitle className="text-lg font-bold text-foreground">Employee Payslips</CardTitle>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-sm text-left">
                                <thead className="bg-secondary/50 text-muted-foreground uppercase text-[10px] tracking-wider font-bold">
                                    <tr>
                                ...
                                <tbody className="divide-y divide-border">
                                    {run.payslips.map((slip) => (
                                        <tr key={slip.employee_code} className="hover:bg-secondary/30 transition-colors group">
                                            <td className="px-6 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <div className="font-bold text-foreground">{slip.employee_name}</div>
                                                        <div className="text-xs text-muted-foreground">{slip.employee_role} • {slip.employee_code}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 text-right font-mono text-secondary-foreground">€{slip.basic_pay.toFixed(2)}</td>
                                            <td className="px-6 py-4 text-right font-mono text-secondary-foreground">€{slip.overtime_pay.toFixed(2)}</td>
                                            <td className="px-6 py-4 text-right font-mono text-foreground font-medium">€{slip.gross_pay.toFixed(2)}</td>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                ...
                                                >
                                                    <Printer className="h-4 w-4 text-muted-foreground group-hover:text-foreground" />
                                                </Button>
                                            </td>
                                        </tr>
                                    ))}
