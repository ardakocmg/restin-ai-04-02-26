            ...
            <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700">
                ...
                <div className="flex items-end justify-between border-b border-white/5 pb-8">
                    <div>
                        <div className="flex items-center gap-4 mb-2">
                            ...
                            <h1 className="text-4xl font-black text-white uppercase tracking-tighter leading-none">Device Manager</h1>
                        </div>
                        <p className="text-zinc-500 font-bold uppercase tracking-[0.2em] text-[10px] ml-14">Infrastructure & Terminal Monitoring</p>
                    </div>
                    <div className="flex items-center gap-3">
                        <Button className="bg-zinc-900 border border-white/10 hover:bg-zinc-800 text-[10px] font-black uppercase px-6 py-6 rounded-xl gap-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    {[
                        ...
                        <Card key={i} className="bg-zinc-900/30 border-white/5 overflow-hidden">
                            <CardContent className="p-6">
                                <div className="flex items-center justify-between mb-2">
                                    <div className={`p-2 rounded-lg bg-${stat.color}-500/10 text-${stat.color}-500`}>
                                        <stat.icon className="w-4 h-4" />
                                    </div>
                                    <span className="text-[10px] font-black text-zinc-600">LIVE</span>
                                </div>
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between px-2">
                        <h2 className="text-[11px] font-bold text-zinc-500 uppercase tracking-widest">Connected Infrastructure ({devices.length})</h2>
                    </div>

                    <div className="grid grid-cols-1 gap-3">
                        {devices.map((device, idx) => (
                            <Card key={idx} className="bg-zinc-900/40 border-white/5 hover:border-white/10 transition-all group overflow-hidden">
                                <CardContent className="p-0">
                                    <div className="flex items-center justify-between p-6">
                                        <div className="flex items-center gap-6">
                                            <div className={`p-4 rounded-2xl ${device.status === 'Online' ? 'bg-zinc-800 text-zinc-100' : 'bg-red-500/10 text-red-500'}`}>
                                                <Cpu className="w-8 h-8" />
                                            </div>
                                            <div>
                                                <div className="flex items-center gap-3">
                                                    <h3 className="text-lg font-black text-white uppercase tracking-tighter leading-none">{device.name}</h3>
                                                ...
                                                <div className="flex items-center gap-4 mt-2">
                                                    <span className="text-[10px] font-bold text-zinc-500 uppercase flex items-center gap-1.5 leading-none">
                                            ...
                                            <div className="flex gap-8">
                                                <div className="text-center">
                                                    ...
                                                    <div className="flex items-center justify-center gap-1 text-zinc-300">
                                                        <Wifi className={`w-4 h-4 ${device.signal === 'None' ? 'text-zinc-700' : 'text-zinc-300'}`} />
                                                        <span className="text-[10px] font-black">{device.signal}</span>
                                                    </div>
                                                </div>
                                                <div className="text-center">
                                                    <p className="text-[8px] font-bold text-zinc-600 uppercase tracking-widest mb-1">Battery</p>
                                                    <div className="flex items-center justify-center gap-1 text-zinc-300">
                                                        <Battery className={`w-4 h-4 ${parseInt(device.battery) < 20 ? 'text-red-500 animate-pulse' : 'text-zinc-300'}`} />
                                                        <span className="text-[10px] font-black">{device.battery}</span>
                                                    </div>
                                                </div>
                                                <div className="text-center">
                                                    <p className="text-[8px] font-bold text-zinc-600 uppercase tracking-widest mb-1">Latency</p>
                                                    <span className="text-[10px] font-black text-zinc-300">{device.lastPing}</span>
                                                </div>
                                            </div>

                                            <div className="flex gap-2">
                                                <Button variant="outline" className="h-10 w-10 p-0 border-white/5 bg-zinc-800/50 hover:bg-zinc-800 text-zinc-400">
                                    ...
                                    <div className="px-6 py-3 bg-white/5 flex items-center justify-between">
                                        <div className="flex items-center gap-4 text-[9px] font-bold text-zinc-500 uppercase tracking-tight">
                ...
                <div className="flex items-center gap-4 p-6 rounded-2xl bg-blue-600/5 border border-blue-500/10">
                    ...
                    <div className="flex-1">
                        <p className="text-xs font-bold text-white uppercase tracking-tight">Operational Support</p>
                        <p className="text-zinc-500 text-[10px] font-medium uppercase tracking-widest mt-1 leading-snug">
