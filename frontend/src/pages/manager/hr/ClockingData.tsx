...
const DEVICE_BADGE: Record<string, { label: string; icon: React.ElementType; bg: string; text: string; border: string }> = {
  ...
  import: { label: 'Import', icon: Download, bg: 'bg-zinc-500/15', text: 'text-muted-foreground', border: 'border-zinc-500/20' },
};
  ...
  const StatusDot = ({ status }: { status: string }) => {
    ...
    return (
      <div className="flex items-center gap-1.5">
        ...
        <span className={`text-[9px] font-bold uppercase tracking-wider ${isActive ? 'text-emerald-400' : 'text-muted-foreground'}`}>
    ...
    <PermissionGate requiredRole="MANAGER">
      <div className="p-6 bg-[#09090b] min-h-screen text-foreground font-sans">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h1 className="text-3xl font-black text-foreground uppercase tracking-tighter mb-1">Clocking Data</h1>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button onClick={handleSearch} className="bg-blue-600 hover:bg-blue-500 text-foreground h-10 px-5">
              <Filter className="h-4 w-4 mr-2" />Filter
            </Button>

            <Button
              onClick={() => navigate('/manager/hr/clocking/add')}
              className="bg-violet-600 hover:bg-violet-500 text-foreground h-10 px-5"
            >
            ...
            <Button
              ...
              className="bg-emerald-600 hover:bg-emerald-500 text-foreground h-10 px-5"
            >
                    ...
                    <tr className="bg-background/50 border-b border-border/50">
                      {TABLE_COLS.map((col) => (
                        <th
                          ...
                          className="p-3 text-left text-[10px] font-bold text-muted-foreground uppercase tracking-widest cursor-pointer hover:text-secondary-foreground transition-colors select-none whitespace-nowrap"
                        >
                      ...
                      <th className="p-3 text-center text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Actions</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-border/30">
                    {sortedData.map((record, idx) => (
                      <tr key={record.id ? `${record.id}_${idx}` : idx} className="group transition-colors odd:bg-[#0f0f11] even:bg-[#151518] hover:bg-blue-900/10">
                        {/* Status */}
                        <td className="p-3"><StatusDot status={record.status || 'completed'} /></td>
                        {/* Day */}
                        <td className="p-3 text-[11px] font-medium text-muted-foreground">{record.day_of_week}</td>
                        {/* Date */}
                        <td className="p-3 text-[11px] font-medium text-muted-foreground font-mono">{record.date}</td>
                        {/* In */}
                        <td className="p-3">
                        ...
                        {/* Duration */}
                        <td className="p-3">
                          <span className="text-[11px] font-bold text-secondary-foreground font-mono">
                          ...
                          >
                            <div className="flex flex-col">
                              ...
                              {record.employee_designation && (
                                <span className="text-[9px] font-bold text-muted-foreground mt-0.5">{record.employee_designation}</span>
                              )}
                            </div>
                          </button>
                        </td>
                        ...
                        {/* Cost Centre */}
                        <td className="p-3">
                          <span className="inline-flex items-center px-2 py-1 rounded bg-secondary text-[9px] font-bold text-muted-foreground uppercase tracking-wider border border-border/50">
                        ...
                        {/* Device Name */}
                        <td className="p-3 text-[10px] font-medium text-muted-foreground">{record.device_name || '—'}</td>
                        {/* Actions */}
                    ...
                    {sortedData.length === 0 && !loading && (
                      <tr>
                        <td colSpan={TABLE_COLS.length + 1} className="p-12 text-center text-muted-foreground text-xs uppercase tracking-widest font-bold">
