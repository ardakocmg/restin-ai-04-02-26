        ...
        <PageContainer
            ...
            actions={
                <div className="flex gap-3">
                    ...
                    {activeTab === 'config' && (
                        <button onClick={saveConfig} disabled={saving} className="flex items-center gap-2 px-6 py-2.5 bg-red-600 hover:bg-red-500 text-white font-bold rounded-xl transition-all disabled:opacity-50">
            ...
            {activeTab === 'config' && (
                <div className="max-w-3xl space-y-3">
                    {configFields.map(cfg => (
                        <div key={cfg.field} className="flex items-center justify-between p-5 bg-zinc-900/50 border border-white/5 rounded-2xl group hover:border-white/10 transition-all">
            ...
            {activeTab === 'assignments' && (
                <div className="space-y-6">
                    ...
                    <div className="p-5 bg-zinc-900/50 border border-white/5 rounded-2xl space-y-4">
                        <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">New Assignment</h3>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                            <input
                                value={newServerName}
                                onChange={e => setNewServerName(e.target.value)}
                                placeholder="Server Name (e.g. John)"
                                className="h-10 bg-zinc-950 border border-white/10 rounded-lg text-white px-3 text-sm outline-none focus:border-red-500"
                            />
                            <input
                                value={newTableNames}
                            ...
                            <button
                                ...
                                className="h-10 bg-red-600 hover:bg-red-500 text-white font-bold text-sm rounded-lg transition-all flex items-center justify-center gap-2"
                            >
                    ...
                    {assignments.length === 0 ? (
                        <div className="p-12 bg-zinc-900/30 border border-dashed border-white/5 rounded-2xl text-center">
                            ...
                            <p className="text-sm text-zinc-500">No server assignments yet</p>
                            <p className="text-xs text-zinc-600 mt-1">Assign tables to servers for tableside ordering</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500">Active Assignments</h3>
                            {assignments.map(a => (
                                <div key={a.id} className="flex items-center justify-between p-4 bg-zinc-900/50 border border-white/5 rounded-xl group hover:border-white/10 transition-all">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-xl bg-red-600/15 flex items-center justify-center">
                                            <Users className="w-5 h-5 text-red-400" />
                                        </div>
                                        <div>
                                            <p className="text-sm font-bold text-white">{a.server_name}</p>
                                            <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                                                {a.table_names.map(t => (
                                                    <span key={t} className="px-2 py-0.5 bg-zinc-800 text-zinc-400 text-[10px] font-bold rounded-md border border-white/5">
                                                        {t}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <button
                                            ...
                                            className="p-2 text-zinc-600 hover:text-red-400 hover:bg-red-600/10 rounded-lg transition-all opacity-0 group-hover:opacity-100"
                ...
                <div className="space-y-6">
                    <div className="p-5 bg-zinc-900/50 border border-white/5 rounded-2xl">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 className="text-sm font-bold text-white">Mobile POS Preview</h3>
                                <p className="text-[10px] text-zinc-500">Tables assigned to your server account</p>
                            </div>
                        </div>
                        {myTables.length === 0 ? (
                            <div className="p-8 bg-zinc-950/50 border border-dashed border-white/5 rounded-xl text-center">
                                <Table2 className="w-8 h-8 text-zinc-700 mx-auto mb-2" />
                                <p className="text-xs text-zinc-500">No tables assigned to you</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                {myTables.map((table, i) => (
                                    <button
                                        key={table.name || i}
                                        className="p-4 bg-zinc-950/50 border border-white/5 rounded-xl text-center hover:border-red-500/30 hover:bg-red-600/5 transition-all group"
                                    >
                                        <Table2 className="w-6 h-6 text-zinc-600 mx-auto mb-2 group-hover:text-red-400 transition-colors" />
                                        <p className="text-sm font-bold text-white">{table.name}</p>
                                        {table.floor && (
                                            <p className="text-[10px] text-zinc-600 mt-0.5">{table.floor}</p>
                                        )}
                                        {table.seats && (
                                            <p className="text-[10px] text-zinc-500">{table.seats} seats</p>
                                        )}
