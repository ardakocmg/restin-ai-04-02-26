...
export default function VisualContentEditor() {
  ...
  useEffect(() => {
    logAction('CONTENT_EDITOR_VIEWED', 'visual_content_editor', undefined, { user_id: user?.id });
  }, []);
      ...
      <PageContainer
        ...
        actions={
          <div className="flex gap-2">
            <button onClick={() => setPreview(!preview)} className="flex items-center gap-2 px-4 py-2 bg-slate-700 text-foreground rounded-lg hover:bg-slate-600">
              <Eye className="h-4 w-4" />
              {preview ? 'Edit' : 'Preview'}
            </button>
            <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-foreground rounded-lg hover:bg-blue-700 disabled:opacity-50">
