  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {loading ? (
          <p className="col-span-full text-center py-12 text-zinc-500 font-bold animate-pulse">Loading Floor Plans...</p>
        ) : floorPlans.length === 0 ? (
          <p className="col-span-full text-center py-12 text-zinc-500 font-bold italic">No floor plans found</p>
        ) : (
          floorPlans.map(plan => (
            <Card key={plan.id} className="bg-zinc-950 border-white/5 shadow-2xl hover:border-red-500/30 transition-all group overflow-hidden">
              <CardContent className="p-6">
                <div className="flex items-start justify-between mb-6">
                  <div className="flex-1">
                    <h3 className="font-black text-white uppercase tracking-tight text-lg mb-2 group-hover:text-red-500 transition-colors">{plan.name}</h3>
                    {plan.is_active && (
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="p-3 rounded-lg bg-zinc-900/50 border border-white/5">
                    ...
                    <p className="text-sm font-bold text-white">{plan.width} x {plan.height}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-zinc-900/50 border border-white/5">
                    <p className="text-[10px] font-black text-zinc-500 uppercase">Version</p>
                    <p className="text-sm font-bold text-white">v{plan.version}</p>
                  </div>
                </div>
                <Button
                  variant="outline"
