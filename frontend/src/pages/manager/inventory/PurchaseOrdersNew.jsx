...
function POStatusBadge({ status }) {
  const config = {
    DRAFT: { label: 'Draft', icon: FileText, className: 'text-zinc-500 border-zinc-400' },
  ...
  const COLUMNS = useMemo(() => [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        return <Badge variant="outline" className="text-xs text-zinc-500 gap-1"><Clock className="h-3 w-3" /> Unmatched</Badge>;
      },
    },
    {
