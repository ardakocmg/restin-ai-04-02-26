          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-zinc-500 text-xs">
                ...
                <span className="text-zinc-600 ml-1">({totalRecords} items)</span>
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-start justify-between gap-2">
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 border ${isActive
                          ...
                          : 'bg-zinc-900/50 border-zinc-800 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50'
                          }`}
                      >
                        <Icon className="w-3.5 h-3.5" />
                        <span className="tracking-wide uppercase">{stat.label}</span>
                        <span className={`font-mono text-sm font-black ${isActive ? `text-${stat.color}-300` : 'text-white'}`}>
                          {stat.value}
                        </span>
                      </button>
                    );
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center justify-between mb-3">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${isActive ? `bg-${stat.color}-500/20` : 'bg-zinc-800'
                            }`}>
                            <Icon className={`w-6 h-6 ${isActive ? `text-${stat.color}-400` : 'text-zinc-500 group-hover:text-zinc-300'}`} />
                          </div>
                        </div>
                        <div className={`text-3xl font-black font-mono mb-1 ${isActive ? `text-${stat.color}-300` : 'text-white'}`}>
                          {stat.value}
                        </div>
                        <div className={`text-xs font-medium uppercase tracking-wider ${isActive ? `text-${stat.color}-400/80` : 'text-zinc-500'}`}>
                          {stat.label}
                        </div>
                      </button>
                    );
              ...
              <div className="flex flex-col gap-1 ml-1">
                <button
                  ...
                  className="p-1.5 rounded-md transition-all text-zinc-500 hover:text-zinc-300 bg-zinc-800/50 hover:bg-zinc-700"
                >
                ...
                <button
                  ...
                  title="Hide stats"
                  className="p-1.5 rounded-md transition-all text-zinc-600 hover:text-zinc-400"
                >
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        <div className="relative flex-1 min-w-[160px] max-w-xs">
          ...
          <Input
            ...
            className="pl-8 h-8 text-xs bg-zinc-900 border-zinc-800 focus:border-emerald-500"
          />
        </div>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className={`h-8 text-xs gap-1.5 border-zinc-800 bg-zinc-900 hover:bg-zinc-800 ${selectedCategories.length > 0 ? 'text-emerald-400 border-emerald-500/40' : 'text-zinc-400'}`}>
              {selectedCategories.length === 0 ? 'All Categories' : `${selectedCategories.length} Cat.`}
              <ChevronsUpDown className="w-3 h-3 opacity-50" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-52 p-0 bg-zinc-950 border-zinc-800" align="start">
            <div className="p-2 border-b border-zinc-800">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Categories</span>
                {selectedCategories.length > 0 && (
                  <button onClick={() => setSelectedCategories([])} className="text-[10px] text-zinc-500 hover:text-white">Clear</button>
                )}
              </div>
            </div>
            <div className="max-h-48 overflow-y-auto p-1">
              {availableCategories.filter(c => c !== 'All').map(c => (
                <label key={c} className="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-zinc-800/80 cursor-pointer text-xs text-zinc-300">
              ...
              {availableCategories.filter(c => c !== 'All').length === 0 && (
                <div className="text-xs text-zinc-600 text-center py-3">No categories</div>
              )}
            </div>
          </PopoverContent>
        </Popover>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className={`h-8 text-xs gap-1.5 border-zinc-800 bg-zinc-900 hover:bg-zinc-800 ${selectedSubcategories.length > 0 ? 'text-emerald-400 border-emerald-500/40' : 'text-zinc-400'}`}>
          ...
          <PopoverContent className="w-52 p-0 bg-zinc-950 border-zinc-800" align="start">
            <div className="p-2 border-b border-zinc-800">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Subcategories</span>
                {selectedSubcategories.length > 0 && (
                  <button onClick={() => setSelectedSubcategories([])} className="text-[10px] text-zinc-500 hover:text-white">Clear</button>
                )}
              </div>
            </div>
            <div className="max-h-48 overflow-y-auto p-1">
              ...
              {availableSubcategories.filter(c => c !== 'All').length === 0 && (
                <div className="text-xs text-zinc-600 text-center py-3">No subcategories</div>
              )}
            </div>
          </PopoverContent>
        </Popover>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button variant="outline" size="sm" className={`h-8 text-xs gap-1.5 border-zinc-800 bg-zinc-900 hover:bg-zinc-800 ${selectedDiets.length > 0 ? 'text-emerald-400 border-emerald-500/40' : 'text-zinc-400'}`}>
              {selectedDiets.length === 0 ? 'All Diets' : `${selectedDiets.length} Diet`}
              <ChevronsUpDown className="w-3 h-3 opacity-50" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-48 p-0 bg-zinc-950 border-zinc-800" align="start">
            <div className="p-2 border-b border-zinc-800">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Diets</span>
                {selectedDiets.length > 0 && (
                  <button onClick={() => setSelectedDiets([])} className="text-[10px] text-zinc-500 hover:text-white">Clear</button>
                )}
              </div>
            </div>
            <div className="max-h-48 overflow-y-auto p-1">
              {DIET_OPTIONS.map(d => (
                <label key={d.value} className="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-zinc-800/80 cursor-pointer text-xs text-zinc-300">
        ...
        {viewMode !== 'trash' && (
          <Button
            variant="ghost"
            size="sm"
            className={`h-8 w-8 p-0 ${statViewMode !== 'hidden' ? 'text-emerald-400 bg-emerald-500/10' : 'text-zinc-500 hover:text-white'}`}
        ...
        <Button variant="ghost" size="sm" className={`h-8 text-xs ${recipeSidebarOpen ? 'text-emerald-400 bg-emerald-500/10' : 'text-zinc-400 hover:text-white'}`} onClick={() => setRecipeSidebarOpen(!recipeSidebarOpen)}>
          <SlidersHorizontal className="w-3.5 h-3.5 mr-1" />
          Filters
          {recipeSidebarFilterCount > 0 && (
            <Badge className="ml-1 h-4 min-w-[16px] px-1 text-[9px] rounded-full bg-emerald-500 text-white">{recipeSidebarFilterCount}</Badge>
          )}
        </Button>
        ...
        <DropdownMenu open={columnMenuOpen} onOpenChange={setColumnMenuOpen}>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" size="sm" className={`h-8 w-8 p-0 ${Object.values(visibleColumns).some(v => !v) ? 'text-amber-400 bg-amber-500/10' : 'text-zinc-400 hover:text-white'}`} title="Toggle columns">
              <LayoutGrid className="w-3.5 h-3.5" />
            </Button>
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="w-44 bg-zinc-900 border-zinc-700">
            <DropdownMenuLabel className="text-[10px] text-zinc-500 uppercase tracking-wider">Columns</DropdownMenuLabel>
        ...
        {/* Action buttons */}
        <Button variant="ghost" size="sm" className="h-8 text-xs text-zinc-400 hover:text-white" onClick={() => navigate('/manager/migration')}>
          <UploadCloud className="w-3.5 h-3.5 mr-1" />
          Import
        </Button>
        <Button variant="ghost" size="sm" className="h-8 text-xs text-zinc-400 hover:text-white" onClick={() => navigate('/manager/data-exports')}>
          <Download className="w-3.5 h-3.5 mr-1" />
          Export
        </Button>
        <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-zinc-400 hover:text-white" onClick={() => loadData()}>
          <RefreshCw className="w-3.5 h-3.5" />
        </Button>
        <Button size="sm" className="h-8 text-xs bg-emerald-600 hover:bg-emerald-500 text-white font-bold" onClick={() => navigate('/manager/inventory-recipes/new')}>
        ...
        <div className="flex rounded-md border border-zinc-800 bg-zinc-900 overflow-hidden">
          <button
            ...
            className={`px-2 py-1.5 text-xs transition-colors ${displayMode === 'list' ? 'bg-emerald-600 text-white' : 'text-zinc-500 hover:text-white'}`}
          >
            <LayoutGrid className="w-3.5 h-3.5 rotate-0" />
          </button>
          <button
            onClick={() => setDisplayMode('grid')}
            className={`px-2 py-1.5 text-xs transition-colors ${displayMode === 'grid' ? 'bg-emerald-600 text-white' : 'text-zinc-500 hover:text-white'}`}
          >
            <LayoutGrid className="w-3.5 h-3.5" />
          </button>
        </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
              {sidebarFilteredRecipes.filter(r => {
                ...
                <Card
                  ...
                  className="bg-zinc-950 border-zinc-800 hover:border-emerald-500/50 hover:shadow-lg hover:shadow-emerald-500/5 transition-all cursor-pointer group overflow-hidden"
                ...
                >
                  <div className="aspect-[4/3] bg-zinc-900 relative overflow-hidden">
                    ...
                    {recipe.raw_import_data?.Category && (
                      <Badge className="absolute top-2 left-2 bg-zinc-900/90 text-xs">
                        {recipe.raw_import_data.Category}
                      </Badge>
                    )}
                  </div>
              ...
              {recipes.length === 0 && (
                <div className="col-span-full text-center py-12 text-zinc-500">No recipes found.</div>
              )}
            </div>
          ) : (
            <div>
              ...
              />
              {/* Pagination Controls */}
              <div className="flex items-center justify-between px-4 py-3 border-t border-white/10 bg-zinc-900/50">
                <div className="flex items-center gap-4">
                  <div className="text-sm text-zinc-500">
                    Showing <span className="text-white font-bold">{((page - 1) * limit) + 1}</span> to <span className="text-white font-bold">{Math.min(page * limit, totalRecords)}</span> of <span className="text-white font-bold">{totalRecords}</span>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1.5 text-sm font-medium text-white">
                    ...
                    <input
                      ...
                      className="w-16 h-8 text-center text-sm font-bold bg-zinc-800 border border-zinc-700 rounded text-white focus:border-emerald-500 focus:outline-none [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                    />
                    <span className="text-zinc-400">of {totalPages || 1}</span>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto bg-zinc-950 border-zinc-900 border-2 text-white">
          ...
          <div className="space-y-8 p-4">
            ...
            <div className="space-y-3 p-4 bg-zinc-900/30 border border-zinc-800/40 rounded-xl">
              <div className="flex items-center gap-2 text-zinc-400 mb-3 pb-2 border-b border-zinc-800/50">
                <Barcode className="w-3 h-3 text-cyan-500" />
                <h4 className="text-[10px] font-black uppercase tracking-widest">PLU â†” POS Mapping</h4>
                <span className="text-[9px] text-zinc-600 ml-auto">{pluMappings.length} code(s)</span>
              </div>
              <p className="text-[10px] text-zinc-500 mb-2">Link POS PLU codes to this recipe for automatic stock deduction. Each outlet can have a different PLU.</p>
              {pluMappings.map((plu, idx) => (
                <div key={idx} className="grid grid-cols-12 gap-2 items-center">
                  <div className="col-span-4">
                    <input
                      className="w-full bg-zinc-950/60 border border-zinc-800/60 rounded-lg px-3 py-2 text-sm text-cyan-300 focus:border-cyan-500/50 outline-none font-mono"
                      value={plu.code}
                  ...
                  <div className="col-span-4">
                    <select
                      className="w-full bg-zinc-950/60 border border-zinc-800/60 rounded-lg px-3 py-2 text-sm text-zinc-200 outline-none appearance-none"
                  ...
                  <div className="col-span-3">
                    <input
                      className="w-full bg-zinc-950/60 border border-zinc-800/60 rounded-lg px-3 py-2 text-sm text-zinc-200 outline-none"
                      value={plu.pos_name || ''}
              ...
              <Button variant="outline" size="sm" className="w-full border-dashed border-zinc-700 text-zinc-500 hover:text-cyan-400 hover:border-cyan-500/30" onClick={() => setPluMappings([...pluMappings, { code: '', outlet: 'all', pos_name: '' }])}>
            ...
            <div className="space-y-3 p-4 bg-zinc-900/30 border border-zinc-800/40 rounded-xl">
              <div className="flex items-center gap-2 text-zinc-400 mb-3 pb-2 border-b border-zinc-800/50">
                ...
                <label className="ml-auto flex items-center gap-2 cursor-pointer">
                  <span className="text-[9px] text-zinc-600">{stockableEnabled ? 'Active' : 'Off'}</span>
              ...
              {stockableEnabled && (
                <div className="space-y-3">
                  <p className="text-[10px] text-zinc-500">This recipe will be tracked as an inventory item. When produced, ingredients are deducted; when sold, the recipe quantity depletes.</p>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <label className="text-[9px] font-bold text-zinc-500 uppercase block mb-1">Inventory Unit</label>
                      <select className="w-full bg-zinc-950/60 border border-zinc-800/60 rounded-lg px-2 py-2 text-sm text-zinc-200 focus:border-emerald-500/50 outline-none"
                    ...
                    <div>
                      <label className="text-[9px] font-bold text-zinc-500 uppercase block mb-1">Shelf Life (days)</label>
                      <input type="number" className="w-full bg-zinc-950/60 border border-zinc-800/60 rounded-lg px-2 py-2 text-sm text-zinc-200 focus:border-emerald-500/50 outline-none"
                    ...
                    <div>
                      <label className="text-[9px] font-bold text-zinc-500 uppercase block mb-1">Storage</label>
                      <select className="w-full bg-zinc-950/60 border border-zinc-800/60 rounded-lg px-2 py-2 text-sm text-zinc-200 focus:border-emerald-500/50 outline-none"
        ...
        <DialogContent className="max-w-lg bg-zinc-950 border-zinc-800 text-white">
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="flex gap-1 bg-zinc-900 rounded-lg p-0.5">
              {[
                ...
                <button key={m.key}
                  ...
                  className={`flex-1 py-2 px-3 rounded-md text-xs font-medium transition ${aiImportMode === m.key ? 'bg-purple-600 text-white' : 'text-zinc-500 hover:text-zinc-300'
                    }`}>
                  {m.icon} {m.label}
                </button>
              ))}
            ...
            {(aiImportMode === 'photo' || aiImportMode === 'pdf') && (
              <div className="border-2 border-dashed border-zinc-700 rounded-xl p-8 text-center hover:border-purple-500/50 transition cursor-pointer"
                onClick={() => document.getElementById('ai-import-file')?.click()}>
                <FileUp className="h-10 w-10 text-zinc-600 mx-auto mb-2" />
      ...
      <Dialog open={showMediaLibrary} onOpenChange={setShowMediaLibrary}>
        <DialogContent className="max-w-2xl bg-zinc-950 border-zinc-800 text-white">
          ...
          <div className="space-y-4 py-2">
            ...
            {/* Upload Zone */}
            <div className="border-2 border-dashed border-zinc-700 rounded-xl p-6 text-center hover:border-blue-500/50 transition cursor-pointer"
              onClick={() => document.getElementById('media-upload-input')?.click()}>
              <Upload className="h-8 w-8 text-zinc-600 mx-auto mb-2" />
              <p className="text-xs text-zinc-500">Drag & drop or click to upload photos/videos</p>
            ...
            <div className="grid grid-cols-4 gap-2">
              {['ğŸ• Pizza Margherita', 'ğŸ¥— Caesar Salad', 'ğŸ° Tiramisu', 'ğŸ Carbonara',
                'ğŸ¥© Wagyu Steak', 'ğŸ£ Salmon Sashimi', 'ğŸ² Ramen Bowl', 'ğŸ§ Cupcake'].map((item, i) => (
                  <div key={i} className="bg-zinc-900 rounded-lg p-3 text-center border border-zinc-800 hover:border-blue-500/30 cursor-pointer transition">
                    <div className="text-2xl mb-1">{item.split(' ')[0]}</div>
                    <p className="text-[10px] text-zinc-500 truncate">{item.split(' ').slice(1).join(' ')}</p>
                  </div>
                ))}
            </div>
