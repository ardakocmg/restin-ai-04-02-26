    ...
    return (
      <PageContainer title="Tasks Board" description="Operational Task Management">
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground mt-2">Please log in to access your tasks.</p>
        </div>
      </PageContainer>
    );
  }
      ...
      actions={
        <div className="flex items-center gap-2">
          <Button
            ...
            className={showMyTasks ? "bg-blue-600 hover:bg-blue-500" : "border-border text-secondary-foreground"}
          >
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 h-[calc(100vh-280px)] overflow-x-auto">
          {board?.columns?.map((column) => (
            <div key={column.key} className="bg-card border border-border p-4 rounded-xl flex flex-col h-full">
              ...
              <div className="space-y-3 flex-1 overflow-y-auto pr-1">
                {filteredCards.filter(c => c.status === column.key).map((task) => {
                  ...
                  return (
                    <Card
                      key={task.id}
                      className={`cursor-pointer bg-background transition-all hover:shadow-lg group ${isMyTask
                          ? 'border-blue-500/30 hover:border-blue-500/50 ring-1 ring-blue-500/10'
                          : 'border-border hover:border-blue-500/50'
                        }`}
                    >
                      <CardContent className="p-3">
                        <p className="font-medium text-foreground text-sm mb-2 group-hover:text-blue-400 transition-colors">{task.title}</p>
                        <div className="flex items-center justify-between">
                          {task.checklist_items && task.checklist_items.length > 0 && (
                            <div className="flex items-center gap-1 text-[10px] text-muted-foreground bg-card w-fit px-2 py-1 rounded">
                ...
                <Button variant="ghost" className="w-full text-muted-foreground text-xs border border-dashed border-border hover:border-border hover:bg-card">
