                ...
                <Tabs defaultValue="steps">
                  ...
                  <TabsContent value="steps" className="space-y-2 mt-4">
                    {activeRun.steps?.map((step) => (
                      ...
                      >
                        {step.status === 'SUCCESS' ? (
                          <CheckCircle className="h-5 w-5" style={{ color: getStepColor(step.status) }} /> /* keep-inline */
                        ) : step.status === 'FAILED' ? (
                          <XCircle className="h-5 w-5" style={{ color: getStepColor(step.status) }} /> /* keep-inline */
                        ) : (
                          <AlertTriangle className="h-5 w-5" style={{ color: getStepColor(step.status) }} /> /* keep-inline */
                        )}
                  ...
                  <TabsContent value="response">
                    <pre
                      ...
                      style={{ backgroundColor: '#27272A', color: '#F5F5F7', maxHeight: '400px' }} /* keep-inline */
                  ...
                  <TabsContent value="trace">
                    <div className="space-y-2" data-testid="testpanel-trace-summary">
                      <div className="flex justify-between">
                        <span style={{ color: '#A1A1AA' }}>Status Code:</span> /* keep-inline */
                        <span style={{ color: '#F5F5F7' }}>{activeRun.status_code}</span> /* keep-inline */
                      </div>
                      <div className="flex justify-between">
                        <span style={{ color: '#A1A1AA' }}>Latency:</span> /* keep-inline */
                        <span style={{ color: '#F5F5F7' }}>{activeRun.trace?.latency_ms || '--'} ms</span> /* keep-inline */
                      </div>
                      <div className="flex justify-between">
                        <span style={{ color: '#A1A1AA' }}>Request ID:</span> /* keep-inline */
                        <span style={{ color: '#F5F5F7' }}>{activeRun.trace?.request_id || '-'}</span> /* keep-inline */
                      </div>
                      <div className="flex justify-between">
                        <span style={{ color: '#A1A1AA' }}>Created:</span> /* keep-inline */
                        <span style={{ color: '#F5F5F7' }}>{new Date(activeRun.created_at).toLocaleString()}</span> /* keep-inline */
                      </div>
                    </div>
                  </TabsContent>
