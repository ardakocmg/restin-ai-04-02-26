      ...
      <div className="grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-6">
        ...
        <Card data-testid="testpanel-request-card">
          ...
          <CardContent className="space-y-4">
            <div>
              <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2 block">Method</label>
            ...
            <div>
              <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2 block">Path</label>
              <Input value={path} onChange={(e) => setPath(e.target.value)} data-testid="testpanel-path-input" />
            </div>
            <div>
              <label className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-2 block">Request Body (JSON)</label>
              <textarea
            ...
            <Button
              onClick={handleRun}
              disabled={loading}
              className="w-full bg-zinc-900 border-white/10 text-zinc-100 hover:bg-zinc-800 transition-all shadow-lg font-bold uppercase tracking-tight"
        ...
        <div className="space-y-6">
          <Card data-testid="testpanel-runs-card">
            ...
            <CardContent className="space-y-2">
              {runsLoading && <p className="text-sm text-zinc-400">Loading runs...</p>}
              ...
              {!runsLoading && runs.map((run) => (
                <button
                  ...
                  className={`w-full text-left p-3 rounded-lg border transition-all ${selectedRunId === run.id ? 'border-red-500 bg-red-500/15 ring-1 ring-red-500/50' : 'border-white/5 bg-zinc-900/60 hover:bg-zinc-800'}`}
                  data-testid={`testpanel-run-${run.display_id}`}
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm text-zinc-200 font-semibold">{run.display_id}</p>
                      <p className="text-xs text-zinc-500">{run.target?.method} {run.target?.path}</p>
                    </div>
