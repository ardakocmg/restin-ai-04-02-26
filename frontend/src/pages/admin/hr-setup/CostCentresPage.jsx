...
export default function CostCentresPage() {
  ...
  const fetchCostCentres = async () => {
    try {
      const response = await api.get('/employee-setup/cost-centres');
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-foreground">Cost Centres</h1>
        <Button><Plus className="h-4 w-4 mr-2" /> Add Cost Centre</Button>
      </div>
            ...
            <tbody>
              {costCentres.map(cc => (
                <tr key={cc.id} className="border-b hover:bg-slate-50">
                  <td className="p-3">{cc.cost_centre_name}</td>
                  <td className="p-3">{cc.cost_centre_code}</td>
