...
export default function EmploymentTypesPage() {
  ...
  const fetchTypes = async () => {
    try {
      const response = await api.get('/employee-setup/employment-types');
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-foreground">Employment Types</h1>
            ...
            <tbody>
              {types.map(type => (
                <tr key={type.id} className="border-b hover:bg-slate-50">
                  <td className="p-3">{type.type_name}</td>
                  <td className="p-3">{type.type_code}</td>
