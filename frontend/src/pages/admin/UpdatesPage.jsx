...
export default function UpdatesPage() {
  const { user } = useAuth();
  useAuditLog('UPDATES_VIEWED', { resource: 'updates-page' });
          ...
          <CardContent className="p-6">
            <Tabs value={viewMode} onValueChange={setViewMode}>
              ...
              <TabsContent value={viewMode} className="mt-4">
                {loadingReleases && <div className="text-sm text-zinc-400">Loading releases…</div>}
                <div className="space-y-4">
                  {releases.map((release) => (
                    <div key={release.id} className="rounded-xl border border-white/10 bg-black/30 p-4" data-testid={`updates-release-${release.version_code}`}>
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-semibold text-indigo-50">{release.version_code}</p>
                        <span className="text-xs text-zinc-400">{new Date(release.created_at).toLocaleString()}</span>
                      </div>
                      <div className="mt-3 space-y-2 text-xs text-zinc-300">
                        {Object.entries(release.notes || {}).map(([section, items]) => (
                          <div key={section}>
                            <p className="text-zinc-400 uppercase tracking-wide">{section}</p>
