...
export default function TrainingStartingReport() {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get('reporting/training-starting-soon');
  ...
  return (
    <div className="p-6 bg-slate-50 min-h-screen">
      <h1 className="text-3xl font-bold text-foreground mb-6">Training & Certifications Starting Soon</h1>
      <Card>
            ...
            <tbody>
              {trainings.map(training => (
                <tr key={training.id} className="border-b hover:bg-slate-50">
                  ...
                  <td className="p-3"><span className="px-2 py-1 bg-blue-100 text-blue-800 rounded text-xs">{training.status}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
