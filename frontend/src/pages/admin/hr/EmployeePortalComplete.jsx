...
export default function EmployeePortalComplete() {
  ...
  const renderCalendar = (monthOffset = 0) => {
    ...
    return (
      <div className="mb-4">
        <h4 className="text-sm font-semibold text-zinc-300 mb-2">{monthNames[month]} {year}</h4>
        <div className="grid grid-cols-7 gap-1 mb-1">
          {dayNames.map(day => (
            <div key={day} className="text-[10px] font-bold text-zinc-600 text-center py-1 uppercase tracking-wider">{day}</div>
          ))}
        </div>
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }, (_, i) => {
            ...
            return (
              <button
                key={date}
                className={`p-1.5 text-center rounded-lg text-sm font-medium transition-all ${isToday ? 'bg-indigo-600 text-white ring-2 ring-indigo-400/50 shadow-lg shadow-indigo-600/20' :
  ...
  return (
    <div className="p-6 bg-black min-h-screen text-zinc-100">
      {/* ─── Header ───────────────────────────────── */}
      <div className="mb-6 flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">Welcome back, {data.my_profile.name.split(' ')[0]} 👋</p>
        </div>
        <div className="flex items-center gap-3">
          {data.pending_requests > 0 && (
            <button className="relative p-2 bg-zinc-900 rounded-lg border border-zinc-800 hover:bg-zinc-800 transition-colors">
              <Bell className="w-5 h-5 text-zinc-400" />
              <span className="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{data.pending_requests}</span>
            </button>
          )}
      ...
      <div className="grid gap-6 lg:grid-cols-12">
          ...
          <Card className="bg-zinc-900/50 border-zinc-800 overflow-hidden">
                ...
                <div className="w-20 h-20 rounded-full overflow-hidden border-4 border-zinc-900 shadow-xl">
                  {data.my_profile.profile_photo ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center">
                      <span className="text-2xl font-bold text-white">{getInitials(data.my_profile.name)}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            <CardContent className="pt-14 pb-5 text-center">
              ...
              {data.my_profile.employee_code && (
                <p className="text-[10px] text-zinc-600 font-mono mt-1">{data.my_profile.employee_code}</p>
              )}
              ...
              <div className="mt-4 space-y-2 text-left">
                ...
                {data.my_profile.reporting_to && (
                  <div className="flex items-center gap-2 text-xs text-zinc-400">
                    ...
                    Reports to: <span className="text-zinc-300">{data.my_profile.reporting_to}</span>
                  </div>
                )}
              </div>
            ...
            <CardContent>
              ...
              <div className="flex gap-3 mb-4 flex-wrap">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-zinc-500">Holiday</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-2.5 h-2.5 rounded-full bg-purple-500" />
                  <span className="text-[10px] text-zinc-500">Remote</span>
                </div>
                <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center justify-between mb-4">
                <button onClick={() => setCurrentMonth(m => m > 0 ? m - 1 : 11)} className="p-1.5 hover:bg-zinc-800 rounded-lg transition-colors">
                  <ChevronLeft className="h-4 w-4 text-zinc-400" />
                </button>
                <div className="text-sm font-semibold text-zinc-300">
                  {new Date(currentYear, currentMonth).toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}
                </div>
                <button onClick={() => setCurrentMonth(m => m < 11 ? m + 1 : 0)} className="p-1.5 hover:bg-zinc-800 rounded-lg transition-colors">
              ...
              <div className="space-y-2">
                {data.payslips?.map((payslip, idx) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 bg-zinc-950/50 border border-zinc-800 rounded-lg hover:border-zinc-700 hover:bg-zinc-900/50 transition-all"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        <p className="text-sm font-medium text-zinc-200">{payslip.month}</p>
                        {payslip.net && <p className="text-[11px] text-zinc-500">Net: {payslip.net}</p>}
                      </div>
                    </div>
                    ...
                    >
                      <div className="relative w-28 h-28 mb-2">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center">
                          <span className="text-2xl font-black text-white group-hover:text-indigo-400 transition-colors">{balance.hours_left}</span>
                          <span className="text-[9px] text-zinc-500 uppercase font-bold tracking-widest">hrs left</span>
                        </div>
                      </div>
                      <p className="text-xs font-semibold text-zinc-400 text-center leading-tight">{balance.leave_type}</p>
                    </button>
                  );
                })}
              </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  {data.upcoming_shifts.map((shift, idx) => (
                    <div key={idx} className={`p-3 rounded-lg border transition-all ${idx === 0 ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-zinc-950/50 border-zinc-800'}`}>
                      <div className="flex justify-between items-start">
                        <div>
                          <p className={`text-sm font-semibold ${idx === 0 ? 'text-indigo-400' : 'text-zinc-300'}`}>{shift.date}</p>
                          <p className="text-xs text-zinc-500 mt-0.5">{shift.time}</p>
                        </div>
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-zinc-800 text-zinc-400 border border-zinc-700">{shift.role}</span>
                      </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  {data.team_members.map((member, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-2.5 hover:bg-zinc-800/30 rounded-lg transition-colors">
                      <div className="relative">
                        <div className="w-9 h-9 rounded-full bg-gradient-to-br from-zinc-700 to-zinc-800 flex items-center justify-center border border-zinc-700">
                          <span className="text-xs font-bold text-zinc-400">{getInitials(member.name)}</span>
                        </div>
        ...
        <div className="lg:col-span-4 space-y-6">
            ...
            <CardContent>
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto custom-scrollbar">
                {data.out_of_office?.map((staff, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-3 p-2.5 hover:bg-zinc-800/30 rounded-lg cursor-pointer transition-all border border-transparent hover:border-zinc-800"
                    onClick={() => toast.info(`${staff.name} — ${staff.leave_type}: ${staff.start_time}${staff.end_time ? ` - ${staff.end_time}` : ''} ${staff.duration || ''}`)}
                  >
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-zinc-700 to-zinc-800 flex items-center justify-center flex-shrink-0 border border-zinc-700">
                      <span className="text-xs font-bold text-zinc-400">{getInitials(staff.name)}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p className="text-sm font-medium text-zinc-200 truncate">{staff.name}</p>
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-3">
                <button
                  ...
                  className="text-center p-4 bg-zinc-950/50 border border-zinc-800 rounded-xl hover:border-zinc-700 transition-all"
                >
                  <p className="text-3xl font-black text-white">{data.leave_metrics?.on_leave_today || 0}</p>
                  <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mt-1">Leave Today</p>
                </button>
                <button
                  onClick={() => toast.info(`${data.leave_metrics?.on_leave_tomorrow || 0} employees on leave tomorrow`)}
                  className="text-center p-4 bg-zinc-950/50 border border-zinc-800 rounded-xl hover:border-zinc-700 transition-all"
                >
                  <p className="text-3xl font-black text-white">{data.leave_metrics?.on_leave_tomorrow || 0}</p>
                  <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mt-1">Tomorrow</p>
                </button>
                <button
                ...
                >
                  ...
                  <p className="text-[10px] text-zinc-500 uppercase font-bold tracking-widest mt-1">Sick Leave</p>
                </button>
              </div>
            </CardContent>
          </Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {data.announcements.map((ann, idx) => (
                    <div key={idx} className={`p-3 rounded-lg border transition-all cursor-pointer hover:bg-zinc-800/30 ${ann.priority === 'high' ? 'border-amber-500/20 bg-amber-500/5' : 'border-zinc-800 bg-zinc-950/30'}`}>
                      <div className="flex justify-between items-start">
                      ...
                      <p className="text-[11px] text-zinc-600 mt-1">{ann.date}</p>
                    </div>
                  ))}
                </div>
              </CardContent>
          ...
          <Card className="bg-zinc-900/50 border-zinc-800">
            ...
            <CardContent>
              <p className="text-sm text-zinc-500">Team leave overview will appear here when data syncs.</p>
            </CardContent>
          </Card>
        </div>
      </div>
      ...
      <Dialog open={showLeaveDialog} onOpenChange={setShowLeaveDialog}>
        <DialogContent className="bg-zinc-900 border-zinc-800">
          <DialogHeader>
            <DialogTitle className="text-white">{selectedLeave?.leave_type} Balance</DialogTitle>
            <DialogDescription className="text-zinc-500">Detailed breakdown of your leave balance</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-zinc-950 border border-zinc-800 rounded-lg">
                <p className="text-xs text-zinc-500 uppercase font-bold tracking-wider">Total Allocation</p>
                <p className="text-2xl font-black text-white mt-1">{selectedLeave?.total_hours}h</p>
              </div>
              <div className="p-4 bg-indigo-500/5 border border-indigo-500/20 rounded-lg">
                <p className="text-xs text-zinc-500 uppercase font-bold tracking-wider">Remaining</p>
                <p className="text-2xl font-black text-indigo-400 mt-1">{selectedLeave?.hours_left}h</p>
              </div>
            </div>
            <div className="p-4 bg-zinc-950 border border-zinc-800 rounded-lg">
              <p className="text-xs text-zinc-500 uppercase font-bold tracking-wider">Used</p>
            ...
            {selectedLeave?.is_segmented && selectedLeave?.segments && (
              <div className="border-t border-zinc-800 pt-4">
                <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-3">Balance Breakdown</p>
                {selectedLeave.segments.map((seg, i) => (
                  <div key={i} className="flex justify-between items-center py-2 border-b border-zinc-800/50 last:border-0">
                    <span className="text-sm text-zinc-400 capitalize">{seg.type}</span>
                    <span className="text-sm font-bold text-white">{seg.hours}h</span>
                  </div>
                ))}
              </div>
            )}
