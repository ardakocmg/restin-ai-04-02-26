        ...
        <div className="min-h-screen bg-[#0A0A0B] text-zinc-100 flex flex-col">
            ...
            <div className="absolute top-8 left-8 right-8 z-20 flex items-start justify-between pointer-events-none">
                <div className="flex flex-col gap-4 pointer-events-auto">
                    ...
                    <div className="flex items-center gap-3">
                        <Button
                            ...
                            className="bg-black/40 backdrop-blur-md border-white/10 rounded-xl w-12 h-12"
                        >
                            <ChevronLeft className="w-5 h-5" />
                        </Button>
                        <div className="bg-black/40 backdrop-blur-md border border-white/10 rounded-xl px-6 py-3">
                ...
                <div className="flex flex-col gap-2 pointer-events-auto">
                    ...
                    <div className="flex items-center gap-2 bg-black/40 backdrop-blur-md border border-white/10 rounded-xl p-1">
                        <Button variant="ghost" size="icon" className="h-10 w-10 text-zinc-400"><Layers className="w-4 h-4" /></Button>
                        <Button variant="ghost" size="icon" className="h-10 w-10 text-zinc-400"><Filter className="w-4 h-4" /></Button>
                        <Button variant="ghost" size="icon" className="h-10 w-10 text-zinc-400"><Calendar className="w-4 h-4" /></Button>
                    </div>
                </div>
            </div>
            ...
            <div className="absolute top-48 bottom-8 left-8 w-80 z-20 pointer-events-none">
                <Card className="h-full bg-black/40 backdrop-blur-xl border-white/5 shadow-2xl overflow-hidden pointer-events-auto flex flex-col">
                    <div className="p-4 border-b border-white/5 bg-white/5">
                        <div className="relative">
                            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-500" />
                            <input
                                className="w-full bg-black/50 border border-white/10 rounded-lg pl-10 pr-4 py-2 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:border-blue-500/50"
                            ...
                            <div key={m.id} className="p-3 bg-zinc-900/50 border border-white/5 rounded-xl hover:bg-zinc-800/50 transition-colors cursor-pointer group">
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex flex-col">
                                        <span className="text-[10px] font-black text-zinc-100 uppercase tracking-tight leading-none truncate w-40">{m.name}</span>
                                        <span className="text-[8px] font-bold text-zinc-500 uppercase mt-1 flex items-center gap-1">
            ...
            <div className="flex-1 relative bg-[#0d0d0f] overflow-hidden">
                ...
                {mockMarkers.map((m, i) => (
                    ...
                    >
                        <div className="relative flex flex-col items-center">
                            {/* Marker Popup */}
                            <div className="absolute bottom-full mb-3 hidden group-hover:block w-48 bg-black/80 backdrop-blur-md border border-white/10 p-3 rounded-xl shadow-2xl animate-in fade-in slide-in-from-bottom-2 duration-300">
                                <p className="text-[10px] font-black text-white uppercase truncate">{m.name}</p>
                                <p className="text-[8px] text-zinc-400 font-bold uppercase mt-1">{m.loc} • {m.time}</p>
                            </div>
                            {/* Pin */}
                            ...
                            <div className="mt-2 text-[8px] font-black text-zinc-400 uppercase tracking-tighter bg-black/60 px-2 py-0.5 rounded backdrop-blur-sm border border-white/5">
                                {m.name.split(' ')[0]}
                            </div>
                        </div>
                    </div>
                ...
                {/* Legend Overlay */}
                <div className="absolute bottom-8 right-8 bg-black/40 backdrop-blur-md border border-white/10 p-4 rounded-xl space-y-3">
                    <div className="flex items-center gap-3">
                        <div className="w-3 h-3 rounded-full bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]" />
                        <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">Mobile App Clocking</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="w-3 h-3 rounded-full bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]" />
                        <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-widest">Fixed Terminal Clocking</span>
                    </div>
                </div>
            </div>
