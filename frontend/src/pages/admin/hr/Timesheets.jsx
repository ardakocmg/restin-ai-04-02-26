...
const STATUS_COLORS = {
  ...
  locked: "bg-zinc-500/20 text-zinc-400 border-zinc-500/50"
};
      ...
      <div className="min-h-screen bg-zinc-950 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            ...
            <p className="text-zinc-400">Time entry and approvals</p>
          </div>
    ...
    <div className="min-h-screen bg-zinc-950 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-zinc-400">Time entry and approval</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-4">
            {timesheets.map((ts) => (
              <div key={ts.id} className="bg-zinc-900 rounded-lg p-6 border border-indigo-500/20">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-sm text-zinc-500">{ts.display_id}</p>
                    <h3 className="text-lg font-bold text-white">Period: {new Date(ts.period_start).toLocaleDateString()} - {new Date(ts.period_end).toLocaleDateString()}</h3>
                  </div>
                ...
                <div className="flex items-center gap-4 text-sm mb-4">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-white">{(ts.total_hours || 0).toFixed(2)}</p>
                  </div>
                  <div>
                    <p className="text-zinc-500">Entries</p>
                    <p className="text-xl text-white">{ts.entries?.length || 0}</p>
                  </div>
                </div>
