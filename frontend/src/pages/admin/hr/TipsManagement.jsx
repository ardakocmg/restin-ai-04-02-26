    ...
    return (
      <div className="min-h-screen bg-zinc-950 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            <h1 className="text-3xl font-bold text-white">Tips Management</h1>
            <p className="text-zinc-400">Tips pooling and distribution</p>
          </div>
            ...
            {tipsPools.map((pool) => (
              <div key={pool.id} className="bg-white dark:bg-slate-900 rounded-lg p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <p className="text-sm text-zinc-500">{pool.display_id}</p>
                    <h3 className="text-lg font-bold text-white">{new Date(pool.date).toLocaleDateString()}</h3>
                  </div>
