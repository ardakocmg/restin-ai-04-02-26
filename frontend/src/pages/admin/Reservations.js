...
const STATUS_COLORS = {
  ...
  completed: 'bg-zinc-600/20 text-zinc-300 border border-zinc-500/30 font-black uppercase text-[10px]',
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <Card className="bg-zinc-950 border-white/5 shadow-xl border-b-2" style={{ borderBottomColor: '#2563eb' }}>
        ...
        <Card className="bg-zinc-950 border-white/5 shadow-xl border-b-2" style={{ borderBottomColor: '#ef4444' }}>
          <CardContent className="p-4">
            <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">No-Show Rate</p>
            <p className="text-2xl font-black text-red-500">{analytics?.no_show_rate || 0}%</p>
          </CardContent>
        </Card>
        <Card className="bg-zinc-950 border-white/5 shadow-xl border-b-2" style={{ borderBottomColor: '#3b82f6' }}>
          <CardContent className="p-4">
            <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Top Channel</p>
            ...
            columns={[
              {
                key: 'guest_name',
                label: 'Guest',
                render: (row) => <span className="text-sm font-black text-white uppercase tracking-tight">{row.guest_name}</span>
              },
              {
                key: 'date',
                label: 'Date',
                render: (row) => <span className="text-sm font-bold text-zinc-300">{row.date}</span>
              },
              {
                key: 'time',
                label: 'Time',
              ...
              { key: 'party_size', label: 'Guests', render: (row) => <span className="text-xs font-bold text-zinc-400">{row.party_size || row.guest_count} pax</span> },
              {
                key: 'status',
                label: 'Status',
                render: (row) => (
              ...
              { key: 'source', label: 'Source', render: (row) => <span className="text-[10px] font-bold text-zinc-500 uppercase">{row.source || row.channel}</span> },
              {
                key: 'actions',
                label: '',
                render: (row) => (
