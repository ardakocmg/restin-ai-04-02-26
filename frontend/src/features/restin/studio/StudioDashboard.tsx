        ...
        <div className="flex flex-col gap-8 animate-in slide-in-from-right duration-700">
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="p-4 bg-card/50 rounded-2xl border border-border/80 flex items-center gap-3">
                        <div className="flex flex-col text-right">
                            <span className="text-[10px] font-black text-muted-foreground uppercase tracking-widest">Assets</span>
                            <span className="text-xs font-black text-foreground">{assets.length} items</span>
                        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                ...
                <div className="lg:col-span-1 space-y-4">
                    <Card className="bg-gradient-to-b from-zinc-900 to-black border-border p-6 overflow-hidden relative group">
                        <div className="absolute -bottom-4 -right-4 text-foreground/5 group-hover:text-red-600/10 transition-colors">
                            <Sparkles size={80} />
                        </div>
                        <h3 className="text-xs font-black text-muted-foreground uppercase tracking-[0.2em] mb-4">Magic Tools</h3>
                        <div className="space-y-2 relative z-10">
                            <Button
                                onClick={() => generateMutation.mutate()}
                                disabled={generateMutation.isPending}
                                className="w-full justify-start h-14 bg-background/50 border border-border hover:border-red-500/30 text-secondary-foreground font-bold px-4 rounded-xl gap-3">
                                <Camera size={20} className="text-red-500" /> {generateMutation.isPending ? 'Dreaming...' : 'Generate Photo'}
                            </Button>
                            <Button className="w-full justify-start h-14 bg-background/50 border border-border hover:border-red-500/30 text-secondary-foreground font-bold px-4 rounded-xl gap-3">
                    ...
                    <Card className="bg-card/20 border-border p-6">
                        <h3 className="text-xs font-black text-muted-foreground uppercase tracking-[0.2em] mb-4">Pipeline Stats</h3>
                        <div className="space-y-4">
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-muted-foreground font-bold uppercase">Render Time</span>
                                <span className="text-foreground font-black italic">Avg 12s</span>
                            </div>
                            <div className="w-full h-1 bg-secondary rounded-full overflow-hidden">
                                <div className="w-3/4 h-full bg-red-600 shadow-[0_0_10px_#dc2626]"></div>
                            </div>
                            <div className="flex justify-between items-center text-xs">
                                <span className="text-muted-foreground font-bold uppercase">Token Cost</span>
                ...
                <div className="lg:col-span-3">
                        ...
                        {['all', 'photos', 'videos', 'menus'].map(filter => (
                            <button
                                ...
                                className={cn(
                                    ...
                                    activeFilter === filter
                                        ? "bg-red-600 text-foreground border-red-500 shadow-lg"
                                        : "bg-transparent text-muted-foreground border-border hover:text-secondary-foreground"
                                )}
                            >
                                {filter}
                            </button>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                        {isLoading ? (
                            ...
                            <Card key={asset.id || i} className="bg-card/40 border-border overflow-hidden group hover:scale-[1.02] transition-all duration-300 shadow-2xl">
                                ...
                                <div className="p-5">
                                    <div className="flex justify-between items-center mb-2">
                                        <h4 className="text-sm font-black text-foreground italic truncate">{asset.prompt || 'Untitled'}</h4>
