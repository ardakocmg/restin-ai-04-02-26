    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      <div className="relative z-10 w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-2xl card-dark">
        ...
        <div className="sticky top-0 z-10 bg-card border-b border-border p-6">
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <h2 className="text-2xl font-heading mb-1" style={{ color: '#F5F5F7' }}> /* keep-inline */
                {item.name}
              </h2>
            ...
            <button
              ...
              style={{ color: '#71717A' }} /* keep-inline */
            >
              <X className="w-6 h-6" />
            </button>
          </div>
        ...
        <div className="overflow-y-auto max-h-[calc(90vh-220px)] p-6 space-y-6">
          ...
          {modifierGroups.map((group) => (
            <div key={group.id} className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="font-semibold" style={{ color: '#F5F5F7' }}> /* keep-inline */
                  {group.name}
                </h3>
                {group.required && (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span style={{ color: isSelected ? '#F5F5F7' : '#D4D4D8' }}> /* keep-inline */
                          {option.name}
                        </span>
                      </div>
          ...
          <div className="space-y-2">
            <label className="block font-semibold" style={{ color: '#F5F5F7' }}> /* keep-inline */
              Special Instructions
            </label>
            <textarea
        ...
        <div className="sticky bottom-0 z-10 bg-card border-t border-border p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <span className="font-semibold" style={{ color: '#F5F5F7' }}>Quantity</span> /* keep-inline */
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={quantity <= 1}
              >
                <Minus className="w-4 h-4" style={{ color: '#D4D4D8' }} /> /* keep-inline */
              </button>
              <span className="text-xl font-bold w-12 text-center" style={{ color: '#F5F5F7' }}> /* keep-inline */
                {quantity}
              </span>
              <button
                ...
                className="w-10 h-10 rounded-lg bg-secondary hover:bg-secondary/80 border border-border flex items-center justify-center transition-colors"
              >
                <Plus className="w-4 h-4" style={{ color: '#D4D4D8' }} /> /* keep-inline */
              </button>
            </div>
          </div>
