...
export default function MergeTableModal({ currentTableId, activeTables, onClose, onMerge }) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 animate-fade-in">
            ...
            <div className="relative z-10 w-full max-w-lg bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl overflow-hidden">
                {/* Header */}
                <div className="flex items-center justify-between p-6 border-b border-white/10 bg-zinc-900/50">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl font-heading text-white">Merge Tables</h2>
                            <p className="text-sm text-zinc-400">Merge with current table ({currentTableId})</p>
                        </div>
                    </div>
                ...
                {/* Content */}
                <div className="p-6">
                    <div className="text-sm text-zinc-400 mb-4">Select a table to merge into this order:</div>
                        ...
                        {tables.map(table => (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center ${selectedTable === table.id ? 'bg-purple-500 text-white' : 'bg-zinc-700 text-zinc-400'
                                        }`}>
                                        <Table2 className="w-5 h-5" />
                                    </div>
                                    <div className="text-left">
                                        <div className="text-white font-medium">{table.name}</div>
                                        <div className="text-xs text-zinc-500">{table.guests} Guests</div>
                                    </div>
                                </div>

                                <div className="text-right">
                                    <div className="font-bold text-white">€{table.total.toFixed(2)}</div>
                ...
                <div className="p-6 border-t border-white/10 bg-zinc-900/50">
                    <button
                        ...
                        className="w-full h-12 bg-purple-600 hover:bg-purple-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all"
                    >
                        Confirm Merge
                    </button>
                </div>
