    ...
    return (
        <div className="h-screen flex bg-zinc-950 overflow-hidden">
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
                ...
                <div className="h-12 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between px-3 shrink-0">
                    <div className="flex items-center gap-3">
                        <h1 className="text-white font-bold text-sm tracking-tight">RESTIN.AI</h1>
                        <span className="text-zinc-500 text-xs">|</span>
                        <span className="text-zinc-400 text-xs">{venue?.name}</span>
                        {selectedTable && (
                            <>
                ...
                <div className="h-14 bg-zinc-900/50 border-b border-zinc-800 flex items-center px-2 gap-1.5 overflow-x-auto shrink-0">
                    {categories.map((cat) => {
                ...
                <div className="flex-1 p-3 overflow-auto">
                    ...
                    ${!item.image && !item.color ? 'bg-zinc-800 border border-zinc-700/50 hover:border-zinc-600' : ''}
                  `}
                                >
                                    ...
                                    <span className={`z-10 text-lg font-bold mt-auto self-end ${hasImage || item.color ? 'text-white' : 'text-white'}`}>
            ...
            <div className="w-80 bg-zinc-900 border-l border-zinc-700/50 flex flex-col">
                {/* Order Header */}
                    ...
                    <div className="p-2">
                        ...
                        {orderItems.length === 0 ? (
                            <div className="text-center py-8">
                                ...
                                <p className="text-zinc-500 text-sm">Tap items to order</p>
                            </div>
                        ) : (
                            <div className="space-y-1">
                                {orderItems.map((item, index) => (
                                    <div
                                        key={index}
                                        className="p-2 rounded-lg bg-zinc-800/60 hover:bg-zinc-800 transition-colors flex items-center gap-2"
                                    >
                                        {/* Quantity Controls */}
                                        <div className="flex items-center gap-0.5 shrink-0">
                                            <button
                                                onClick={() => onUpdateItemQuantity(index, -1)}
                                                className="w-6 h-6 rounded bg-zinc-700 hover:bg-zinc-600 flex items-center justify-center text-zinc-300"
                                            >
                                                <Minus className="w-3 h-3" />
                                            </button>
                                            <span className="text-white font-mono text-xs w-5 text-center font-bold">
