    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <p style={{ color: '#A1A1AA' }}>Combine multiple tables into one order</p>
        </DialogHeader>

        <div className="space-y-4">
          {/* Current Table */}
          <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(229, 57, 53, 0.1)', border: '1px solid rgba(229, 57, 53, 0.3)' }}>
            <p className="text-sm mb-2" style={{ color: '#A1A1AA' }}>Primary Table:</p>
            <div className="flex items-center justify-between">
              <div>
                <p className="font-bold" style={{ color: '#F5F5F7' }}>{currentTable?.name}</p>
                <p className="text-sm" style={{ color: '#71717A' }}>Current order: €{currentTable?.order_total?.toFixed(2) || '0.00'}</p>
              </div>
              <Badge>Primary</Badge>
            </div>
          </div>
          ...
          {/* Available Tables */}
          <div>
            <p className="text-sm mb-3" style={{ color: '#D4D4D8' }}>Select tables to merge:</p>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {availableTables?.filter(t => t.id !== currentTable?.id).map(table => {
                ...
                return (
                  ...
                  >
                    <p className="font-medium" style={{ color: '#F5F5F7' }}>{table.name}</p>
          ...
          {selectedTables.length > 0 && (
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(74, 222, 128, 0.1)', border: '1px solid rgba(74, 222, 128, 0.3)' }}>
              <p className="text-sm mb-2" style={{ color: '#86EFAC' }}>Merge Preview:</p>
              <div className="flex items-center gap-2">
                <Badge>{currentTable?.name}</Badge>
                {selectedTables.map(t => (
                  <React.Fragment key={t.id}>
                    <ArrowRight className="h-4 w-4" style={{ color: '#71717A' }} />
