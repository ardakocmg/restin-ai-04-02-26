    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          ...
          <p style={{ color: '#A1A1AA' }}>Total: €{totalAmount.toFixed(2)}</p> /* keep-inline */ /* keep-inline */ /* keep-inline */
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          {splitType === 'equal' && (
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)' }}> /* keep-inline */ /* keep-inline */ /* keep-inline */
              <label className="block mb-2" style={{ color: '#D4D4D8' }}>Number of People</label> /* keep-inline */ /* keep-inline */ /* keep-inline */
              <div className="flex items-center gap-4">
                <Button onClick={() => setNumberOfSplits(Math.max(2, numberOfSplits - 1))}>-</Button>
                <span className="text-2xl font-bold" style={{ color: '#F5F5F7' }}>{numberOfSplits}</span> /* keep-inline */ /* keep-inline */ /* keep-inline */
                <Button onClick={() => setNumberOfSplits(Math.min(10, numberOfSplits + 1))}>+</Button>
              </div>
              <div className="mt-4">
                <p style={{ color: '#A1A1AA' }}>Amount per person:</p> /* keep-inline */ /* keep-inline */ /* keep-inline */
                <p className="text-2xl font-bold" style={{ color: '#E53935' }}>€{(totalAmount / numberOfSplits).toFixed(2)}</p> /* keep-inline */ /* keep-inline */ /* keep-inline */
              </div>
            </div>
          )}
          ...
          {splitType === 'by_item' && (
            <div className="space-y-3">
              <p className="text-sm" style={{ color: '#A1A1AA' }}>Select items for each person</p> /* keep-inline */ /* keep-inline */ /* keep-inline */
              {items.map((item, idx) => (
                <div 
                  key={item.id || idx}
                ...
                >
                  <div className="flex-1">
                    <p style={{ color: '#F5F5F7' }}>{item.name}</p> /* keep-inline */ /* keep-inline */ /* keep-inline */
                    <p className="text-sm" style={{ color: '#71717A' }}>€{item.line_total?.toFixed(2)}</p> /* keep-inline */ /* keep-inline */ /* keep-inline */
                  </div>
