        ...
        <>
            ...
            {isOpen && (
                <motion.div
                    ...
                    dragMomentum={false}
                    dragElastic={0.1}
                    style={{ touchAction: "none" }} /* keep-inline */
                >
                    {/* Header */}
                    <div className="rin-header" style={{ cursor: "move" }}> /* keep-inline */
                                    ...
                                    <div className="wk-member-chips">
                                        {CHANNEL_MEMBERS.map(m => (
                                            <div key={m.name} className="wk-chip">
                                                <span className="wk-avatar" style={{ background: m.color }}>{m.initials}</span> /* keep-inline */
                                ...
                                {hiveMessages.map(msg => (
                                    <div key={msg.id} className={`hv-msg ${msg.sender === (user?.name || 'You') ? 'me' : ''}`}>
                                        {msg.sender !== (user?.name || 'You') && (
                                            <span className="hv-avatar" style={{ background: msg.color }}>{msg.initials}</span> /* keep-inline */
                                        )}
