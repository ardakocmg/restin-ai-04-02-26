  ...
  return (
    <Card className={cn('overflow-hidden', className)}>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <CardTitle className="text-sm font-medium" style={{ color: '#A1A1AA' }}> /* keep-inline */
          {title}
        </CardTitle>
        {Icon && <Icon className="h-4 w-4" style={{ color: '#71717A' }} />} /* keep-inline */
      </CardHeader>
      <CardContent>
        ...
        {trend && (
          <p className="text-xs mt-1" style={{ /* keep-inline */
...
export default function Stats({ title, value, icon: Icon, trend, className }: DefaultStatsProps) {
  return (
    <Card className={cn('overflow-hidden', className)}>
      ...
      <CardContent>
        <div className="text-2xl font-bold" style={{ color: '#F5F5F7' }}> /* keep-inline */
          {value}
        </div>
        {trend && (
          <p className="text-xs mt-1" style={{ /* keep-inline */
