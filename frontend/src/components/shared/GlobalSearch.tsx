...
export default function GlobalSearch({ open, onOpenChange }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl p-0 bg-background border-border" aria-describedby="search-description">
        ...
        <div className="flex items-center border-b border-border px-4 py-3">
          ...
          <Input aria-label="Search pages, orders, guests, staff..."
            ...
            className="border-0 focus-visible:ring-0 text-base bg-transparent text-foreground placeholder:text-muted-foreground"
            autoFocus
            autoComplete="off"
          />
        ...
        <ScrollArea className="max-h-[400px]">
          ...
          {allItems.length > 0 && (
            <div className="p-2">
                ...
                return (
                  ...
                  >
                    <div className={cn(
                      ...
                      <Icon className={cn("h-3.5 w-3.5", selectedIndex === idx ? "text-red-500" : "text-muted-foreground")} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className={cn("text-sm font-medium truncate", selectedIndex === idx ? "text-foreground" : "text-secondary-foreground")}>
                        {page.title}
                      </div>
              ...
              {/* Data Section Header */}
              {hasData && (
                <div className={cn("px-2 py-1.5 text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2", hasPages && "mt-3 border-t border-border pt-3")}>
                  ...
                  <div key={type} className="mb-2">
                    ...
                    {entities.slice(0, 5).map((entity, entityIdx) => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex-1 min-w-0">
                            <div className={cn("text-sm font-medium truncate", selectedIndex === globalIdx ? "text-foreground" : "text-secondary-foreground")}>
                              {entityName}
                            </div>
                            {entitySub && (
                              <div className="text-xs text-muted-foreground truncate">{entitySub}</div>
                            )}
                          </div>
                          <Badge variant="outline" className="ml-2 shrink-0 text-[9px] border-border text-muted-foreground">
                            {type.replace(/_/g, ' ')}
                          </Badge>
                        </button>
                      );
        ...
        <div className="border-t border-border px-4 py-2 text-xs text-muted-foreground flex items-center justify-between">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1">
              <kbd className="pointer-events-none inline-flex h-5 select-none items-center gap-1 rounded border border-border bg-card px-1.5 font-mono text-[10px]">
                ↑↓
              </kbd>
              navigate
            </span>
