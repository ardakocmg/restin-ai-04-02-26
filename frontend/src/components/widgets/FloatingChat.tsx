...
export default function FloatingChat() {
            ...
            >
                ...
                {totalUnread > 0 && (
                    <motion.div
                        initial={{ scale: 0 }}
                        animate={{ scale: 1 }}
                        className="absolute -top-1 -right-1 h-5 min-w-[20px] rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center px-1 border-2 border-zinc-950"
                    >
                        {totalUnread}
                    </motion.div>
                )}
    ...
    if (isMinimized) {
        return (
            <motion.div
                className="fixed bottom-6 right-6 z-50 h-12 bg-zinc-900 border border-zinc-700 rounded-xl flex items-center gap-2 px-4 shadow-2xl cursor-pointer"
            ...
            >
                ...
                <span className="text-sm text-zinc-300 font-medium">Hive Chat</span>
                {totalUnread > 0 && (
    ...
    return (
        <AnimatePresence>
            <motion.div
                className="fixed bottom-6 right-6 z-50 w-[380px] h-[520px] bg-zinc-950 border border-zinc-800 rounded-2xl shadow-[0_8px_60px_rgba(0,0,0,0.6)] flex flex-col overflow-hidden"
            ...
            >
                {/* Header */}
                <div className="h-12 border-b border-zinc-800 flex items-center justify-between px-3 bg-zinc-900/80 backdrop-blur-sm flex-shrink-0">
                ...
                <AnimatePresence>
                    {showChannels && (
                        <motion.div
                            className="border-b border-zinc-800 bg-zinc-900/60 overflow-hidden flex-shrink-0"
                            ...
                            <div className="p-2 space-y-0.5">
                                {CHANNELS.map(ch => (
                                    ...
                                    >
                                        ...
                                        {ch.unread > 0 && (
                                            <Badge className="h-4 min-w-[16px] px-1 text-[10px] bg-blue-600 text-white border-0">{ch.unread}</Badge>
                                        )}
                                    </button>
                                ))}
                            </div>
                    ...
                    {channelMessages.length === 0 ? (
                        <div className="flex-1 flex items-center justify-center h-full">
                            <div className="text-center">
                                ...
                                <p className="text-xs text-zinc-600">No messages in #{activeChannelData?.name}</p>
                            </div>
                        </div>
                    ) : (
                        channelMessages.map(msg => (
                            ...
                            >
                                <div className={`h-7 w-7 rounded-full ${msg.senderColor} flex items-center justify-center flex-shrink-0 mt-0.5`}>
                                    <span className="text-white text-[9px] font-bold">{msg.senderInitials}</span>
                                </div>
                                    ...
                                    {msg.reactions && Object.keys(msg.reactions).length > 0 && (
                                        <div className="flex gap-1 mt-1">
                                            {Object.entries(msg.reactions).map(([emoji, count]) => (
                                                <button key={emoji} className="px-1.5 py-0.5 rounded-full bg-zinc-800 text-[10px] hover:bg-zinc-700 transition-colors">
                                                    {emoji} {count}
                                                </button>
                                            ))}
                                        </div>
                                    ...
                                    <div className="flex gap-0.5 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        {['👍', '❤️', '😂', '🔥', '🎉'].map(emoji => (
                                            <button
                                                ...
                                                className="h-5 w-5 rounded hover:bg-zinc-800 text-[10px] flex items-center justify-center transition-colors"
                                            >
                                                {emoji}
                                            </button>
                                        ))}
                ...
                {/* Input */}
                <div className="p-3 border-t border-zinc-800 bg-zinc-900/50 flex-shrink-0">
