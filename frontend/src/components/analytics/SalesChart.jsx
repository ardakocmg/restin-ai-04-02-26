...
export default function SalesChart({ data, title = "Hourly Sales" }) {
    ...
    return (
        <div className="w-full bg-zinc-900 border border-white/10 rounded-xl p-4 shadow-lg">
                ...
                {chartData.map((d, i) => {
                    ...
                    return (
                        <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                            ...
                            >
                                {/* Tooltip */}
                                <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-zinc-800 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap border border-white/10 z-10">
                                    €{d.value}
                                </div>
                            </div>
                            <span className="text-xs text-zinc-500 font-mono">{d.label}</span>
                        </div>
                    );
                })}
