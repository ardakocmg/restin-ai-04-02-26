...
export default function VoicePage() {
    ...
    const { data: logs = [] } = useQuery({
        queryKey: ['voice-logs', tenantId],
        queryFn: () => voiceService.getCallLogs(tenantId),
        enabled: !!tenantId
    });

    // Mock logs if empty (for demo/dev)
    const activities = logs.length > 0 ? logs.map(log => ({
        time: new Date(log.timestamp).toLocaleString(),
        guest: `Call ${log.id.substring(0, 8)}`,
        action: `Duration: ${log.duration}s, Outcome: ${log.outcome}`,
        status: log.outcome
    })) : [
    ...
    onClick = {() => router.push('/dashboard/voice/settings')
}
                        >
...
<Button variant=\"ghost\" className=\"w-full mt-4 text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] hover:text-white\">
    ...
    <div className=\"flex gap-2\">
        < Button variant =\"ghost\" size=\"sm\" className=\"text-xs font-bold text-zinc-500\">Filter</Button>
            < Button variant =\"ghost\" size=\"sm\" className=\"text-xs font-bold text-zinc-500\" onClick={() => router.push('/dashboard/voice/logs')}>View All</Button>
                            </div >
                        </div >
