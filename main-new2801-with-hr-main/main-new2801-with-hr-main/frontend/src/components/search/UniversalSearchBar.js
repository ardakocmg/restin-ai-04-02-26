    ...
    <div className="relative w-full">
      <div className="relative">
        ...
        <input
          ...
          className="w-full rounded-md border border-zinc-700 bg-zinc-900 py-2 pl-10 pr-24 text-zinc-100 placeholder:text-zinc-500 focus:outline-none focus:ring-2 focus:ring-zinc-600"
        />
        ...
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-2">
          {loading ? (
            ...
            <span className="text-xs text-zinc-500">min 2</span>
          )}
        </div>
      </div>
      ...
      {open && enabled && (err || groups.length > 0) && (
        <div className="absolute z-50 mt-2 w-full overflow-hidden rounded-md border border-zinc-700 bg-zinc-950 shadow-lg">
          {err ? (
            <div className="p-3 text-sm text-red-300">{err}</div>
          ) : (
            <div className="max-h-72 overflow-auto">
              {groups.map((g) => (
                <div key={g.key} className="border-b border-zinc-800 last:border-b-0">
                  ...
                  {g.items.slice(0, 8).map((item, idx) => (
                    <button
                      ...
                      className="flex w-full flex-col gap-1 px-3 py-2 text-left hover:bg-zinc-900"
                    >
                      <div className="text-sm text-zinc-100">{pickLabel(item)}</div>
                      {pickSub(item) ? (
                        <div className="text-xs text-zinc-500">{pickSub(item)}</div>
                      ) : null}
                    </button>
                  ))}
                </div>
