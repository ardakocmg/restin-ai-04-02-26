    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          const Icon = m.icon;
          return (
            <Card key={i} className="bg-zinc-950 border-white/5 shadow-2xl">
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        <div className="relative flex-1">
          ...
          <Input
            ...
            className="bg-zinc-950 border-white/5 pl-10 h-11 text-sm font-medium"
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
        </div>
        <Tabs value={activeSegment} onValueChange={setActiveSegment} className="w-full md:w-auto">
          <TabsList className="bg-zinc-950 border border-white/5 h-11 p-1">
      ...
      <div className="space-y-3">
        {loading ? (
          ...
          <div className="py-20 text-center bg-zinc-950 border border-white/5 rounded-2xl">
            <Users className="w-12 h-12 text-zinc-800 mx-auto mb-4" />
            <h3 className="text-white font-black uppercase italic tracking-tighter text-xl">No Guests Found</h3>
            <p className="text-zinc-500 font-bold uppercase text-[11px]">Expand your search or clear filters</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {guests.map(guest => (
              <Card
                ...
                className="bg-zinc-950 border-white/5 hover:border-white/10 transition-all cursor-pointer group shadow-xl"
              >
                <CardContent className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div className="h-12 w-12 rounded-xl bg-gradient-to-br from-zinc-800 to-zinc-900 border border-white/5 flex items-center justify-center text-xl font-black text-zinc-500">
                        {guest.first_name[0]}{guest.last_name[0]}
                      </div>
                      <div>
                        <h3 className="font-black text-white uppercase tracking-tight text-sm group-hover:text-red-500 transition-colors">
                          {guest.first_name} {guest.last_name}
                        </h3>
                        <div className="flex items-center gap-2 mt-1">
                          {guest.tags.map(tag => (
                            <Badge key={tag} className="bg-zinc-900 text-zinc-400 border-white/5 text-[8px] font-black uppercase tracking-widest px-1.5 py-0">
                              {tag}
                            </Badge>
                          ))}
                        </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-6">
                    <div className="p-2 rounded-lg bg-zinc-900/50 border border-white/5 text-center">
                      <p className="text-[8px] font-black text-zinc-500 uppercase">Visits</p>
                      <p className="text-sm font-black text-white">{guest.visit_summary?.total_visits || 0}</p>
                    </div>
                    <div className="p-2 rounded-lg bg-zinc-900/50 border border-white/5 text-center">
                      <p className="text-[8px] font-black text-zinc-500 uppercase">Lvl</p>
                      <p className="text-xs font-black text-blue-500 uppercase italic">{guest.loyalty?.tier || 'BRN'}</p>
                    </div>
                    <div className="p-2 rounded-lg bg-zinc-900/50 border border-white/5 text-center">
                      <p className="text-[8px] font-black text-zinc-500 uppercase">Spend</p>
                      <p className="text-sm font-black text-green-500">€{Math.round(guest.visit_summary?.total_spend || 0)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-white/5">
                    <div className="flex items-center gap-2 text-[10px] font-bold text-zinc-500 uppercase">
                      ...
                      {guest.visit_summary?.last_visit ? `Last seen ${new Date(guest.visit_summary.last_visit).toLocaleDateString()}` : 'New Guest'}
                    </div>
