...
export default function SummaryDashboard() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-screen bg-zinc-950">
      <div className="animate-pulse flex flex-col items-center gap-4">
        ...
        <p className="text-zinc-500 font-bold uppercase tracking-widest text-xs">Synchronizing HR Intelligence...</p>
      </div>
    </div>
  );

  if (!data) return <div className="p-8 bg-zinc-950 min-h-screen text-zinc-500">No core intelligence data available.</div>;
  ...
  return (
    <div className="p-6 space-y-12 bg-zinc-950 min-h-screen text-zinc-100 pb-20">
      {/* Header */}
      <div className="flex justify-between items-end border-b border-white/5 pb-8">
        <div>
          <h1 className="text-4xl font-black text-white uppercase tracking-tighter leading-none mb-2">Human Resources</h1>
          <p className="text-zinc-500 font-bold uppercase tracking-[0.2em] text-[10px]">Unified Intelligence & Global Control</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="text-right">
            <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Venue</label>
            <select
              value={selectedCompany}
              onChange={(e) => setSelectedCompany(e.target.value)}
              className="border rounded-lg px-4 py-1.5 bg-zinc-900 border-white/5 text-zinc-100 text-[10px] font-bold uppercase tracking-widest focus:ring-1 focus:ring-red-500/50 outline-none"
            >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
        {[
          ...
          <button
            ...
            className="flex flex-col items-center justify-center h-28 rounded-xl border border-white/5 bg-[#121214] hover:bg-zinc-800 hover:border-red-500/30 transition-all group shadow-xl"
          >
            <div className="mb-3 p-2 rounded-lg bg-zinc-800 text-zinc-400 group-hover:text-red-500 transition-colors">
              <item.icon className="h-5 w-5" />
            </div>
            <span className="text-[10px] font-bold text-zinc-100 uppercase tracking-tighter leading-tight">{item.title}</span>
          </button>
        ))}
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {data.kpi_metrics.map((metric, idx) => {
          ...
          return (
            <Card key={idx} className="border-white/5 bg-zinc-900/40 shadow-xl backdrop-blur-md">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{metric.label}</p>
                    <p className="text-4xl font-black text-white mt-2 font-heading tracking-tight">{metric.value}</p>
                    <p className="text-[10px] text-zinc-500 mt-2 font-bold uppercase tracking-tighter">Synchronized Intelligence</p>
                  </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <Card className="border-white/5 bg-zinc-900/40">
          <CardHeader><CardTitle className="text-[11px] uppercase tracking-widest text-zinc-400 font-bold">Temporal Headcount Trend</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="border-white/5 bg-zinc-900/40">
          <CardHeader><CardTitle className="text-[11px] uppercase tracking-widest text-zinc-400 font-bold">Employment Archetype</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
        ...
        <Card className="border-white/5 bg-zinc-900/40">
          <CardHeader><CardTitle className="text-[11px] uppercase tracking-widest text-zinc-400 font-bold">Demographic Bracket</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={200}>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card className="border-white/5 bg-zinc-900/40">
          <CardHeader><CardTitle className="text-[11px] uppercase tracking-widest text-zinc-400 font-bold">Attrition vs Engagement Dynamics</CardTitle></CardHeader>
          <CardContent>
        ...
        <Card className="border-white/5 bg-zinc-900/40">
          <CardHeader><CardTitle className="text-[11px] uppercase tracking-widest text-zinc-400 font-bold">Gender Polarity Matrix</CardTitle></CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={250}>
              <PieChart>
                <Pie
      ...
      {/* Footer */}
      <div className="pt-12 border-t border-white/5 flex items-center justify-between text-zinc-600 font-bold uppercase text-[9px] tracking-[0.3em]">
