      ...
      <div className="min-h-screen bg-zinc-950 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-6">
            ...
            <p className="text-zinc-400">HR document management</p>
          </div>
    ...
    <div className="min-h-screen bg-zinc-950 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-50 mb-2">Documents & Compliance</h1>
            <p className="text-slate-600 dark:text-slate-400">Employee documents and certifications</p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {documents.map((doc) => (
              <div key={doc.id} className="bg-white dark:bg-slate-900 rounded-lg p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
                ...
                <p className="text-sm text-zinc-500 mb-1">{doc.display_id}</p>
                <p className="text-xs text-zinc-600 capitalize mb-3">{doc.type} • {doc.visibility}</p>

                {doc.expiry_date && (
                  <div className="text-sm">
                    <p className="text-zinc-500">Expires:</p>
                    <p className={isExpiringSoon(doc.expiry_date) ? "text-yellow-400" : "text-white"}>
