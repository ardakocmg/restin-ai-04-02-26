...
export default function InternalOrders() {
  ...
  return (
    <PageContainer title="Internal Orders" description="Orders from outlets to central kitchen" actions={<button className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"><Plus className="h-4 w-4" />New Order</button>}>
      <div className="space-y-4">
        {loading ? <Card><CardContent className="p-8 text-center">Loading...</CardContent></Card> : orders.length === 0 ? <Card><CardContent className="p-8 text-center text-slate-400">No internal orders</CardContent></Card> : orders.map((order) => (
