...
import { Building2, Lock, Shield, ChefHat, LayoutGrid, Monitor, Undo2 } from "lucide-react";
...
export default function LoginNew() {
  ...
  const [loginTarget, setLoginTarget] = useState("pos");
  ...
  const handlePinLogin = async () => {
    ...
    try {
      ...
      if (response.data.requires_mfa) {
        ...
        setPinSuccess(true);
        setTimeout(() => handleLoginSuccess(response.data), 300);
      }
    } catch (error) {
  ...
  const handleLoginSuccess = (data) => {
    ...
    if (data.allowedVenueIds && data.allowedVenueIds.length > 1) {
      ...
      login(token, { ...data.user, venue_id: venueId });
  ...
  const handleVenueSelect = () => {
    ...
    login(userToken, { ...userData.user, venue_id: selectedVenue });
